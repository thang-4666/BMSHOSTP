SET DEFINE OFF;
CREATE OR REPLACE PROCEDURE od0091_2(PV_REFCURSOR IN OUT PKG_REPORT.REF_CURSOR,
                                     OPT          IN VARCHAR2,
                                     pv_BRID      IN VARCHAR2,
                                     TLGOUPS      IN VARCHAR2,
                                     TLSCOPE      IN VARCHAR2,
...
BEGIN
  ...
  OPEN PV_REFCURSOR FOR
    SELECT V_STRNOIIN NOIIN, SUM(OD.matchqtty) matchqtty, OD.custodycd, OD.fullname,
             od.CUSTATCOM,
             od.address,
             od.exectype,
             od.txdate,
      ...
      from (SELECT
             ...
             /*v_taxrate+v_whtax*/   (DECODE(CF.VAT, 'Y', v_taxrate, 'N', 0) +
                          DECODE(CF.WHTAX, 'Y', v_whtax, 'N', 0)) TAX_RATE,
                ...
                (CASE
                  ...
                  ELSE
                   (ROUND((IO.MATCHQTTY * IO.MATCHPRICE *
                          (DECODE(CF.VAT, 'Y', v_taxrate, 'N', 0) +
                          DECODE(CF.WHTAX, 'Y', v_whtax, 'N', 0))) / 100,
                          0) + NVL(sts.ARIGHT, 0))
                END)
               ELSE
                0
     ...
     group by OD.custodycd, OD.fullname,
             od.CUSTATCOM,
             od.address,
             od.exectype,
             od.txdate,
