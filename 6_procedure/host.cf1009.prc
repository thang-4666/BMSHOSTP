...
-- return;
    ...
    OPEN PV_REFCURSOR FOR
        ...
        (
            ...
            select max(tse.autoid) orderid, tse.custid, tse.afacctno, max(tse.tllog_autoid) autoid,tse.txtype,tse.txcd,
                tse.busdate, case when tse.TLTXCD = '8867' then 'Bán ' || trim(to_char(sum(tse.namt),'999,999,999,999,999,999')) || ' ' || max(tse.symbol) || ' ngày ' || TO_CHAR(tse.busdate)
        ...
        0 ci_credit_amt, 0 ci_debit_amt,
            ...
            where tci.busdate >= v_FromDate AND tci.busdate <= v_ToDate
              ...
              max(CASE WHEN tci.tltxcd IN ('2678','7702','5566','2695','2674','2679','1156','2624') THEN 0000000001
                 ...
                 tci.busdate,
                 case when TCI.TLTXCD = '8865' then 'Trả tiền mua CK ngày ' || to_char(max(od.txdate),'dd/mm/rrrr')--TO_CHAR(tci.busdate)
                    when TCI.TLTXCD = '8889' then 'Trả tiền mua CK ngày ' || to_char(max(od.txdate),'dd/mm/rrrr')--TO_CHAR(tci.busdate)
                    when TCI.TLTXCD = '8855' then 'Trả phí mua CK ngày ' || to_char(max(od.txdate),'dd/mm/rrrr')--TO_CHAR(tci.busdate)
                    when TCI.TLTXCD = '8866' then 'Nhận tiền bán CK ngày ' || to_char(max(od.txdate),'dd/mm/rrrr')--TO_CHAR(tci.busdate)
                    else  'Trả phí bán CK ngày ' || to_char(max(od.txdate),'dd/mm/rrrr')--TO_CHAR(tci.busdate)
                    end TXDESC,
                '' symbol, 0 se_credit_amt, 0 se_debit_amt,
        ...
        (
            ...
            from v_getbuyorderinfo V, afmast af,  (SELECT * FROM CFMAST WHERE FNC_VALIDATE_SCOPE(BRID, CAREBY, TLSCOPE, pv_BRID, TLGOUPS)=0) cf
