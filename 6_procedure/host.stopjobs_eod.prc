...
begin
  ...
  plog.error(pkgctx, 'Init stopjobs_eod');
  ...
  begin
    ...
    BEGIN
    plog.error(pkgctx, 'STOP GTWJBS_#MONEYTRANSFER');
    ...
    EXCEPTION WHEN OTHERS THEN
        plog.error(pkgctx, sqlerrm);
        plog.error(pkgctx, 'Disable GTWJBS_#MONEYTRANSFER');
        dbms_scheduler.disable(name => 'GTWJBS_#MONEYTRANSFER', force =>  true);
    END;

    BEGIN
    plog.error(pkgctx, 'STOP GTWJBS_#CARIGHTOFFREGISTER');
    ...
    EXCEPTION WHEN OTHERS THEN
        plog.error(pkgctx, sqlerrm);
        plog.error(pkgctx, 'Disable GTWJBS_#CARIGHTOFFREGISTER');
        dbms_scheduler.disable(name => 'GTWJBS_#CARIGHTOFFREGISTER', force =>  true);
    END;
  exception
