SET DEFINE OFF;
CREATE OR REPLACE PROCEDURE se200902
(
...
BEGIN
   ...
   if PV_CUSTODYCD = 'ALL' then
    ...
    OPEN PV_REFCURSOR
    FOR
        select fullname,symbol,custodycd,recustodycd, qtty ,qttywft,catype,tradeplace, nvl(symbol1,'1') symbol1 from (
select cf.fullname,sb.symbol,cf.custodycd,ses.recustodycd, sum(log.qtty) qtty ,sum(log.qtty) qttywft,sep.catype,a1.cdcontent tradeplace
        ...
        where cf.custid = af.custid
              ...
              --  and ses.tltxcd=log.tltxcd
                and log.deltd <> 'Y'
                and sep.catype = '021'
                and a1.cdname='TRADEPLACE' and a1.cdtype='SE' and a1.cdval= (case when SUBSTR(sb.symbol,-3,3) = 'WFT' then (select tradeplace from sbsecurities where codeid=sb.refcodeid)
                    else sb.tradeplace end)
                and ses.txdate BETWEEN TO_DATE (F_DATE  ,'DD/MM/YYYY') AND TO_DATE (T_DATE  ,'DD/MM/YYYY')
        group by cf.fullname,sb.symbol,cf.custodycd,ses.recustodycd,sep.catype,a1.cdcontent) R left join
        (select sb.symbol symbol1,cf.custodycd custodycd1,ses.recustodycd recustodycd1
        from SE2244_LOG log, cfmast cf, afmast af, sepitlog sep, sbsecurities sb,allcode a1,
        (  select autoid, recustodycd recustodycd, txdate, '2244' tltxcd, outward ward from SESENDOUT where deltd = 'N'
           union all
           select autoid, rcvcustodycd recustodycd, txdate, '2257' tltxcd, inward ward from sefulltransfer_log where deltd = 'N'
        ...
        where cf.custid = af.custid
              ...
              --  and ses.tltxcd=log.tltxcd
                ...
                and ses.txdate BETWEEN TO_DATE (F_DATE  ,'DD/MM/YYYY') AND TO_DATE (T_DATE  ,'DD/MM/YYYY')
        group by cf.fullname,sb.symbol,cf.custodycd,ses.recustodycd,sep.catype,a1.cdcontent) k
        on r.custodycd = k.custodycd1 and r.recustodycd = k.recustodycd1 and r.symbol = k.symbol1
                ;
