SET DEFINE OFF;
CREATE OR REPLACE PROCEDURE od0091 (
 ...
 SELECT V_STRNOIIN NOIIN, cf.custodycd,cf.fullname,CF.CUSTATCOM, cf.address,od.exectype, od.txdate,sts.cleardate,  od.afacctno,
        ...
        (   case when OD.execamt>0 and OD.feeacr=0  AND OD.TXDATE = V_CUR_DATE THEN  ODT.deffeerate
               ...
               (CASE WHEN (OD.execamt * OD.feeacr) = 0 THEN 0 ELSE
                   (CASE WHEN OD.TXDATE = V_CUR_DATE
                      ...
                      END)
               END)
             end ) FEE_RATE, /*v_taxrate+v_whtax*/(DECODE (CF.VAT,'Y', v_taxrate,'N',0 ) + DECODE (CF.WHTAX,'Y', v_whtax,'N',0 )) TAX_RATE,
       ...
       (CASE WHEN OD.execamt = 0 THEN 0 ELSE
        ...
        END)   feeamt,
             ...
             (CASE WHEN od.EXECTYPE IN('NS','SS','MS') AND (CF.VAT = 'Y' or CF.WHTAX='Y') /*AND CF.VAT = 'Y'*/ THEN
                (CASE WHEN IO.iodtaxsellamt <> 0 THEN IO.iodtaxsellamt
           ELSE (ROUND((IO.MATCHQTTY * IO.MATCHPRICE *(DECODE (CF.VAT,'Y', v_taxrate,'N',0 ) + DECODE (CF.WHTAX,'Y', v_whtax,'N',0 )))/100, 0) + NVL(sts.ARIGHT, 0)) END)
