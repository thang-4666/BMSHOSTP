SET DEFINE OFF;
CREATE OR REPLACE PROCEDURE "SP_BD_GETORDERFORCANCEL_SUM" (PV_REFCURSOR IN OUT PKG_REPORT.REF_CURSOR, CUSTODYCD IN VARCHAR2)
  IS
...
BEGIN
    ...
    SELECT PRICETYPE, DESC_EXECTYPE, SYMBOL, DESC_STATUS ORSTATUS, CANCELSTATUS,
        ...
        AFACCTNO, CUSTODYCD, FEEDBACKMSG, EXECTYPE, CODEID, BRATIO, ACCTNO ORDERID, REFORDERID, TXDATE, TXTIME
    FROM
    -- OD
    (SELECT CFMAST.CUSTODYCD, MST.TXDATE, MST.REFORDERID, MST.AFACCTNO, MST.orderid ACCTNO, '' ORGACCTNO, MST.EXECTYPE,MST.REFORDERID REFACCTNO,
        ...
        (CASE WHEN MST.CANCELQTTY>0 THEN '?? h?y'  WHEN MST.EDITSTATUS='C' THEN '?ang h?y' ELSE '----' END) CANCELSTATUS,
        (CASE WHEN MST.CANCELQTTY>0 THEN 0 WHEN MST.EDITSTATUS='C' THEN 0 ELSE 1 END) ISCANCEL,
    ...
    FROM CFMAST, AFMAST CF,(SELECT OD1.*,OD2.EDSTATUS EDITSTATUS from odmast OD1,(SELECT * FROM ODMAST WHERE EDSTATUS IN ('C','A')) OD2   WHERE OD1.ORDERID=OD2.REFORDERID(+) AND substr(OD1.EXECTYPE,1,1) <> 'C' AND substr(OD1.EXECTYPE,1,1) <> 'A') MST,TLLOG TL, sbsecurities sb,
    ...
    WHERE MST.ORSTATUS <> '7' AND CF.ACCTNO=MST.AFACCTNO AND CFMAST.Custodycd=V_PARAFILTER
      ...
      UNION ALL
    -- OD + FO
    SELECT CFMAST.CUSTODYCD, TO_DATE(substr(MST.ACTIVATEDT,0,10),'DD/MM/YYYY') TXDATE, '' REFORDERID, MST.AFACCTNO, MST.ACCTNO, MST.ORGACCTNO, MST.EXECTYPE,MST.REFACCTNO REFACCTNO, MST.PRICETYPE,
        ...
        (CASE WHEN REFOD.CANCELQTTY>0 THEN '?? h?y' WHEN REFOD.EDITSTATUS='C' THEN '?ang h?y' ELSE '----' END) CANCELSTATUS,
        (CASE WHEN REFOD.CANCELQTTY>0 THEN 0 WHEN REFOD.EDITSTATUS='C' THEN 0 ELSE 1 END) ISCANCEL,
        (CASE WHEN REFOD.ADJUSTQTTY>0 THEN 'Amended' WHEN REFOD.EDITSTATUS='A' THEN 'Amending' ELSE '----' END) AMENDSTATUS,
        SYS.SYSVALUE CURRSECSSION,REFOD.HOSESESSION ODSECSSION,MST.Username maker, MST.CODEID, MST.BRATIO
    FROM CFMAST, AFMAST CF, FOMAST MST, (select OD1.*,OD2.EDSTATUS EDITSTATUS from odmast OD1,(SELECT * FROM ODMAST WHERE EDSTATUS IN ('C','A')) OD2   WHERE OD1.ORDERID=OD2.REFORDERID(+) AND substr(OD1.EXECTYPE,1,1) <> 'C' AND substr(OD1.EXECTYPE,1,1) <> 'A') REFOD,
        ALLCODE CD1, ALLCODE CD2, ALLCODE CD3, ALLCODE CD4, ALLCODE CD5, ALLCODE CD6, ALLCODE CD7, ALLCODE CD8, ALLCODE CD9 ,ORDERSYS SYS
    WHERE  REFOD.ORSTATUS <> '7' AND MST.DELTD='N' AND CF.ACCTNO=MST.AFACCTNO AND CFMAST.Custodycd=V_PARAFILTER
    AND MST.STATUS = 'A' AND MST.acctno = mst.orgacctno
      ...
      UNION ALL
    -- FO
    SELECT CFMAST.CUSTODYCD, TO_DATE(substr(MST.ACTIVATEDT,0,10),'DD/MM/YYYY') TXDATE,'' REFORDERID, MST.AFACCTNO, MST.ACCTNO, MST.ORGACCTNO, MST.EXECTYPE,(CASE WHEN MST.STATUS='R' THEN '' ELSE MST.REFACCTNO END) REFACCTNO, MST.PRICETYPE,
        ...
        CD5.cdcontent DESC_MATCHTYPE, CD6.cdcontent DESC_NORK, CD7.cdcontent DESC_PRICETYPE,
        MST.EXECQTTY, MST.EXECAMT,(CASE WHEN MST.EXECQTTY>0 THEN ROUND(MST.EXECAMT/1000/MST.EXECQTTY,2) ELSE 0 END) AVEXECPRICE, MST.REMAINQTTY-NVL(ROOT.ORDERQTTY,0) REMAINQTTY, SUBSTR(MST.activatedt,12,9) txtime, mst.CANCELQTTY , mst.AMENDQTTY ADJUSTQTTY,MST.BOOK BOOK,CD8.cdcontent VIA,'T' VIACD,
        ('----') CANCELSTATUS, 1 ISCANCEL, ('----') AMENDSTATUS,
        SYS.SYSVALUE CURRSECSSION,'N' ODSECSSION,MST.Username maker, MST.CODEID, MST.BRATIO
    FROM CFMAST, AFMAST CF, FOMAST MST, SECURITIES_INFO SYINFO,
    ...
    AND MST.DELTD='N' AND CF.ACCTNO=MST.AFACCTNO AND SYINFO.SYMBOL=MST.SYMBOL AND CFMAST.Custodycd=V_PARAFILTER
        AND CD1.cdtype ='FO' AND CD1.CDNAME='STATUS' AND CD1.CDVAL=MST.status
        AND SYS.SYSNAME='CONTROLCODE'  AND CFMAST.CUSTID=CF.CUSTID
        AND CD2.cdtype ='FO' AND CD2.CDNAME='EXECTYPE' AND CD2.CDVAL=MST.EXECTYPE
        AND CD3.cdtype ='FO' AND CD3.CDNAME='BOOK' AND CD3.CDVAL=MST.BOOK
...
WHERE ISCANCEL=1 AND REMAINQTTY>0
