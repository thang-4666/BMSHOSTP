...
select v_OnDate txdate, vn_BRID chi_nhanh, vn_TRADEPLACE san_gd, v_CustodyCD custody, v_AFAcctno afacc, v_Symbol lsymbol,
    ...
    SB.TRADEPLACE, a0.cdcontent TRADEPLACE_name,
        ...
        --- ban cho giao
        sum(case when SB.REFSYMBOL is null then (nvl(khop_qtty.execqtty,0) ) else 0 end) khop_qtty,
        --- ban cho khop
    ...
    (--- so luong chung khoan ban cho giao
        ...
        (
            ...
            where execqtty > 0
                ...
                and deltd <> 'Y'
            union all
            select odhist.codeid, odhist.afacctno, seacctno, execqtty, odhist.txdate
