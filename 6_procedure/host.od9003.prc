SET DEFINE OFF;
CREATE OR REPLACE PROCEDURE "OD9003" (
...
OPEN PV_REFCURSOR
FOR
select cf.fullname , (case when cf.country = '234' then cf.idcode else cf.tradingcode end) idcode, cf.custodycd,
    ...
    od.via, od.txdate, od.exectype sua_huy, af.acctno,
    ...
    (
        select * from odmast
        where exectype in ('CS','CB') and deltd = 'N' and txdate = V_IN_DATE
        union all
...
select cf.fullname , (case when cf.country = '234' then cf.idcode else cf.tradingcode end) idcode, cf.custodycd,
    sb.symbol, od_ex.orderqtty,--od.orderqtty,
    --od.quoteprice,
    (CASE WHEN OD.PRICETYPE IN ('ATO','ATC')THEN  OD.PRICETYPE  ELSE   TO_CHAR(OD.QUOTEPRICE) END ) quoteprice,
    od.via, od.txdate, od.exectype sua_huy, af.acctno,
    v_fullname_au fullname_au, v_licenseno_au licenseno_au,
    ...
    (
        select * from odmast
        where exectype in ('NB','NS','MS') and deltd = 'N' and txdate = V_IN_DATE
        union all
        select * from odmasthist
        where exectype in ('NB','NS','MS') and deltd = 'N' and txdate = V_IN_DATE
    ) od_ex, --Lenh sua thanh cong
    (
        select * from odmast
        where exectype in ('AB','AS') and deltd = 'N' and txdate = V_IN_DATE
        union all
        select * from odmasthist
        where exectype in ('AB','AS') and deltd = 'N' and txdate = V_IN_DATE
    ) od --Lenh yeu cau sua
    ...
    where od.afacctno = af.acctno
        ...
        and af.acctno like V_AFACCTNO
        AND (CASE WHEN OD.VIA = 'O' THEN 'O' ELSE 'F' END) LIKE V_VIATYPE
        and od.reforderid= od_ex.reforderid
    ;
