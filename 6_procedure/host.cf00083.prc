SET DEFINE OFF;
CREATE OR REPLACE PROCEDURE "CF00083" (
...
OPEN PV_REFCURSOR
  ...
  SELECT OD.AFACCTNO, OD.TXDATE, OD.ORDERID,
        ...
        STS.CLEARDATE, io.symbol
  FROM odmast OD, stschd STS, ODTYPE ODT, SYSVAR SYS, (SELECT * FROM CFMAST WHERE FNC_VALIDATE_SCOPE(BRID, CAREBY, TLSCOPE, pv_BRID, TLGOUPS)=0) cf, afmast af,
  ...
  SELECT OD.AFACCTNO, OD.TXDATE, OD.ORDERID,
        ...
        STS.CLEARDATE, io.symbol
  FROM odmasthist OD, stschdhist STS, ODTYPE ODT, SYSVAR SYS, (SELECT * FROM CFMAST WHERE FNC_VALIDATE_SCOPE(BRID, CAREBY, TLSCOPE, pv_BRID, TLGOUPS)=0) cf, afmast af,
    vw_iod_all io,AFTYPE AFT
  WHERE  OD.ORDERID = STS.ORGORDERID AND STS.DUETYPE IN ('RM', 'RS')
      AND STS.DELTD = 'N' AND OD.DELTD = 'N'
      and od.orderid = io.orgorderid
