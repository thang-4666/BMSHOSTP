...
-- Prepare Parameters
   V_STRTLID:= TLID;
    ...
    OPEN PV_REFCURSOR
    FOR
    SELECT restype, rlstype, custodycd, afacctno, rlsdate, overduedate, lnschdid, rlsprin, paid, lnprin, intamt, feeintamt ,fullname, mnemonic
    FROM (
        select NVL(DF.ISVSD,'N') ISVSD, nvl(cfb.shortname,l_companyshortname) restype,
            ...
            cf.fullname, ln.acctno lnacctno, aft.mnemonic
        from vw_lnmast_all ln, vw_lnschd_all ls, (SELECT * FROM CFMAST WHERE FNC_VALIDATE_SCOPE(BRID, CAREBY, TLSCOPE, p_BRID, TLGOUPS)=0) cf, afmast af,aftype aft, cfmast cfb,
            ...
            (
                    ...
                    select mst.afacctno, tn.AUTOID, nhom.reacctno, mst.recustid, tn.fullname
                    from regrplnk nhom, regrp tn,
                    (
                        select re.afacctno, MAX(mst.custid) recustid, max(re.reacctno) reacctno
                        from reaflnk re, remast mst, RETYPE
                        where V_CURRDATE between re.frdate and nvl(re.clstxdate,re.todate)-1
                            and re.reacctno =  mst.acctno
                            and re.status <> 'C' and re.deltd <> 'Y'
                            AND mst.actype = RETYPE.ACTYPE
                            AND RETYPE.rerole = 'RM'
                        GROUP BY re.AFACCTNO
                    ) mst
                    where nhom.deltd <> 'Y' and tn.autoid = nhom.refrecflnkid
                        and nhom.reacctno = mst.reacctno
                        and V_CURRDATE between nhom.frdate and nvl(nhom.clstxdate,nhom.todate)-1
                ) re,
