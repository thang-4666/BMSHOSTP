SET DEFINE OFF;
CREATE OR REPLACE PROCEDURE cf0077 (
  ...
  -- END IF;
   ...
   -- GET REPORT'S DATA
      ...
      SELECT getcurrdate CURR,CF.FULLNAME,  A2.CDCONTENT VIA ,NVL(BR.BRNAME,'') BRNAME,CF.IDPLACE,
            ...
            NVL(CF.NOTE,'') NOTE , A1.CDCONTENT STATUS, NVL(CF.OPNDATE,'') OPNDATE  ,
             TL.TLNAME, TLOG.TXDATE,NVL(CF1.CUSTODYCD,'') CUSTODYCD
       FROM (SELECT * FROM CFMASTTEMP_HIST UNION ALL SELECT * FROM CFMASTTEMP) CF,ALLCODE A1  , ALLCODE A2  , BRGRP BR,  TLPROFILES TL,CFMAST CF1,
            (SELECT * FROM VW_TLLOG_ALL WHERE TLTXCD='0037' AND TXSTATUS='1' ) TLOG
       WHERE  A1.CDNAME = 'STSCFTMP' AND A1.CDTYPE = 'CF' AND CF.STATUS = A1.CDVAL
              AND CF.AREA =BR.GLMAPID(+)
             AND NVL(TLOG.TLID,'000')=TL.TLID(+)
             AND CF.IDCODE=CF1.IDCODE(+)
             ...
             ORDER BY TO_DATE(CF.OPNDATE,'DD/MM/YYYY'),CF1.CUSTODYCD
