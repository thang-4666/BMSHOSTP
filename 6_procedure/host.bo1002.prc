SET DEFINE OFF;
CREATE OR REPLACE PROCEDURE "BO1002" (
...
FROM BONDDEAL MST, CFMAST CF, SBSECURITIES SB, ALLCODE A0, ALLCODE A2,
    (SELECT custid, fullname FROM CFMAST WHERE FNC_VALIDATE_SCOPE(BRID, CAREBY, TLSCOPE, pv_BRID, TLGOUPS)=0 UNION ALL SELECT 'NULL' CUSTID, '---' FULLNAME FROM DUAL) RCF,
    sysvar sys
...
FROM BONDDEAL MST, (SELECT * FROM CFMAST WHERE FNC_VALIDATE_SCOPE(BRID, CAREBY, TLSCOPE, pv_BRID, TLGOUPS)=0) CF, SBSECURITIES SB, ALLCODE A0, ALLCODE A2,
