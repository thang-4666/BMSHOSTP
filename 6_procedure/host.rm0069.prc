SET DEFINE OFF;
CREATE OR REPLACE PROCEDURE rm0069
   (
...
BEGIN
    ...
    select refacctno
        into v_BankAcc
    from CRBDEFACCT
    where refbank=DECODE(UPPER(PV_BANKTYPE),'ALL','BIDV',PV_BANKTYPE) and trfcode='TRFCICAMT' and rownum<=1;
...
--AND LG.AUTOID LIKE v_TxNum
--AND req.TRFCODE LIKE v_NumType
--AND REQ.objname in ('6668','6642')
AND REQ.trfcode='TRFCICAMT'
...
AND case when PV_TIME ='ALL' then 'ALL'
    when trunc(req.createdate) <>  req.txdate then '001'
