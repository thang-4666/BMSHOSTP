SET DEFINE OFF;
CREATE OR REPLACE PROCEDURE getcancelorder (PV_REFCURSOR IN OUT PKG_REPORT.REF_CURSOR,AFACCTNO IN VARCHAR2)
  IS
...
BEGIN
    ...
    SUBSTR(AFACCTNO,1,4) || '.' || SUBSTR(AFACCTNO,5,6) CIACCTNO, '' TXNUM, FO.TXDATE, '' TXTIME,
    ...
    0 STOPPRICE, FO.PRICE*INF.TRADEUNIT LIMITPRICE, FO.QUANTITY ORDERQTTY, FO.REMAINQTTY, FO.EXECQTTY, 0 STANDQTTY,
    FO.CANCELQTTY, 0 ADJUSTQTTY, 0 REJECTQTTY,'' REJECTCD,'' CUSTID,0 EXPRICE,0 EXQTTY,A9.CDCONTENT DELTD,
    'N' OODSTATUS,OUTPRICEALLOW,'O' puttype,'' contraorderid,'' contrafrm, a11.cdcontent isdisposal
    FROM FOMAST FO, AFMAST AF, CFMAST CF, ALLCODE A0,ALLCODE A1,ALLCODE A2,ALLCODE A3,
        ALLCODE A4,ALLCODE A5,ALLCODE A6,ALLCODE A7,ALLCODE A9,ALLCODE A10, allcode a11, SBSECURITIES SE,SECURITIES_INFO INF
    ...
    AND A11.CDTYPE = 'SY' AND A11.CDNAME = 'YESNO' AND A11.CDVAL= fo.isdisposal
    ...
    OOD.OODSTATUS,'Y' OUTPRICEALLOW,ODMAST.puttype,ODMAST.contraorderid,ODMAST.contrafrm, a11.cdcontent isdisposal
    FROM ODMAST, OOD ,ALLCODE A0,ALLCODE A1,ALLCODE A2,ALLCODE A3,
        ALLCODE A4,ALLCODE A5,ALLCODE A6,ALLCODE A7,ALLCODE A8,ALLCODE A9,ALLCODE A10, allcode a11, SBSECURITIES SE,
    ...
    AND OOD.BORS IN ('B','S') AND OOD.OODSTATUS <> 'C' AND SE.CODEID = ODMAST.CODEID AND (ODMAST.PRICETYPE NOT IN ('ATO','ATC') OR STATUS.CDCONTENT='Pending to Send')
    ...
    AND A11.CDTYPE = 'SY' AND A11.CDNAME = 'YESNO' AND A11.CDVAL= ODMAST.isdisposal
    AND ODMAST.AFACCTNO=V_AFACCTNO
    AND ODMAST.REMAINQTTY>0
    ...
    AND ODMAST.ORDERID NOT IN (SELECT REFORDERID FROM ODMAST WHERE REFORDERID IS NOT NULL)
    AND A0.CDVAL='001' AND ((replace(OOD.TXTIME,':','') >='135900' AND replace(OOD.TXTIME,':','') <='141700' and replace(TO_CHAR(SYSDATE,'HH24:MI:SS'),':','') <='141700')
