SET DEFINE OFF;
CREATE OR REPLACE PROCEDURE "CF0042"
   (
...
/*    V_STROPT := upper(OPT);
 ...
 if(upper(PV_CUSTODYCD) = 'ALL' OR LENGTH(PV_CUSTODYCD) < 1 )then
   ...
   if(upper(PV_SYMBOL) = 'ALL' OR LENGTH(PV_SYMBOL) < 1 )then
        V_STRSYMBOL := '%';
    else
...
SELECT CF.CUSTODYCD,br.brname,SB.SYMBOL, CF.FULLNAME, NVL(CF.TRADINGCODE,'') TRADINGCODE, CF.BRID, TL.MSGAMT,
...
FROM (SELECT * FROM CFMAST WHERE FNC_VALIDATE_SCOPE(BRID, CAREBY, TLSCOPE, pv_BRID, TLGOUPS)=0) CF,
     SBSECURITIES SB, VW_TLLOG_FULL TL, TLPROFILES TL1, TLPROFILES TL2,brgrp br,allcode al
WHERE SB.CODEID=TL.CCYUSAGE
      AND CF.BRID=BR.BRID
      AND CF.CUSTODYCD=TL.MSGACCT
      AND TL.TLTXCD='2287'
      AND DELTD='N'
...
ORDER BY CF.CUSTODYCD, TL.TXDATE, SB.SYMBOL, TL.TXTIME
         ;
