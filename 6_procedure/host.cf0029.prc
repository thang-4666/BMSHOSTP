...
SELECT LG.MAKER_DT ACTIONDATE, CF.CUSTODYCD, AF.ACCTNO, AFT.TYPENAME, CF.FULLNAME, 
    A1.CDCONTENT ACTION, ODPOLD.TYPENAME POLICYNAME_OLD , ODPOLD.ACTYPE POLICYID_OLD,
    ODPNEW.TYPENAME POLICYNAME_NEW , ODPNEW.ACTYPE POLICYID_NEW, 
...
FROM  (SELECT * FROM CFMAST 
    ...
    AFMAST AF, AFTYPE AFT, CIFEEDEF_EXT ODPOLD, CIFEEDEF_EXT ODPNEW, TLPROFILES TLP1, TLPROFILES TLP2, ALLCODE A1,
    (
        SELECT ACTION_FLAG, RECORD_KEY, MAKER_DT, MAKER_ID, 
            ...
            MAX(CASE WHEN COLUMN_NAME = 'ACTYPE' THEN FROM_VALUE ELSE '' END) OLD_REFAUTOID, 
            MAX(CASE WHEN COLUMN_NAME = 'ACTYPE' THEN  TO_VALUE ELSE '' END) NEW_REFAUTOID 
        FROM MAINTAIN_LOG
        WHERE TABLE_NAME = 'CIFEEDEF_EXTLNK'
...
WHERE LG.AFACCTNO = AF.ACCTNO AND AF.CUSTID = CF.CUSTID
    AND AF.ACTYPE = AFT.ACTYPE AND NVL(LG.OLD_REFAUTOID,99999) = ODPOLD.ACTYPE(+)
    AND NVL(LG.NEW_REFAUTOID,99999) = ODPNEW.ACTYPE(+) AND LG.MAKER_ID = TLP1.TLID
