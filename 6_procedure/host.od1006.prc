...
from (select * from odmast where txdate = V_P_date
    ...
    sbsecurities sb, (SELECT * FROM CFMAST WHERE FNC_VALIDATE_SCOPE(BRID, CAREBY, TLSCOPE, pv_BRID, TLGOUPS)=0)  cf, afmast af, ODTYPE ODT, aftype aft, SYSVAR SYS ,
    SYSVAR SYS1
