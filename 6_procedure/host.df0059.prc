SET DEFINE OFF;
CREATE OR REPLACE PROCEDURE df0059 (
...
SELECT A.CUSTODYCD, A.ACCTNO, A.fullname, A.idcode, A.idplace, A.iddate, A.address, A.phone, A.fax,A.GROUPID LNACCTNO, A.OPNDATE, SYMBOL,PARVALUE,TRADEPLACE,DEBIT, CREDIT,
    l_companyshortname RRNAME, SUM(RLSQTTY) TRADE , BBRID v_strbrid,
       case WHEN PV_BANKNAME ='ALL' THEN 'ALL'
            when a.rrtype = 'C' then l_companyshortname else cf.fullname end RRFULLNAME
    FROM (

 SELECT LNM.RRTYPE,LNM.CUSTBANK,CF.CUSTODYCD, AF.ACCTNO AFACCTNO, LNM.OPNDATE, DF.GROUPID, DF.ACCTNO, SB.SYMBOL
    ...
    lnM.rrtype || case when lnM.rrtype = 'B' then cf.shortname when lnM.rrtype = 'C' then l_companyshortname else null end rsctype,
           case when lnM.rrtype = 'C' then l_companyshortname
            when lnM.rrtype = 'B' then cfb.fullname
            else '' end rsctype_desc,
    SE.NAMT RLSQTTY,
    case when substr(cf.custodycd,4,1) = 'C' then '01232'
        when substr(cf.custodycd,4,1) = 'P' then '01231'
        else '01233' end DEBIT,
    case when substr(cf.custodycd,4,1) = 'C' then '01212'
        when substr(cf.custodycd,4,1) = 'P' then '01211'
        else '01213' end CREDIT
...
FROM DFTYPE DFT, AFMAST AF, (SELECT * FROM CFMAST WHERE FNC_VALIDATE_SCOPE(BRID, CAREBY, TLSCOPE, BBRID, TLGOUPS)=0) CF, SBSECURITIES SB, vw_dfmast_all DF,
    ...
    and case when l_BANKNAME = 'ALL' then 1
                when l_BANKNAME = l_companyshortname and LNM.rrtype = 'C' then 1
    ...
    SELECT NULL RRTYPE, NULL CUSTBANK, CF.CUSTODYCD, AF.ACCTNO AFACCTNO, se.txdate OPNDATE, NULL GROUPID, NULL ACCTNO,
        SB.SYMBOL, parvalue , se.msgamt TRADE,
     CASE WHEN sb.markettype = '001' AND sb.sectype IN ('003','006','222','333','444') THEN utf8nums.c_const_df_marketname
    WHEN  nvl(sb.tradeplace,'') = '001' THEN ' HOSE'
    WHEN  nvl(sb.tradeplace,'') = '002' THEN ' HNX'
    WHEN  nvl(sb.tradeplace,'') = '005' THEN ' UPCOM'  END tradeplace,
...
) A, cfmast cf WHERE     A. RLSQTTY > 0  AND A.CUSTBANK = cf.custid(+)
GROUP BY A.CUSTODYCD, A.ACCTNO,A.fullname, A.idcode, A.idplace, A.iddate, A.address, A.phone, A.fax,A.GROUPID, A.OPNDATE, SYMBOL,PARVALUE,TRADEPLACE,DEBIT, CREDIT,
    l_companyshortname,case WHEN PV_BANKNAME ='ALL' THEN 'ALL'
            when a.rrtype = 'C' then l_companyshortname else cf.fullname end
order by A.tradeplace, A.CUSTODYCD, A.ACCTNO, A.GROUPID, A.SYMBOL
