SET DEFINE OFF;
CREATE OR REPLACE PROCEDURE "CI2000" (
...
DELETE FROM CI2000_GL_DAUKY;


if V_F_DATE = to_date('20/01/2014','dd/mm/rrrr') then

INSERT INTO CI2000_GL_DAUKY(GLACCOUNT,FULLNAME, DAUKY)
    select bankacctno TRANSSUBTYPE, FULLNAME, nvl(glaccount,0) from banknostro where bankacctno not in ('12210000061342','12210000344025','12210000440613','12210000440631') ;

else

    INSERT INTO CI2000_GL_DAUKY(GLACCOUNT,FULLNAME, DAUKY)
    select TRANSSUBTYPE, FULLNAME, DAUKY + CI_CRAMT + OW_CRAMT - CI_DRAMT - OW_DRAMT CUOIKY
    from (

        SELECT
            gl.bankacctno TRANSSUBTYPE,  max(NVL(GL.GLACCOUNT,0)) DAUKY, MAX(GL.FULLNAME) FULLNAME,
            SUM(CASE WHEN A.TLTXCD IN ('1133','1134','1135','1136') AND a.TXTYPE = 'C' THEN a.NAMT ELSE 0 END) CI_CRAMT,
            SUM(CASE WHEN A.TLTXCD IN ('1133','1134','1135','1136') AND a.TXTYPE = 'D' THEN a.NAMT ELSE 0 END) CI_DRAMT,
            SUM(CASE WHEN A.TLTXCD NOT IN ('1133','1134','1135','1136') AND a.TXTYPE = 'C' THEN a.NAMT ELSE 0 END) OW_CRAMT,
            SUM(CASE WHEN A.TLTXCD NOT IN ('1133','1134','1135','1136') AND a.TXTYPE = 'D' THEN a.NAMT ELSE 0 END) OW_DRAMT
...
/*                         WHEN ci.corebank = 'N' and substr(ci.acctno,1,4) = '0001' and ci_to.corebank = 'Y' and substr(ci_to.acctno,1,4) = '0101' THEN '31010000462867'
                ...
                FROM  (SELECT * FROM vw_citran_gen WHERE tltxcd IN ('1120','1130') and txcd = '0011')   ci
                , (SELECT * FROM vw_citran_gen WHERE tltxcd IN ('1120','1130') and txcd = '0012') ci_to
                ...
                and ci.BUSDATE >= to_date('20/01/2014','dd/mm/rrrr') AND ci.BUSDATE < V_F_DATE
                ...
                SELECT ci.acctno, ci.tltxcd, CI.txdate, ci.txnum, ci.txcd, ci.namt, ci.field, ci.txtype,ci.txdesc,
                            ...
                            FROM
                                vw_citran_gen   ci
                            WHERE ci.BUSDATE >= to_date('20/01/2014','dd/mm/rrrr') AND ci.BUSDATE < V_F_DATE
             ...
             /*   SELECT ci.acctno, ci.tltxcd, CI.txdate, ci.txnum, ci.txcd, ci.namt, ci.field, ci.txtype, ci.txdesc,
                ...
                union all*/
                  SELECT ci.acctno, ci.tltxcd, CI.txdate, ci.txnum, ci.txcd, ci.namt, ci.field, 'C' txtype, ci.txdesc,
                               CASE
                                     WHEN substr(acctno,1,4) = '0001' THEN '12310000349850'
                                     WHEN substr(acctno,1,4) = '0101' THEN '31010000462867'
                                     else 'KHAC' END TRANSSUBTYPE
                            FROM
                                vw_citran_gen   ci
                            WHERE ci.BUSDATE >= to_date('20/01/2014','dd/mm/rrrr') AND ci.BUSDATE < V_F_DATE
...
/*            select tl.msgacct acctno, tl.tltxcd, tl.txdate, tl.txnum, '' txcd, tl.msgamt namt, 'BALANCE' FIELD , 'D' TXTYPE,tl.txdesc, CVALUE TRANSSUBTYPE from vw_tllog_all tl, vw_tllogfld_all tla
                ...
                UNION ALL
                    ...
                    select tl.msgacct acctno, tl.tltxcd, tl.txdate, tl.txnum, '' txcd,
                        ...
                        from vw_tllog_all tl, vw_tllogfld_all tla, vw_tllogfld_all tl1, vw_tllogfld_all tl2, vw_tllogfld_all tl3
                            where tl.tltxcd like '1135' and tl.txnum = tla.txnum and tl.txdate = tla.txdate and tla.fldcd = '05'
            ...
            select '' acctno, tl.tltxcd, tl.txdate, tl.txnum, '' txcd, tl.msgamt namt, 'BALANCE' FIELD , 'C' TXTYPE,tl.txdesc, TLA.CVALUE TRANSSUBTYPE
                    from vw_tllog_all tl, vw_tllogfld_all tla, vw_tllogfld_all tla1
            ...
            case when tl.msgamt <0 then 'D' else 'C' end TXTYPE, tl.txdesc, CVALUE TRANSSUBTYPE from vw_tllog_all tl, vw_tllogfld_all tla
                where tl.tltxcd like '1136' and tl.txnum = tla.txnum and tl.txdate = tla.txdate and tla.fldcd = '05'
                 AND TL.BUSDATE >= to_date('20/01/2014','dd/mm/rrrr') AND TL.BUSDATE < V_F_DATE
            union all

                select '' acctno, tl.tltxcd, tl.txdate, tl.txnum, '' txcd, tl.msgamt namt, 'BALANCE' FIELD , 'D' TXTYPE,tl.txdesc, CVALUE TRANSSUBTYPE from vw_tllog_all tl, vw_tllogfld_all tla
                 where tl.tltxcd like '1121' and tl.txnum = tla.txnum and tl.txdate = tla.txdate and tla.fldcd = '05'
                 AND TL.BUSDATE >= to_date('20/01/2014','dd/mm/rrrr') AND TL.BUSDATE < V_F_DATE

            union all
            select '' acctno, tl.tltxcd, tl.txdate, tl.txnum, '' txcd, tl.msgamt namt, 'BALANCE' FIELD , 'C' TXTYPE,tl.txdesc, TLA.CVALUE TRANSSUBTYPE
                    from vw_tllog_all tl, vw_tllogfld_all tla, vw_tllogfld_all tla1, vw_tllogfld_all tla2
                ...
                select tl.msgacct acctno, tl.tltxcd, tl.txdate, tl.txnum, '' txcd, tl.msgamt namt, 'BALANCE' FIELD , 'C' TXTYPE,   tl.txdesc,CVALUE TRANSSUBTYPE from vw_tllog_all tl, vw_tllogfld_all tla
...
/*            SELECT ci.acctno, ci.tltxcd, CI.txdate, ci.txnum, ci.txcd, ci.namt, ci.field,
             case when ci.tltxcd in ('1182','1189') then 'C' else ci.txtype end txtype, ci.txdesc,
                        ...
                        FROM
                            vw_citran_gen   ci,aftype aft, afmast af
...
*/
    ...
    SELECT ci.acctno, ci.tltxcd, CI.txdate, ci.txnum, ci.txcd, ci.namt, ci.field,
               ...
               /*SELECT ci.acctno, ci.tltxcd, CI.txdate, ci.txnum, ci.txcd, ci.namt, ci.field, ci.txtype, ci.txdesc,
                            ...
                            FROM
                                vw_citran_gen   ci, vw_tllogfld_all tl
               ...
               /* SELECT ci.acctno, ci.tltxcd, CI.txdate, ci.txnum, ci.txcd,
                                case when substr(CI.acctno,1,4) = '0001' then  ci.namt
                                     when substr(CI.acctno,1,4) = '0101' then  ci.namt + tl1.nvalue + tl2.nvalue
                                     else ci.namt
                                end namt
                                , ci.field, ci.txtype,ci.txdesc,
                                  tl.cvalue TRANSSUBTYPE
                                FROM
                                    vw_citran_gen   ci, vw_tllogfld_all tl, vw_tllogfld_all tl1, vw_tllogfld_all tl2
                                WHERE ci.txdate = tl.txdate and ci.txnum = tl.txnum and tl.fldcd='08'
...
/*                                case when substr(CI.acctno,1,4) = '0001' then  ci.namt
                                ...
                                , ci.field, ci.txtype, ci.txdesc,
                                  tl.cvalue TRANSSUBTYPE
                                FROM
                                    vw_citran_gen   ci, vw_tllogfld_all tl, vw_tllogfld_all tl1, vw_tllogfld_all tl2, vw_tllogfld_all tl3
                                WHERE ci.txdate = tl.txdate and ci.txnum = tl.txnum and tl.fldcd='08'
                                and ci.txdate = tl1.txdate and ci.txnum = tl1.txnum and tl1.fldcd='11'
                                and ci.txdate = tl2.txdate and ci.txnum = tl2.txnum and tl2.fldcd='13'
...
/*
                ...
                ELSE 'C' END txtype, ci.txdesc,
                    ...
                    SELECT ci.acctno, ci.tltxcd, CI.txdate, ci.txnum, ci.txcd, ci.namt, ci.field,   CASE WHEN TXCD = '0029' THEN 'D'
                     ...
                     SELECT ci.acctno, ci.tltxcd, CI.txdate, ci.txnum, ci.txcd, ci.namt, ci.field, 'C' txtype, ci.txdesc,
                                  CASE
                                         WHEN substr(CI.txnum,1,4) = '0001' and aft.corebank = 'Y' THEN '12312000002335'
                                         WHEN substr(CI.txnum,1,4) = '0101' and aft.corebank = 'Y' THEN '31010000462867'
                                         else 'KHAC' END TRANSSUBTYPE
                                FROM
                                    vw_citran_gen   ci  ,aftype aft, afmast af
                                WHERE ci.acctno = af.acctno and af.actype = aft.actype
               ...
               SELECT ci.acctno, ci.tltxcd, CI.txdate, ci.txnum, ci.txcd, ci.namt, ci.field, 'C' txtype, ci.txdesc,
                                   tl.cvalue TRANSSUBTYPE
                                FROM
                                    vw_citran_gen   ci, vw_tllogfld_all tl, aftype aft, afmast af
                                WHERE ci.txdate = tl.txdate and ci.txnum = tl.txnum and tl.fldcd='05'
        ...
        ) a, cfmast cf where gl.BANKACCTNO  = A.TRANSSUBTYPE(+) and a.custid = cf.custid (+)
        and gl.bankacctno not in ('12210000061342','12210000344025','12210000440613','12210000440631')
         and NVL(GL.GLACCOUNT,0)>0
         and case when a.TRANSSUBTYPE = '12310000392049' and V_STRBRID = '0001' then 0 else 1 end >0
        GROUP BY gl.BANKACCTNO
    ) ;
end if;
...
select gl.glaccount TRANSSUBTYPE, nvl(CI_CRAMT,0) CI_CRAMT, nvl(CI_DRAMT,0) CI_DRAMT, nvl(OW_CRAMT,0) OW_CRAMT, nvl(OW_DRAMT,0) OW_DRAMT , GL.DAUKY, gl.fullname
from (
SELECT
    a.TRANSSUBTYPE,
    SUM(CASE WHEN A.TLTXCD IN ('1133','1134','1135','1136') AND a.TXTYPE = 'C' THEN a.NAMT ELSE 0 END) CI_CRAMT,
...
) a, (SELECT * FROM CFMAST WHERE FNC_VALIDATE_SCOPE(BRID, CAREBY, TLSCOPE, pv_BRID, TLGOUPS)=0) cf where a.custid = cf.custid (+)

GROUP BY a.TRANSSUBTYPE
) a,  CI2000_GL_DAUKY GL

 where gl.GLACCOUNT = A.TRANSSUBTYPE (+)
   ORDER BY a.TRANSSUBTYPE
;
