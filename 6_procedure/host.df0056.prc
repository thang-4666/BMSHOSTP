SET DEFINE OFF;
CREATE OR REPLACE PROCEDURE "DF0056" (
...
SELECT case when a.rrtype = 'C' then cspks_system.fn_get_sysvar('SYSTEM','COMPANYSHORTNAME') else cf.fullname end RRNAME, a.*,BBRID v_strbrid  FROM (

 SELECT LNM.RRTYPE,LNM.CUSTBANK,CF.CUSTODYCD, AF.ACCTNO AFACCTNO,df.acctno, LNM.ACCTNO LNACCTNO, SB.SYMBOL
    , parvalue ,SE.NAMT TRADE,CF.CUSTTYPE,
