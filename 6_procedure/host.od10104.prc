...
BEGIN
   ...
   select sum(lnprin+lnprovd) into l_MRAMT_BG
    ...
    from vw_odmast_all od, afmast af, cfmast cf
    where od.deltd <> 'Y' and od.isdisposal = 'Y' and od.execamt <> 0
    ...
    if V_currdate = V_TODATE then
        ...
        --MR0063
        select sum(nvl(ADD_TO_MRCRATE,0)) into l_ADD_TO_MRCRATE
        FROM TBL_MR0063 t, cfmast cf
        where t.custid = cf.custid and cf.custatcom = 'Y'
            and t.indate = v_next_indate;
        -- end MR0063
        --MR0058
        SELECT sum(nvl(NML,0)+nvl(LAI_DUKIEN,0)) into l_NML
        FROM TBL_MR0058 t, cfmast cf
        where t.custid = cf.custid and cf.custatcom = 'Y'
            and t.indate = v_next_indate;
        -- end MR0058
        l_ADDAMT_BG := NVL(l_ADD_TO_MRCRATE,0)+NVL(l_NML,0);
...
if V_currdate = V_TODATE then
    ...
    (
        ...
        from tbl_mr3007_log lg, sbsecurities sb, cfmast cf
        where lg.txdate = V_TODATE
            and lg.codeid = sb.codeid and sb.sectype <> '004'
            and lg.custodycd = cf.custodycd and cf.custatcom = 'Y'
