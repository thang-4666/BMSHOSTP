SET DEFINE OFF;
CREATE OR REPLACE PROCEDURE "CI0037" (
...
SELECT SUBSTR(TO_DATE(V_CFROMDATE,'DD/MM/YYYY'),4,2) ,SUBSTR(TO_DATE(getcurrdate,'DD/MM/YYYY'),4,2) INTO V_FDATE,V_TODATE  FROM DUAL;

   -- GET REPORT'S DATA
IF TO_NUMBER(V_FDATE)<TO_NUMBER(V_TODATE) THEN
    OPEN  PV_REFCURSOR FOR

         SELECT V_CFROMDATE THANG,V_CTODATE THANG1, CF.BRID,AD.TXNUM,NVL(TL.TXNUM,'') TXNUMPAID,ad.txkey,cf.custodycd,af.acctno,cf.fullname,
                  ad.txdate,NVL(adt.amt,0) PAIDAMT,NVL(ad.paiddate,ad.cleardt) PAIDDATE,AD.ADTYPE,
...
ELSE
  ...
  SELECT V_CFROMDATE THANG,V_CTODATE THANG1, CF.BRID,AD.TXNUM,NVL(TL.TXNUM,'') TXNUMPAID,ad.txkey,cf.custodycd,af.acctno,cf.fullname,
        ...
        FROM
            (
            SELECT AUTOID,ISMORTAGE,STATUS,DELTD,ACCTNO,TXDATE,TXNUM,REFADNO,CLEARDT,AMT,
                   FEEAMT,VATAMT,BANKFEE,PAIDAMT,RRTYPE,CUSTBANK,CIACCTNO,ODDATE,NVL(PAIDDATE,'') PAIDDATE, NVL(adtype,'----') ADTYPE,
                   to_char(txdate,'DDMMRRRR') || substr(txnum,5,6) txkey
            FROM adschd where TXDATE >= V_CFROMDATE and TXDATE <= V_CTODATE
            UNION ALL
            SELECT AUTOID,ISMORTAGE,STATUS,DELTD,ACCTNO,TXDATE,TXNUM,REFADNO,CLEARDT,AMT,
