SET DEFINE OFF;
CREATE OR REPLACE PROCEDURE "MATCHING_ORDER_UPCOM"
is
   ...
   Cursor c_SqlMaster Is
        ...
        FROM ODMAST OD,SBSECURITIES SEC,STCTRADEBOOK A,STCORDERBOOK B WHERE OD.ORDERID=B.ORDERID AND  A.ORDERNUMBER=B.ORDERNUMBER AND OD.CODEID=SEC.CODEID AND SEC.TRADEPLACE='005'
        MINUS
        SELECT OD.ORDERID,OD.CODEID,SEC.SYMBOL,B.CUSTODYCD,OD.AFACCTNO,OD.SEACCTNO,OD.CIACCTNO,B.BSCA BORS,B.NORP,OD.NORK AORN,
        OD.QUOTEPRICE EXPRICE, OD.ORDERQTTY EXQTTY, A.VOLUME QTTY,A.PRICE PRICE,A.ORDERNUMBER REFORDERID, A.REFCONFIRMNUMBER CONFIRM_NO,
        OD.BRATIO,OD.CLEARDAY,OD.CLEARCD , B.CUSTODYCD || '.' || B.BSCA || '.' || SEC.SYMBOL || '.' || A.VOLUME || '.' || A.PRICE  DESCRIPTION
        ...
        AND A.REFCONFIRMNUMBER=C.REFCONFIRMNUMBER AND OD.CODEID=SEC.CODEID AND C.DELTD<>'Y' AND SEC.TRADEPLACE='005'
...
BEGIN
return;
If to_char(sysdate,'hh24mi')> '1600' or to_char(sysdate,'hh24mi') < '1000' then
    Return;
  End if;
   --0 lay cac tham so
   v_brid := '0000';
  ...
  --7.ODMAST
                 ...
                 ELSE
                    INSERT INTO stschd
                         ...
                         VALUES (seq_stschd.NEXTVAL, i.ORDERID, i.CODEID,
                                 ...
                                 i.CLEARCD, i.PRICE * i.QTTY, 0,
                                 i.QTTY, 0, 0, 'N', 'N', 0,
                         getduedate(TO_DATE (v_txdate, 'DD/MM/YYYY'),i.CLEARCD,/*'000'*/'005',least(v_mtrfday,l_trfbuyext)) --ngoc.vu-Jira561
                                );
                 END IF;
   ...
   EXCEPTION
       ...
       INSERT INTO log_err
           ...
           VALUES ( seq_log_err.NEXTVAL,SYSDATE, ' MATCHING_ORDER_UPCOM ', v_err
                  );
