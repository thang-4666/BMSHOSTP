...
-- BANG KE UONG TRUOC TIEN BAN NGUON NGAN HANG
-- MODIFICATION HISTORY
-- PERSON      DATE    COMMENTS
-- NGOCVTT   21-MAY-15  CREATED
-- ---------   ------  -------------------------------------------
  ...
  --------------------------
   ...
   -- GET REPORT'S DATA
    ...
    OPEN  PV_REFCURSOR FOR
         ...
         SELECT LN.*,NVL(RE.REFULLNAME,'') MG_CHINH,NVL(REFT.REFULLNAME,'')MG_PHU
   FROM (SELECT FULLNAME,BRID,CUSTODYCD,IDCODE,TXDATE,CLEARDT,NGAYUT,CUSTBANK,CUSTID,TXDATE1,
       MAX(BANKFEERATE) BANKFEERATE,SUM(NAMT) NAMT, SUM(FEEAMT) FEEAMT, SUM(AMT) AMT, SUM(bankfee) bankfee, SUM(vatamt) vatamt
FROM(SELECT  CF.BRID ,CF.IDCODE,TO_CHAR(AD.TXDATE,'DD/MM/RRRR') TXDATE, CF.CUSTODYCD ,CF.CUSTID,AD.TXDATE TXDATE1,
        ...
        FROM (SELECT * FROM
             ...
             CFMAST CF2, VW_ADVSRESLOG_ALL ADT,ADTYPE ADTY
        WHERE AD.DELTD <> 'Y' AND ADT.DELTD <> 'Y'
            ...
            OR (CASE WHEN SUBSTR(AD.TXNUM,1,2) = '68' THEN 'O'
                WHEN SUBSTR(AD.TXNUM,1,2) = '99' THEN 'A' ELSE 'F' END) LIKE V_STRVIA)
                  ...
                  GROUP BY FULLNAME,BRID,CUSTODYCD,IDCODE,TXDATE,CLEARDT,NGAYUT,CUSTBANK,CUSTID,TXDATE1
          )LN
                  LEFT JOIN
    ...
    ORDER BY LN.TXDATE1, LN.NGAYUT, LN.CUSTODYCD,LN.CUSTBANK;
