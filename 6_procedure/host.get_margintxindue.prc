SET DEFINE OFF;
CREATE OR REPLACE PROCEDURE "GET_MARGINTXINDUE" (PV_REFCURSOR IN OUT PKG_REPORT.REF_CURSOR, TLID IN VARCHAR2, Tx in number)
  IS
...
mst.SEASS, mst.BALANCE, mst.MARGINRATE, mst.GRPNAME, mst.CAREBY, mst.MONEYPAY, mst.DAYPAY,depofeeamt,
mst.RTNAMOUNTREF,
mst.OVDAMOUNTREF,
...
(
  ...
  v_getsecmarginratio sec, (select * from tlgroups where grptype = 2) grp
  ...
  having sum(case when od.txdate= (select sbdate from sbcurrdate where numday =-Tx and sbtype ='B')  then
        od.matchamt + od.feeacr
...
where  mst.afacctno = od.afacctno(+)
 ...
 mr, tlgrpusers tl, tlgroups gr
        where mr.careby = tl.grpid and tl.grpid= gr.grpid and gr.grptype='2' and tl.tlid=v_TLID;
