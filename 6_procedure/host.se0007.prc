...
OPEN PV_REFCURSOR
     FOR
SELECT I_DATE I_DATE, sb.TRADEPLACE, SB.symbol,SB.parvalue , CF.FULLNAME  FULLNAME,CF.IDCODE ,AL.CDCONTENT COUNTRY,cf.address address
,ISS.fullname SYMBOLNAME , substr(NUM.ACCTNO,1,10) ACCTNO,NUM.AMT, cf.email, cf.mobilesms fax1, cf.custodycd, af.acctno
FROM AFMAST AF,(SELECT * FROM CFMAST WHERE FNC_VALIDATE_SCOPE(BRID, CAREBY, TLSCOPE, pv_BRID, TLGOUPS)=0) CF,  SBSECURITIES  SB,ALLCODE AL, ISSUERS ISS,
(
