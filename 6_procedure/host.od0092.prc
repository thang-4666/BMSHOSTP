SET DEFINE OFF;
CREATE OR REPLACE PROCEDURE od0092 (
 ...
 SELECT V_STRNOIIN NOIIN, cf.custodycd,cf.fullname,CF.CUSTATCOM, cf.address,od.exectype, od.txdate,sts.cleardate,  od.afacctno,
         ...
         (   case when OD.execamt>0 and OD.feeacr=0  AND OD.TXDATE = V_CUR_DATE THEN  ODT.deffeerate
               ...
               (CASE WHEN (OD.execamt * OD.feeacr) = 0 THEN 0 ELSE
                   (CASE WHEN OD.TXDATE = V_CUR_DATE
                      ...
                      END)
               END)
             end ) FEE_RATE, /*v_taxrate+TO_NUMBER(MR1.varvalue)*/
             (DECODE (CF.VAT,'Y', TO_NUMBER(MR.varvalue),'N',0 ) + DECODE (CF.WHTAX,'Y', TO_NUMBER(MR1.varvalue),'N',0 )  ) TAX_RATE,
       ...
       (CASE WHEN OD.execamt = 0 THEN 0 ELSE
        ...
        END)   feeamt,
           ...
           ELSE (ROUND((IO.MATCHQTTY * IO.MATCHPRICE * (DECODE (CF.VAT,'Y', TO_NUMBER(MR.varvalue),'N',0 ) + DECODE (CF.WHTAX,'Y', TO_NUMBER(MR1.varvalue),'N',0 )  ))/100, 0) + NVL(sts.ARIGHT, 0)) END)
              ELSE 0 END) taxsellamt

    from vw_odmast_all od , vw_iod_all io,vw_stschd_all sts ,(SELECT * FROM CFMAST WHERE FNC_VALIDATE_SCOPE(BRID, CAREBY, TLSCOPE, pv_BRID, TLGOUPS)=0) cf, odtype odt, afmast af
    ,sysvar MR,sysvar mr1
    where od.deltd <> 'Y'
        and od.execamt <> 0
