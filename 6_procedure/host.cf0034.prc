...
OPEN PV_REFCURSOR
   ...
   decode(CTYPE,'C','TN','P','TD','F','NN') CTYPE ,SUM(MATCHAMT) MATCHAMT, SUM(TAXSELLAMT) TAXSELLAMT
    FROM
   (
   ...
   DECODE (V_TAXTYPE,'002',0, SUM(OD.MATCHAMT)) MATCHAMT,
   DECODE (V_TAXTYPE,'002',0, SUM(OD.TAXSELLAMT)) TAXSELLAMT
   FROM
   (SELECT AFACCTNO, SUM(MATCHAMT) MATCHAMT, SUM(TAXSELLAMT) TAXSELLAMT FROM
     (
       SELECT OD.AFACCTNO,OD.MATCHAMT,(OD.MATCHAMT * NVL((SELECT TO_NUMBER(VARVALUE)
       ...
       UNION ALL
          SELECT OD.AFACCTNO,OD.MATCHAMT, OD.TAXSELLAMT + ST.ARIGHT TAXSELLAMT FROM ODMASTHIST OD, STSCHDHIST ST
   ...
   SELECT  CF.FULLNAME, CF.IDCODE, CF.IDTYPE, CF.IDDATE, CF.IDPLACE, CF.CUSTODYCD, CF.TAXCODE,
   MAX(SUBSTR(CF.CUSTODYCD,4,1)) CTYPE,   --MAX(CF.ISBANKING) ISBANKING,
   DECODE (V_TAXTYPE,'001',0, SUM(TL.MATCHAMT)) MATCHAMT,
   DECODE (V_TAXTYPE,'001',0, SUM(TL.TAXSELLAMT)) TAXSELLAMT
   FROM
   (SELECT TL.MSGACCT, SUM(CI2.NAMT) MATCHAMT, SUM(CI.NAMT) TAXSELLAMT
     FROM
        (
        ...
        SELECT  TXNUM ,TXDATE , NAMT FROM CITRANA WHERE  TXCD ='0011'  AND DELTD <>'Y'
         AND BKDATE >= V_F_DATE AND BKDATE <= V_T_DATE
           ...
           (
        SELECT  TXNUM ,TXDATE , NAMT FROM CITRAN  WHERE TXCD ='0012' AND DELTD <>'Y'
         AND BKDATE >= V_F_DATE AND BKDATE <= V_T_DATE
        UNION ALL
        SELECT  TXNUM ,TXDATE , NAMT FROM CITRANA WHERE  TXCD ='0012'  AND DELTD <>'Y'
         AND BKDATE >= V_F_DATE AND BKDATE <= V_T_DATE
        ) CI2
     WHERE  TL.TXNUM = CI.TXNUM
          ...
          GROUP BY TL.MSGACCT
     ) TL, (SELECT * FROM CFMAST WHERE FNC_VALIDATE_SCOPE(BRID, CAREBY, TLSCOPE, pv_BRID, TLGOUPS)=0) CF, AFMAST AF
   ...
   CTYPE HAVING SUM(MATCHAMT)  + SUM(TAXSELLAMT) >0
