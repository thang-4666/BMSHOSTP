SET DEFINE OFF;
CREATE OR REPLACE PROCEDURE getsingleaccountmarginrateex (
        ...
        OPEN PV_REFCURSOR FOR
                ...
                (SELECT
                      /* nvl(af.MRCRLIMIT,0) +*/ nvl(se.SEASS,0)  +  v_quantity* v_mrratiorate/100*least(v_marginprice,v_mrpricerate) NAVACCOUNT,
                       balance+least(nvl(af.MRCRLIMIT,0),nvl(secureamt,0))+ nvl(adv.avladvance,0)- odamt  - dfdebtamt - dfintdebtamt - NVL (advamt, 0)-nvl(secureamt,0) - ramt-f_quantity*f_price*f_ratio/100 OUTSTANDING,
                       nvl(af.mrcrlimitmax,0) + balance+nvl(af.MRCRLIMIT,0)- odamt - nvl(secureamt,0) - ramt avllimit
