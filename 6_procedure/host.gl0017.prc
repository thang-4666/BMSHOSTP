SET DEFINE OFF;
CREATE OR REPLACE PROCEDURE "GL0017" (
...
-- GET REPORT'S DATA
   ...
   OPEN PV_REFCURSOR
       ...
       SELECT SUM(CASE WHEN GL.DORC = 'C' THEN GL.AMT ELSE 0 END) CRAMT,
            SUM(CASE WHEN GL.DORC = 'D' THEN GL.AMT ELSE 0 END) DRAMT,   nvl(MI.MICD,'MIS') MICD ,MAX(AL.CDCONTENT)
              FROM (select  TXDATE ,TXNUM,SUBTXNO ,DORC,ACCTNO ,max(CUSTID) CUSTID ,max(TASKCD) TASKCD,max(DEPTCD) DEPTCD,max(MICD) MICD
               ...
               MI, (SELECT * FROM GLTRAN where deltd<>'Y' UNION SELECT * FROM GLTRANA where deltd<>'Y' ) GL,ALLCODE  AL
             WHERE  GL.TXDATE = MI.TXDATE (+)
              ...
              GROUP BY  nvl(MI.MICD,'MIS')
              order by nvl(MI.MICD,'MIS') ;
