...
select a.*, V_FROMDATE ky_truoc, V_TODATE ky_nay,V_CURDATE CURRDATE from (
        select ISS.FULLNAME,
        ( case when sb.sectype in ('003','006','222') then
                    (case when   SB.BONDTYPE = '005' THEN '03' --'Trai phieu cong ty'
                          else '02'--'Trai phieu'
                          end)
            when sb.sectype in  ('007','008') then '04'--'Chung chi quy'
            when sb.sectype in ('001','002') then '01'--'Co phieu niem yet'
            else '07'--'Loai khac'
            end) orderid ,

        (case when sb.sectype in ('003','006') then
                    (case when  SB.BONDTYPE = '005' then 'TAPCT'
                          else 'TAP' end)
            when sb.sectype in  ('007','008') then 'TCCQ'
            when sb.sectype in ('001','002') then 'CP'
            else '' end) danh_muc, sb.symbol, sb.tradeplace,
...
group by sb.symbol, sb.sectype, SB.BONDTYPE, sb.tradeplace,ISS.FULLNAME


---------------------------------tinh tien mat
union all
select 'TM' FULLNAME, '05' orderid, 'TM' danh_muc, 'Tien' symbol, '000' tradeplace,  ROUND(SUM(CI_CURR.BALANCE - NVL(CI_TR.TR_BALANCE, 0) - NVL(SECU.OD_BUY_SECU, 0))) SL,
