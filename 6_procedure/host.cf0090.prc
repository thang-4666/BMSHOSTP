...
-- GET REPORT'S DATA
 OPEN PV_REFCURSOR
 FOR
       SELECT CF.CUSTODYCD, CF.PHONE CUSTPHONE, CF.ADDRESS, '' TLFULLNAME,'' TXNUM, '' TXDATE, VARVALUE CURRDATE, AF.ACCTNO AFACCTNO_R, NVL(CI.CRINTACR,0) CRINTACR_R, NVL(CI.ODINTACR,0) ODINTACR_R , NVL(CI.ODAMT,0) ODAMT_R,
       0 ADVBALANCE_R,
       ...
       NVL(SE.DEPOSIT,0) DEPOSIT_QTTY_R,  NVL(SE.WITHDRAW,0) WITHDRAW_QTTY_R, NVL(seretl.qtty,0) SERETAIL_QTTY_R, nvl(CI.NS_CHK_QTTY,0) NS_CHK_QTTY_R,
        ...
        CASE WHEN PV_CLOSETYPE = 'ALL' THEN  FN_GETTRFEE(0,'01',AF.ACCTNO, '00007') ELSE 0 END TRFEE_R,
...
--- NEU CHK_QTTY > 0 THI KHONG CHO PHEP DONG
       FROM ALLCODE A0, (SELECT * FROM CFMAST WHERE FNC_VALIDATE_SCOPE(BRID, CAREBY, TLSCOPE, pv_BRID, TLGOUPS)=0) CF, AFTYPE TYP, AFMAST AF, MRTYPE, SYSVAR,
           (
           ...
           (
             ...
             FROM (SELECT  BLOCKED,  afacctno from semast )
              ...
              ( -- CK  cam co VSD
             SELECT SUM(STANDING) qtty, afacctno
             FROM       (
