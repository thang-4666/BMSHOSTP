SET DEFINE OFF;
CREATE OR REPLACE PROCEDURE pr_release_hnx_order
IS
  pkgctx     plog.log_ctx;
...
BEGIN
 ...
 --Init log
  ...
  plog.setbeginsection(pkgctx, 'ReleaseHNXOrders');
  ...
  plog.error(pkgctx, 'Start releasing');
   ...
   FOR I IN(SELECT      CCY.SYMBOL, MST.*
            ...
            WHERE       MST.AFACCTNO = AF.ACCTNO
                        ...
                        AND CCY.TRADEPLACE = '002'
                        and txdate = to_date(v_txdate, 'dd/mm/yyyy')
                        AND MST.EXECTYPE IN ('NB','NS','MS') AND MST.MATCHTYPE <> 'P')
   LOOP
...
--      dbms_output.put_line(I.ORDERID || ' ' || V_REMAINQTTY_CUR||' ' ||V_CANCELQTTY || ' '|| V_EXECQTTY_CUR || ' ' || V_ORDERQTTY_CUR);
  ...
  plog.setendsection(pkgctx, 'Finish releasing HNX Orders');
   --COMMIT;
EXCEPTION
   WHEN others
   THEN
  plog.setendsection(pkgctx, 'Finish releasing with exception');   
   ...
   v_err:=substr(sqlerrm,1,200);
      INSERT INTO log_err
           ...
           VALUES ( seq_log_err.NEXTVAL,SYSDATE, ' pr_release_hnx_order ', v_err
                  );
