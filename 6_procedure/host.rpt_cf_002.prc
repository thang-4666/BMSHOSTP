SET DEFINE OFF;
CREATE OR REPLACE PROCEDURE "RPT_CF_002" (pv_strXML IN OUT NVARCHAR2)
   IS
--
...
-- ---------   ------  -------------------------------------------
    ...
    v_strVal1           NVARCHAR2(1000);
    v_strVal2           NVARCHAR2(1000);
    v_strVal3           NVARCHAR2(1000);
    v_strVal4           NVARCHAR2(1000);

    v_strFilter         VARCHAR2(4000);
    v_strSQL            NVARCHAR2(4000);
...
BEGIN
    ...
    For i in 1..DBMS_XMLDOM.getLength(v_xmlNodelist) loop
        ...
        End if;
    End loop;
    dbms_output.put_line('Filter: '||v_strFilter);

    v_strSQL := 'SELECT CDTYPE, CDNAME, CDVAL, CDCONTENT FROM ALLCODE WHERE 0=0' || NVL(v_strFilter, ' AND 1=1');
    dbms_output.put_line('SQL: '||v_strSQL);

    -- Open ref cursor to get data from database
    Open v_refCursor for
        'SELECT CDTYPE, CDNAME, CDVAL, CDCONTENT FROM ALLCODE WHERE 0=0' || NVL(v_strFilter, ' AND 1=1');

    Loop
        Fetch v_refCursor into v_strVal1, v_strVal2, v_strVal3, v_strVal4;
        Exit when v_refCursor%NOTFOUND;

        dbms_output.put_line('Du lieu: '||v_strVal1||' '||v_strVal2||' '||v_strVal3||' '||v_strVal4);
    End loop;
