SET DEFINE OFF;
CREATE OR REPLACE PROCEDURE "SP_BD_GETORDERBOOK" (PV_REFCURSOR IN OUT PKG_REPORT.REF_CURSOR, AFACCTNO IN VARCHAR2)
  IS
...
BEGIN
    ...
    FROM CFMAST, AFMAST CF, OOD, (SELECT OD1.*,OD2.EDSTATUS EDITSTATUS from odmast OD1,(SELECT * FROM ODMAST WHERE EDSTATUS IN ('C','A')) OD2   WHERE OD1.ORDERID=OD2.REFORDERID(+) AND substr(OD1.EXECTYPE,1,1) <> 'C' AND substr(OD1.EXECTYPE,1,1) <> 'A') MST,TLLOG TL, sbsecurities sb,
        ...
        ordermap MAP
    WHERE MST.ORSTATUS <> '7' AND CF.ACCTNO=MST.AFACCTNO AND MST.AFACCTNO=V_PARAFILTER AND MST.orderid=OOD.ORGORDERID(+)
    ...
    WHERE  REFOD.ORSTATUS <> '7' AND MST.DELTD='N' AND CF.ACCTNO=MST.AFACCTNO AND MST.AFACCTNO=V_PARAFILTER AND MST.ACCTNO=OOD.ORGORDERID(+)
    ...
    AND MST.DELTD='N' AND CF.ACCTNO=MST.AFACCTNO AND SYINFO.SYMBOL=MST.SYMBOL AND MST.AFACCTNO=V_PARAFILTER AND MST.ACCTNO=OOD.ORGORDERID(+)
        AND CD1.cdtype ='FO' AND CD1.CDNAME='STATUS' AND CD1.CDVAL=MST.status
        AND SYS.SYSNAME='CONTROLCODE'  AND CFMAST.CUSTID=CF.CUSTID
        AND CD2.cdtype ='FO' AND CD2.CDNAME='EXECTYPE' AND CD2.CDVAL=MST.EXECTYPE
        AND CD3.cdtype ='FO' AND CD3.CDNAME='BOOK' AND CD3.CDVAL=MST.BOOK
