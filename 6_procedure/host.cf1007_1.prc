SET DEFINE OFF;
CREATE OR REPLACE PROCEDURE cf1007_1 (
...
-- return;
    ...
    OPEN PV_REFCURSOR FOR
        select PV_AFACCTNO AFACC,0 tmtracham,
        ...
        (
            ...
            select cf.custid, cf.custodycd,
             ...
             /*    sum(balance + emkamt) ci_balance,*/ sum(balance + emkamt+netting) ci_balance, --ngoc.vu
        ...
        (
            ...
            select max(tse.autoid) orderid, tse.custid, tse.afacctno, max(tse.tllog_autoid) autoid,tse.txtype,tse.txcd,
                tse.busdate, case when tse.TLTXCD = '8867' then 'Bán' || trim(to_char(sum(tse.namt),'999,999,999,999,999,999')) || ' ' || max(tse.symbol) || ' ngày' || TO_CHAR(tse.busdate,'DD/MM/RRRR')
                             else  'Mua ' || trim(to_char(sum(tse.namt),'999,999,999,999,999,999')) || ' ' || max(tse.symbol) || ' ngày' || TO_CHAR(max(od.TXDATE),'DD/MM/RRRR') end TXDESC,
                 to_char(max(tse.symbol)) symbol,
            ...
            union all
                    select se.tllog_autoid orderid, se.custid, se.afacctno, se.tllog_autoid+1 autoid, 'C' txtype, '0028' txcd,
                se.busdate,
               -- nvl(tci.trdesc,tci.txdesc) txdesc,
                se.txdesc,
                se.symbol symbol, se.namt se_credit_amt, 0 se_debit_amt,
                0 ci_credit_amt,
                0 ci_debit_amt,
                se.tltxcd, '' trdesc
            from vw_setran_gen se
            where se.busdate >= v_FromDate AND se.busdate <=  v_todate
            ...
            select tci.autoid orderid, tci.custid, tci.acctno afacctno, tci.tllog_autoid autoid,tci.txtype,tci.txcd,
                tci.busdate,
               nvl(tci.trdesc,tci.txdesc) txdesc,
            ...
            where tci.busdate >= v_FromDate AND tci.busdate <= v_ToDate
              ...
              select  max(tci.autoid) orderid, tci.custid, tci.acctno afacctno,max(tci.tllog_autoid) autoid,tci.txtype,tci.txcd,
                 tci.busdate,
           ...
           select  max(tci.autoid) orderid, tci.custid, tci.acctno afacctno,max(tci.tllog_autoid) autoid,tci.txtype,tci.txcd,
                      tci.busdate,
                      (case when tci.tltxcd='8889' then replace(tci.txdesc,UTF8NUMS.C_CONST_TLTX_TXDESC_8865,'Phong tỏa tiền mua')
                            when tci.tltxcd='8868' then 'Thanh toán tiền mua CK ngày ' ||TO_CHAR(od.TXDATE, 'DD/MM/RRRR')
                            else tci.txdesc end ) TXDESC,
                      '' symbol, 0 se_credit_amt, 0 se_debit_amt,
                      SUM(case when tci.txtype = 'C' then namt else 0 end) ci_credit_amt ,
                      SUM(case when tci.txtype = 'D' then namt else 0 end) ci_debit_amt,
                      tci.tltxcd, '' trdesc
            from vw_citran_gen tci,VW_ODMAST_ALL od
                where tci.busdate >= v_FromDate AND tci.busdate <= v_ToDate
                and tci.custodycd = v_CustodyCD
                and TCI.corebank <> 'Y' --khong lay ps tien cua tk corebank.
                and tci.acctno like v_AFAcctno
                and tci.field ='NETTING'
                and tci.ref=od.orderid
                GROUP BY tci.custid, tci.acctno, tci.busdate,tci.tltxcd,tci.txtype,tci.txcd,TCI.TXDESC,od.TXDATE
        ...
        (
            ...
            where tr.busdate >= v_FromDate and tr.busdate <= v_CurrDate
               ...
               -- and tr.field in ('HOLDBALANCE','BALANCE')
                and tr.field in ('HOLDBALANCE','BALANCE','NETTING')  --NGOC.VU
       ...
       --HAVING sum(nvl(se_credit_amt,0)) - sum(nvl(se_debit_amt,0)) + (nvl(ci_credit_amt,0)) + (nvl(ci_debit_amt,0))  <> 0
        order by --tr.busdate, tr.autoid,(nvl(se_debit_amt,0)) ,(nvl(se_credit_amt,0))  --tr.txtype
                tr.busdate,tr.autoid, tr.txtype, tr.orderid,TR.afacctno, -- CHAUNH add
                 case when tr.tltxcd = '1143' and tr.txcd = '0077' then utf8nums.c_const_RPT_CF1000_1143
                 when tr.tltxcd in ('1143','1153') and tr.txcd = '0011' and tr.trdesc is null then utf8nums.c_const_RPT_CF1000_1153
                 else to_char(tr.txdesc)
                end
