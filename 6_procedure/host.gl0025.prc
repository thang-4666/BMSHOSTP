SET DEFINE OFF;
CREATE OR REPLACE PROCEDURE "GL0025" (
...
-- GET REPORT'S DATA
 ...
 SELECT SUM(CASE WHEN GL.DORC = 'C' THEN GL.AMT ELSE 0 END) CRAMT,
        SUM(CASE WHEN GL.DORC = 'D' THEN GL.AMT ELSE 0 END) DRAMT,   nvl(MI.MICD,'MIS') MICD ,MAX(AL.CDCONTENT),   nvl(MI.DEPTCD,'MIS') DEPTCD ,MAX(AL1.CDCONTENT)
 FROM (select  TXDATE ,TXNUM,SUBTXNO ,DORC,ACCTNO ,max(CUSTID) CUSTID ,max(TASKCD) TASKCD,max(DEPTCD) DEPTCD,max(MICD) MICD
               ...
               MI, (SELECT * FROM GLTRAN WHERE DELTD<>'Y' UNION SELECT * FROM GLTRANA WHERE DELTD<>'Y' ) GL,ALLCODE  AL, ALLCODE AL1
             WHERE  GL.TXDATE = MI.TXDATE (+)
               AND  AL.CDNAME ='IECD'
               AND AL.CDVAL = nvl(MI.MICD,'MIS')
               AND AL.CDTYPE ='SA'
               AND AL1.CDNAME ='DEPTCD'
               AND AL1.CDVAL = nvl(MI.DEPTCD,'MIS')
               AND AL1.CDTYPE ='SA'
              ...
              GROUP BY  nvl(MI.MICD,'MIS'), nvl(MI.DEPTCD,'MIS')
              order by nvl(MI.MICD,'MIS'),nvl(MI.DEPTCD,'MIS') ;
