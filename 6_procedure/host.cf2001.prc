...
SELECT TO_DATE(I_DATE,'DD/MM/RRRR') INDATE,v_inBRID inBRID,V_BRNAME BRNAME, 'NEW' TYPE,A.TXDATE||A.TXNUM TXTYPE, to_char(A.TXDATE,'DD/MM/RRRR') TXDATE, A.TXNUM, A.CUSTID,
       ...
       (CASE WHEN A.NIDDATE<> A.OIDDATE THEN to_char(A.NIDDATE,'DD/MM/RRRR') ELSE '' END) IDDATE,
      (CASE WHEN A.OCUSTTYPE<>A.NCUSTTYPE THEN (CASE WHEN A.NCUSTTYPE = 'I' AND CF.COUNTRY = '234' THEN '3'
          ...
          (CASE WHEN A.NCOUNTRY IS NULL THEN '' WHEN A.OCOUNTRY=A.NCOUNTRY THEN '' ELSE A.NCOUNTRY END) COUNTRY, '' CUSTODYCD, '' TXDESC,'' VSDIDTYPE
FROM CFVSDLOG A, (SELECT * FROM CFMAST WHERE FNC_VALIDATE_SCOPE(BRID, CAREBY, TLSCOPE, pv_BRID, TLGOUPS)=0) CF, vw_tllog_all TL,ALLCODE A1,
...
WHERE A.TXDATE = B.TXDATE AND A.TXNUM = B.TXNUM
       AND A.CUSTID = CF.CUSTID  AND A.TXDATE = TL.TXDATE
...
SELECT TO_DATE(I_DATE,'DD/MM/RRRR') INDATE,v_inBRID inBRID,V_BRNAME BRNAME, 'OLD' TYPE,A.TXDATE||A.TXNUM TXTYPE, to_char(A.TXDATE,'DD/MM/RRRR') TXDATE, A.TXNUM, A.CUSTID,
        OFULLNAME FULLNAME ,OADDRESS ADDRESS, OIDCODE IDCODE, to_char(OIDDATE,'DD/MM/RRRR') IDDATE,
        ...
        /*A1.CDCONTENT*/A.OCOUNTRY COUNTRY, CF.CUSTODYCD,  (CASE WHEN SUBSTR(A.TXNUM,1,2)='68' THEN 'Thay d?i d?a ch? m?i' ELSE TL.txdesc END )txdesc,
        (CASE WHEN CF.IDTYPE = '001' THEN '1'
            WHEN CF.IDTYPE = '009' THEN '2'
            WHEN CF.IDTYPE = '005' THEN '3'
            WHEN CF.IDTYPE = '002' THEN '5' ELSE '4' END) VSDIDTYPE
FROM CFVSDLOG A, (SELECT * FROM CFMAST WHERE FNC_VALIDATE_SCOPE(BRID, CAREBY, TLSCOPE, pv_BRID, TLGOUPS)=0) CF, vw_tllog_all TL,ALLCODE A1,
    (SELECT MAX(TXDATE) TXDATE, MAX(TXNUM) TXNUM FROM CFVSDLOG WHERE TXDATE = TO_DATE(I_DATE,'DD/MM/RRRR') group by txdate, txnum ORDER BY TXDATE, TXNUM ) B
WHERE A.TXDATE = B.TXDATE AND A.TXNUM = B.TXNUM
      AND A.CUSTID = CF.CUSTID AND A.TXDATE = TL.TXDATE
