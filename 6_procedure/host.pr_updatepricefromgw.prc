...
-- p_update_mode CN : Cuoi ngay
    plog.error(pkgctx,'pr_updatepricefromgw.p_update_mode:' || p_update_mode
                                            ||', symbol: '||p_symbol
                                            ||', p_basic_price: ' || p_basic_price
                                            ||', p_floor_price:'|| p_floor_price
                                            ||', p_ceiling_price:'|| p_ceiling_price);
    l_update_mode := p_update_mode;
    Select codeid,tradeplace, sectype into l_code_id,l_tradeplace,l_sectype
    From sbsecurities
    Where SYMBOL = p_symbol;
    l_basic_price:=p_basic_price;
    v_ceiling_price:= p_ceiling_price;

  --Ngay 07/07/2021 NamTv chinh sua dap ung he thong moi HOSE
  IF l_sectype='006' and p_floor_price=0 and p_ceiling_price>0 THEN
    v_floor_price:=1;
  ELSE
    v_floor_price:=p_floor_price;
  END IF;
  --Ngay 07/07/2021 NamTv
    ...
    BEGIN
        ...
        WHERE       symbol = p_symbol
        AND (p_ceiling_price + v_floor_price) / 2 >= fromprice
        AND (p_ceiling_price + v_floor_price) / 2 <= toprice;
    ...
    IF l_sectype <> '012' and ABS((p_ceiling_price + v_floor_price)/2 - p_basic_price) > 2* v_ticksize THEN
        l_basic_price:= Round(
                               ((p_ceiling_price + v_floor_price)/2)
                               /v_ticksize
                             )
                             * v_ticksize ;
    END IF;
    ...
    --San = san - 1 buoc gia
    --2.1.11.0: ko tinh lai  voi TPDN
      IF l_sectype <> '012' and p_ceiling_price=v_floor_price THEN
    ...
    -- DN : Dau ngay
        ...
        Set --FLOORPRICE         = p_floor_price,
            ...
            marginprice        = fn_calmarginprice(symbol,l_basic_price),--l_basic_price,
            margincallprice    = fn_calmarginprice(symbol,l_basic_price),--l_basic_price,
            marginrefprice     = fn_calmarginprice(symbol,l_basic_price),--l_basic_price,
            marginrefcallprice = fn_calmarginprice(symbol,l_basic_price)--l_basic_price
        ...
        update securities_info
             set FLOORPRICE         = v_floor_price,
                 CEILINGPRICE       = p_ceiling_price
         where v_floor_price >0 and p_ceiling_price >0 and
           CODEID in (Select CODEID From SBSECURITIES  Where sectype  not in ('003','006') and (CODEID = l_code_id or REFCODEID = l_code_id));

        -- Xu ly cap nhat gia cho ma nao la trai phieu, trai phieu chuyen doi
        update securities_info
         set FLOORPRICE         = v_floor_price,
             CEILINGPRICE       = p_ceiling_price
       where  CODEID in (Select CODEID From SBSECURITIES  Where sectype   in ('003','006') and (CODEID = l_code_id or REFCODEID = l_code_id));
