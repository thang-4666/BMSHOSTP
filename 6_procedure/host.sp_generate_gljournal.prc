SET DEFINE OFF;
CREATE OR REPLACE PROCEDURE sp_generate_gljournal
...
if v_tltxcd ='3387' then
    SELECT cvalue into v_afacctno  FROM vw_TLLOGFLD_ALL WHERE TXNUM =pv_txnum and txdate =v_txdate and fldcd ='03';
ELSIF v_tltxcd IN ('1192','8894','2251','2266') then
    select SUBSTR(msgacct,1,10)  into v_afacctno  from vw_tllog_all where tltxcd IN ('1192','8894','2251','2266') and TXNUM =pv_txnum and txdate =v_txdate;
else
    select max(acctno), max( busdate), max(ref) into v_afacctno, v_busdate,v_refci  from vw_citran_gen where txnum = pv_txnum and txdate = v_txdate ;
end if ;
...
select sb.symbol into v_symbol  from caschd ca,sbsecurities sb  where ca.codeid = sb.codeid and  CA.AUTOID = v_refci AND CA.deltd='N';
    ...
    SELECT cvalue into v_symbol  FROM TLLOGFLDALL WHERE TXNUM =pv_txnum and txdate =v_txdate and fldcd ='04';
else
--NGOAI BANG
...
select max(cvalue) into v_bankid  from vw_tllogfld_all where txnum = pv_txnum and txdate = v_txdate and fldcd ='02'  ;
end if ;

if v_tltxcd in ('1104','3387') then
select max(cvalue) into v_bankid  from vw_tllogfld_all where txnum = pv_txnum and txdate = v_txdate and fldcd ='05'  ;
end if ;

if v_tltxcd in ('2624','2646','2648') then
select max(cvalue) into v_groupid  from vw_tllogfld_all where txnum = pv_txnum and txdate = v_txdate and fldcd =case when v_tltxcd= '2648' then '05' else '20' end ;
...
SELECT nvalue into V_dueno FROM vw_tllogfld_all WHERE  TXNUM = pv_txnum AND TXDATE  =v_txdate AND fldcd='09';
...
IF v_TLTXCD in ('2244','2266') then
            ...
            if v_TLTXCD = '2266' then
                 ...
                 ELSE
                 SELECT ACCTNO  into v_afacctno FROM AFMAST WHERE ACCTNO IN (
                    SELECT cvalue  FROM TLLOGFLDALL WHERE TXNUM =pv_txnum and txdate =v_txdate and fldcd ='55');
                 end if  ;
            end if;
