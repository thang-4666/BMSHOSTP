SET DEFINE OFF;
CREATE OR REPLACE PROCEDURE "SE00312" (
   PV_REFCURSOR   IN OUT   PKG_REPORT.REF_CURSOR,
   OPT            IN       VARCHAR2,
   BRID           IN       VARCHAR2,
   TLGOUPS        IN       VARCHAR2,
   TLSCOPE        IN       VARCHAR2,
   F_DATE         IN       VARCHAR2,
   T_DATE         IN       VARCHAR2,
   PV_CUSTODYCD   IN       VARCHAR2,
   PV_AFACCTNO    IN       VARCHAR2,
   PLSENT         IN       VARCHAR2
)
IS

-- RP NAME : YEU CAU CHUYEN KHOAN CHUNG KHOAN TAT TOAN TAI KHOAN
-- PERSON --------------DATE---------------------COMMENTS
-- THANHNM            17/07/2012                 CREATE
-- SE00311: report main
-- ---------   ------  -------------------------------------------
   V_STRAFACCTNO  VARCHAR2 (15);
   V_CUSTODYCD VARCHAR2 (15);
   V_TYPE  VARCHAR2(10);
   V_CURRDATE DATE;
   V_FLAG NUMBER(2,0);
   V_INBRID        VARCHAR2(4);
   V_STRBRID      VARCHAR2 (50);
   V_STROPTION    VARCHAR2(5);

BEGIN
-- GET REPORT'S PARAMETERS
   V_STROPTION := upper(OPT);
   V_INBRID := BRID;
    if(V_STROPTION = 'A') then
        V_STRBRID := '%%';
    else
        if(V_STROPTION = 'B') then
            select br.mapid into V_STRBRID from brgrp br where  br.brid = V_INBRID;
        else
            V_STRBRID := V_INBRID;
        end if;
    end if;
    V_CUSTODYCD := UPPER( PV_CUSTODYCD);

    SELECT TO_DATE(VARVALUE,'DD/MM/RRRR') INTO V_CURRDATE
     FROM SYSVAR WHERE VARNAME = 'CURRDATE' AND GRNAME = 'SYSTEM';

   IF  (PV_AFACCTNO <> 'ALL')
   THEN
         V_STRAFACCTNO := PV_AFACCTNO;
   ELSE
         V_STRAFACCTNO := '%';
   END IF;


-- GET REPORT'S DATA  DEVIDENTRATE
 OPEN PV_REFCURSOR
 FOR
 SELECT DT.*,
            A0.CDCONTENT SAN_GD
        FROM (
 SELECT MAX(CF.CUSTODYCD) CUSTODYCD, MAX(REPLACE(SB.SYMBOL,'_WFT',''))  SYMBOL, CA.REPORTDATE,
        --  MAX(DECODE(CA.DEVIDENTRATE,0,CA.DEVIDENTVALUE ||'(/cp)' , CA.DEVIDENTRATE|| '(%)')) CA_TYLE,
        MAX(DECODE(CA.DEVIDENTRATE,0,TO_CHAR(CA.DEVIDENTVALUE)  , TO_CHAR(CA.DEVIDENTRATE))) CA_TYLE,

        SUM(CAS.TRADE)  REPORT_QTTY, SUM(CAS.CUTPBALANCE) CUTPBALANCE,
        SUM(CAS.QTTY + cas.cutqtty + cas.sendqtty)  QTTY,
        SUM(CAS.CUTAQTTY) CUTAQTTY
        , SUM( cas.cutamt + cas.sendamt )   AMT,
       -- SUM((CASE WHEN substr(CAS.PSTATUS,LENGTH(CAS.PSTATUS),1)='K' THEN  CAS.AMT - CAS.AMT*CA.EXERATE/100 + cas.cutamt + cas.sendamt ELSE
        --CAS.AMT + cas.cutamt + cas.sendamt END)) amt,
         0 CP_LE, 0 RIGHT_QTTY, 0 CK_MUA, ' ' NOTE, ca.description
        FROM CASCHD  CAS, CAMAST CA, CFMAST CF,
         AFMAST AF, SBSECURITIES SB
        WHERE CF.CUSTODYCD = V_CUSTODYCD AND CF.CUSTID = AF.CUSTID
        AND CAS.AFACCTNO = AF.ACCTNO AND CA.CAMASTID = CAS.CAMASTID
        AND CA.CATYPE='010' AND AF.ACCTNO LIKE V_STRAFACCTNO
        AND CAS.CODEID = SB.CODEID AND CAS.DELTD<>'Y'
        AND (af.brid LIKE V_STRBRID or instr(V_STRBRID,af.brid) <> 0 )
        AND CAS.STATUS ='O' GROUP BY CA.CAMASTID,CA.REPORTDATE,CAS.PSTATUS, ca.description ) DT, SBSECURITIES SB
        , ALLCODE A0 WHERE DT.SYMBOL = SB.SYMBOL AND A0.CDTYPE='SE' AND A0.CDNAME='TRADEPLACE' AND A0.CDVAL=SB.TRADEPLACE

      /* UNION ALL

       SELECT V_CUSTODYCD  CUSTODYCD, 'XXX' SYMBOL, TO_DATE(F_DATE,'DD/MM/YYYY') REPORTDATE,
        ' ' CA_TYLE,
       0  REPORT_QTTY,
        0 CUTPBALANCE, 0  QTTY,
         0 CUTAQTTY
        , 0   AMT, 0 CP_LE, 0 RIGHT_QTTY, 0 CK_MUA, ' ' NOTE,'C. UPCOM' SAN_GD
       FROM DUAL*/
         ;


EXCEPTION
   WHEN OTHERS
   THEN
      RETURN;
END;


---3

 
 
 
 
/
