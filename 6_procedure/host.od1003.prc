SET DEFINE OFF;
CREATE OR REPLACE PROCEDURE od1003 (
...
select V_P_date in_date, cf.fullname, cf.address, cf.custodycd,
    ...
    (case when od.execamt > 0 and od.feeacr = 0 then
        ...
        (CASE WHEN (od.execamt * od.feeacr) = 0 THEN 0 ELSE
            ...
            END)
    end) feeamt, sb.symbol, od.exectype, V_Set_day txdate, od.orderid, al.cdcontent tradeplace
from (select * from odmast where txdate = V_P_date
    ...
    sbsecurities sb, (SELECT * FROM CFMAST WHERE FNC_VALIDATE_SCOPE(BRID, CAREBY, TLSCOPE, pv_BRID, TLGOUPS)=0)  cf, afmast af, ODTYPE ODT, allcode al
