...
if l_CurrDate = to_date(p_DATE,'DD/MM/RRRR') then
    ...
    select cf.custodycd, sec74.afacctno, cf.fullname, af.mriratio mrirate, af.mrmratio mrmrate, af.mrlratio mrlrate, sec74.marginrate74 marginrate,
        ...
        round(greatest((af.mrmratio/100 - sec74.marginrate74/100) * (sec74.sereal + GREATEST(balance + nvl(avladvance,0) - nvl(ln.marginamt,0),0)),0),0) addvnd,
    ...
    where cf.custid = af.custid and af.acctno = sec74.afacctno
        ...
        and sec74.marginrate74 <= af.mrmratio and sec74.marginrate74 > af.mrlratio
    ...
    from report_rskmngt_log log, (SELECT * FROM CFMAST WHERE FNC_VALIDATE_SCOPE(BRID, CAREBY, TLSCOPE, pv_BRID, TLGOUPS)=0) cf, afmast af, cfmast cfb
