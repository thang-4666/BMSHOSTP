 ...
 (:newval.txstatus = '1' and (length(trim(:oldval.txstatus)) = 0 or :oldval.txstatus is null or :oldval.txstatus <> '1') )
  ...
  if :newval.txstatus = '1' and (:oldval.txstatus is null or :oldval.txstatus <> '1')
    ...
    IF L_COUNT >0 THEN
        l_search := '%';
        --KRX04
        if instr('/2241/2255/2292/8815/2275/2235/2238/', :newval.tltxcd) > 0  OR (:NEWVAL.TLTXCD='2247' AND :NEWVAL.MSGAMT >0) THEN  -- HSX04: iss 131: khong con CK chi con quyen -> khong sinh dien 
            begin
                SELECT CASE WHEN SECTYPE NOT IN ('003','006','222','444','012') AND REFCODEID IS NULL THEN '%UNIT/NONE/%'         -- Co phieu giao dich
                WHEN SECTYPE IN ('003','006','222','444','012') AND REFCODEID IS NULL THEN '%FAMT/NONE/%'             -- Trai phieu giao dich
                WHEN SECTYPE NOT IN ('003','006','222','444','012') AND REFCODEID IS NOT NULL THEN '%UNIT/PTA/%'      -- Co phieu cho giao dich
                  WHEN SECTYPE IN ('003','006','222','444','012') AND REFCODEID IS NOT NULL THEN '%FAMT/PTA/%'          -- Trai phieu cho giao dich
                ELSE '%UNIT/NONE/%' END
                into l_search
                from sbsecurities
