SET DEFINE OFF;DELETE FROM SEARCH WHERE 1 = 1 AND NVL(SEARCHCODE,'NULL') = NVL('DF3006','NULL');Insert into SEARCH   (SEARCHCODE, SEARCHTITLE, EN_SEARCHTITLE, SEARCHCMDSQL, OBJNAME, FRMNAME, ORDERBYCMDSQL, TLTXCD, CNTRECORD, ROWPERPAGE, AUTOSEARCH, INTERVAL, AUTHCODE, ROWLIMIT, CMDTYPE, CONDDEFFLD, BANKINQ, BANKACCT, CHKSCOPECMDSQL) Values   ('DF3006', 'Theo dõi trạng thái Deal tổng bị CẢNH BÁO', 'DF Deal Status', 'SELECT * FROM ( select al1.cdcontent DEALFLAGTRIGGER,DF.GROUPID,CF.CUSTODYCD,CF.FULLNAME,
...
DF.TAX,DF.AMTMIN,DF.IRATE,DF.ARATE,DF.ALRATE,DF.MRATE,DF.LRATE,DF.RLSAMT,DF.DESCRIPTION, lns.rlsdate, lns.overduedate,
to_date (lns.overduedate,''DD/MM/RRRR'') - to_date ((SELECT VARVALUE FROM SYSVAR WHERE VARNAME=''CURRDATE''),''DD/MM/RRRR'') duenum, lnt.exdays,
...
ROUND(ODCALLSELLARATE) ODCALLSELLARATE, ROUND(ODCALLSELLALRATE) ODCALLSELLALRATE,
...
) WHERE ODDF>0 AND ((RTT <= ALRATE AND RTT> MRATE) or (duenum < =exdays ) )', 'DFGROUP', 'frmViewDFMAST', 'GROUPID DESC', '', NULL, 50, 'N', 30, 'NYNNYYYYYN', 'Y', 'T', '', 'N', '', 'CUSTODYCD');COMMIT;