SET DEFINE OFF;DELETE FROM SEARCH WHERE 1 = 1 AND NVL(SEARCHCODE,'NULL') = NVL('OD8857','NULL');Insert into SEARCH   (SEARCHCODE, SEARCHTITLE, EN_SEARCHTITLE, SEARCHCMDSQL, OBJNAME, FRMNAME, ORDERBYCMDSQL, TLTXCD, CNTRECORD, ROWPERPAGE, AUTOSEARCH, INTERVAL, AUTHCODE, ROWLIMIT, CMDTYPE, CONDDEFFLD, BANKINQ, BANKACCT, CHKSCOPECMDSQL) Values   ('OD8857', 'Tra cứu giải tỏa lệnh bán ATO (8857)', 'Refuse ATO sell order(wait for 8857)', 'SELECT MST.ORDERID,OOD.ORGORDERID,OOD.OODSTATUS, MST.ACTYPE, MST.CODEID, CCY.SYMBOL, CCY.PARVALUE, MST.AFACCTNO, MST.SEACCTNO, MST.CIACCTNO,
...
(CASE WHEN MATCHTYPE=''N'' THEN ''N'' ELSE ''P'' END) NORP,  (CASE WHEN MST.REMAINQTTY < MST.ORDERQTTY THEN ''4'' WHEN MST.REMAINQTTY = MST.ORDERQTTY THEN ''6'' ELSE MST.ORSTATUS END) EXORSTATUS,
...
FROM ODMAST MST, TLLOG TLG,AFMAST AF, CFMAST CF,OOD, SBSECURITIES CCY, CIMAST CI,SEMAST SE,ALLCODE A0,ALLCODE A1
WHERE TRIM(MST.AFACCTNO) = TRIM(AF.ACCTNO) And TRIM(MST.CIACCTNO) = TRIM(CI.ACCTNO) AND TRIM(MST.SEACCTNO) = TRIM(SE.ACCTNO)
AND MST.REMAINQTTY>0 AND MST.DELTD<> ''Y'' AND MST.TXDATE = TLG.TXDATE AND MST.TXNUM = TLG.TXNUM
AND TRIM(AF.CUSTID) = TRIM(CF.CUSTID) AND TRIM(MST.CODEID) = TRIM(CCY.CODEID)
AND OOD.ORGORDERID=MST.ORDERID
AND (CASE WHEN EXECTYPE=''NB'' OR EXECTYPE=''BC'' THEN ''B'' ELSE ''S'' END)=''S''
AND MST.PRICETYPE IN (''ATO'', ''ATC'', ''MP'', ''MO'') AND MST.ORSTATUS=''8'' AND (CASE WHEN (tlg.offid IS NULL) AND tlg.tltxcd NOT IN (''8876'', ''8877'', ''8882'', ''8883'', ''8884'', ''8885'') THEN ''1'' ELSE ''0'' END ) =''0'' AND A0.CDTYPE=''OD'' AND A0.CDNAME=''ORSTATUS'' AND A0.CDVAL=MST.ORSTATUS AND A1.CDTYPE=''OD'' AND A1.CDNAME=''PRICETYPE'' AND A1.CDVAL=MST.PRICETYPE
 AND OOD.OODSTATUS=''N''
', 'ODMAST', '', '', '8857', NULL, 50, 'N', 30, 'NYNNYYYNNN', 'Y', 'T', '', 'N', '', '');COMMIT;