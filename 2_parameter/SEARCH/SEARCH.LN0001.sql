SET DEFINE OFF;DELETE FROM SEARCH WHERE 1 = 1 AND NVL(SEARCHCODE,'NULL') = NVL('LN0001','NULL');Insert into SEARCH   (SEARCHCODE, SEARCHTITLE, EN_SEARCHTITLE, SEARCHCMDSQL, OBJNAME, FRMNAME, ORDERBYCMDSQL, TLTXCD, CNTRECORD, ROWPERPAGE, AUTOSEARCH, INTERVAL, AUTHCODE, ROWLIMIT, CMDTYPE, CONDDEFFLD, BANKINQ, BANKACCT, CHKSCOPECMDSQL) Values   ('LN0001', 'Tra cứu thông tin tài khoản vay', 'View loan accounts', 'SELECT SCHD.AUTOID, MST.ACTYPE, (CASE WHEN SCHD.REFTYPE = ''P'' THEN ''Margin loan'' WHEN SCHD.REFTYPE = ''GP'' THEN ''T0 loan'' END) CATEGORY,
...
refullname, FULLNAME_GRP,mst.intpaidmethod, c1.cdcontent intpaidmethoddis, mst.drate, c2.cdcontent dratedis,
c3.cdcontent AUTOAPPLY, AFT.mnemonic LOAIHINH, tlg.grpname,NVL(TO_NUMBER(EXTIMES),0 )EXTIMES ,NVL(TO_NUMBER(EXDAYS),0) EXDAYS,
SCHD.nml+SCHD.ovd+SCHD.paid orgnml, SCHD.overduedate-SCHD.rlsdate-NVL(TO_NUMBER(EXDAYS),0) PERIODCHD
FROM vw_lnschd_all SCHD, vw_lnmast_all MST, CFMAST CF, CIMAST CI, AFMAST AF, CFMAST CFB, ALLCODE C1, ALLCODE C2, ALLCODE C3,
    AFTYPE AFT,
(SELECT VARVALUE BUSDATE FROM SYSVAR WHERE VARNAME=''BUSDATE'')SYS,
(select re.afacctno, max(cf.fullname) refullname, MAX(REG.FULLNAME) FULLNAME_GRP
    from reaflnk re, sysvar sys, cfmast cf, regrplnk regl ,regrp reg,RETYPE
...
', 'LNMAST', '', '', '', NULL, 50, 'N', 30, 'NYNNYYYNNN', 'Y', 'T', '', 'N', '', 'CUSTODYCD');COMMIT;