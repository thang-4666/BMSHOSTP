SET DEFINE OFF;DELETE FROM SEARCH WHERE 1 = 1 AND NVL(SEARCHCODE,'NULL') = NVL('ADDTEMPLATES','NULL');Insert into SEARCH   (SEARCHCODE, SEARCHTITLE, EN_SEARCHTITLE, SEARCHCMDSQL, OBJNAME, FRMNAME, ORDERBYCMDSQL, TLTXCD, CNTRECORD, ROWPERPAGE, AUTOSEARCH, INTERVAL, AUTHCODE, ROWLIMIT, CMDTYPE, CONDDEFFLD, BANKINQ, BANKACCT, CHKSCOPECMDSQL) Values   ('ADDTEMPLATES', 'Thông tin mẫu SMS/Email', 'SMS/Email templates', 'SELECT T.CODE, T.NAME, T.SUBJECT, A1.CDCONTENT TYPE, A2.CDCONTENT CYCLE,
 ...
 INNER JOIN ALLCODE A5 ON CASE WHEN T.TYPE = ''H'' THEN T.ALLOW_ZIP ELSE ''N'' END = A5.CDVAL AND A5.CDTYPE = ''SY''
   AND A5.CDNAME = ''YESNO''
 where code not in (select template_code from aftemplates where custid =''<@KEYVALUE>'')
 AND REQUIRE_REGISTER=''Y''', 'TEMPLATES', '', 'NAME', 'EXEC', NULL, 50, 'N', 30, '', 'Y', 'T', '', 'N', '', '');COMMIT;