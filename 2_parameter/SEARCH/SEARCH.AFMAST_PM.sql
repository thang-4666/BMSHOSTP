SET DEFINE OFF;DELETE FROM SEARCH WHERE 1 = 1 AND NVL(SEARCHCODE,'NULL') = NVL('AFMAST_PM','NULL');Insert into SEARCH   (SEARCHCODE, SEARCHTITLE, EN_SEARCHTITLE, SEARCHCMDSQL, OBJNAME, FRMNAME, ORDERBYCMDSQL, TLTXCD, CNTRECORD, ROWPERPAGE, AUTOSEARCH, INTERVAL, AUTHCODE, ROWLIMIT, CMDTYPE, CONDDEFFLD, BANKINQ, BANKACCT, CHKSCOPECMDSQL) Values   ('AFMAST_PM', 'Quản lý tiểu khoản tự doanh', 'Contract management PM', '
Select a.*, nvl(LIMITMAX,0)LIMITMAX ,nvl(USERHAVE,0)USERHAVE from
...
''N'' AS DELALLOW,A14.CDCONTENT BRKFEETYPE
...
and cfaf.afacctno = af.acctno and cfaf.afacctno IN (<@KEYVALUE>) and (cfaf.adminid = ''<$TELLERID>''  or cfaf.leaderid = ''<$TELLERID>'' or cfaf.traderid  = ''<$TELLERID>'' )
)a
LEFT JOIN
(Select NVL(US.ACCTLIMIT,0) LIMITMAX,NVL(US.ALLOCATELIMMIT-US.USEDLIMMIT,0) USERHAVE
  from userlimit US where US.tliduser=''<$TELLERID>'') us on 0=0', 'AFMAST_PM', 'frmAFMAST_PM', '', '', NULL, 50, 'N', 30, '', 'Y', 'T', '', 'N', '', '');COMMIT;