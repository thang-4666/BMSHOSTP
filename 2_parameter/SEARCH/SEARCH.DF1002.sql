SET DEFINE OFF;DELETE FROM SEARCH WHERE 1 = 1 AND NVL(SEARCHCODE,'NULL') = NVL('DF1002','NULL');Insert into SEARCH   (SEARCHCODE, SEARCHTITLE, EN_SEARCHTITLE, SEARCHCMDSQL, OBJNAME, FRMNAME, ORDERBYCMDSQL, TLTXCD, CNTRECORD, ROWPERPAGE, AUTOSEARCH, INTERVAL, AUTHCODE, ROWLIMIT, CMDTYPE, CONDDEFFLD, BANKINQ, BANKACCT, CHKSCOPECMDSQL) Values   ('DF1002', 'View theo dõi trạng thái deal bị call', 'Status of Called DF Deals', '
select * from
(SELECT v.*, ln.overduedate, nvl(NML,0) DUEAMT,round(mrate/100*refprice,0) CALLPRICE,CD.CDCONTENT DEALSTATUS,CD1.CDCONTENT DEALFLAGTRIGGER,
...
FROM v_getdealinfo v, ALLCODE CD, ALLCODE CD1, TLPROFILES TLPR, BRGRP BRG,
    (select DISTINCT acctno, overduedate  from (select acctno, overduedate from lnschd union all select acctno, overduedate from lnschdhist) )ln,
     (SELECT S.ACCTNO, SUM(NML) NML, M.TRFACCTNO FROM LNSCHD S, LNMAST M
        ...
        ORDER BY S.ACCTNO) sts, (select * from emailsmslog where reportname = ''DF1002'') esl,
...
', 'DFMAST', '', 'ACCTNO DESC', '', NULL, 50, 'N', 30, 'NYNNYYYYYN', 'Y', 'T', '', 'N', '', '');COMMIT;