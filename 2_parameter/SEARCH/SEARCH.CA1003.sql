SET DEFINE OFF;DELETE FROM SEARCH WHERE 1 = 1 AND NVL(SEARCHCODE,'NULL') = NVL('CA1003','NULL');Insert into SEARCH   (SEARCHCODE, SEARCHTITLE, EN_SEARCHTITLE, SEARCHCMDSQL, OBJNAME, FRMNAME, ORDERBYCMDSQL, TLTXCD, CNTRECORD, ROWPERPAGE, AUTOSEARCH, INTERVAL, AUTHCODE, ROWLIMIT, CMDTYPE, CONDDEFFLD, BANKINQ, BANKACCT, CHKSCOPECMDSQL) Values   ('CA1003', 'Chia cổ tức bằng tiền(đối chiếu)', 'Chia cổ tức bằng tiền(đối chiếu)', 'SELECT * FROM (SELECT CF.CUSTODYCD,MAX(CAS.AUTOID) AUTOID,CAS.CAMASTID,
  ...
  MAX(A0.CDVAL) CATYPEVALUE, CF.FULLNAME,
   ...
   SUM(CAS.BALANCE) BALANCE,SUM(CAS.QTTY) QTTY, SUM(CAS.AMT) AMT,
   SUM(CAS.AQTTY) AQTTY, MAX(CA.REPORTDATE) REPORTDATE,MAX(CA.ACTIONDATE) ACTIONDATE,
...
AND CAS.CAMASTID = CA.CAMASTID AND CA.CODEID = SYM.CODEID AND CA.CATYPE IN (''010'')
