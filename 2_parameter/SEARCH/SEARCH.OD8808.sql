SET DEFINE OFF;DELETE FROM SEARCH WHERE 1 = 1 AND NVL(SEARCHCODE,'NULL') = NVL('OD8808','NULL');Insert into SEARCH   (SEARCHCODE, SEARCHTITLE, EN_SEARCHTITLE, SEARCHCMDSQL, OBJNAME, FRMNAME, ORDERBYCMDSQL, TLTXCD, CNTRECORD, ROWPERPAGE, AUTOSEARCH, INTERVAL, AUTHCODE, ROWLIMIT, CMDTYPE, CONDDEFFLD, BANKINQ, BANKACCT, CHKSCOPECMDSQL) Values   ('OD8808', 'Tra cứu giải tỏa lệnh mua (8808)', 'View clearing buy order(wait for 8808)', 'SELECT MST.ORDERID, MST.ACTYPE, MST.CODEID, CCY.SYMBOL,CCY.TRADEPLACE, CCY.PARVALUE, MST.AFACCTNO, MST.SEACCTNO, MST.CIACCTNO,
...
(CASE WHEN EXECTYPE=''NB'' OR EXECTYPE=''BC'' THEN 1 ELSE 0 END)*(CASE WHEN MST.SECUREDAMT-MST.MATCHAMT-MST.RLSSECURED>0 THEN MST.SECUREDAMT-(MST.MATCHAMT +(MST.SECUREDAMT-MST.ORDERQTTY*MST.QUOTEPRICE)*MST.EXECQTTY/MST.ORDERQTTY )-MST.RLSSECURED ELSE 0 END) AVLCANCELAMT,
...
AND MST.ORDERID NOT IN (SELECT ODMAST.REFORDERID FROM ODMAST,OOD WHERE EXECTYPE IN (''AB'',''CB'') AND ODMAST.ORDERID=OOD.ORGORDERID AND OOD.OODSTATUS  IN (''N'',''B'') )', 'ODMAST', '', '', '8808', NULL, 50, 'N', 30, 'NYNNYYYNNN', 'Y', 'T', '', 'N', '', '');COMMIT;