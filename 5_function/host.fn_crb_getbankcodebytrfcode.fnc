...
ELSIF v_trfcode='TRFADVFRBANK' THEN
  BEGIN
    SELECT DECODE(RRTYPE,'B', CUSTBANK, NULL) INTO v_return FROM ADTYPE WHERE ACTYPE=v_refvalue;
    IF NOT v_return IS NULL THEN
      SELECT SHORTNAME INTO v_return FROM CFMAST WHERE CUSTID=v_return;
    END IF;
  END;
