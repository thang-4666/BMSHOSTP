SET DEFINE OFF;
CREATE OR REPLACE FUNCTION fn_3332_get_aqtty( pv_CAMASTID IN VARCHAR2,pv_AFACCTNO IN VARCHAR2,pv_TRADE IN NUMBER,pv_autoid number)
    RETURN NUMBER IS
    v_Result  NUMBER;
                                 ...
                                 ELSIF l_catype = '021' THEN --gc_CA_CATYPE_KIND_STOCK
                                  ...
                                  ELSIF l_catype = '012' THEN --gc_CA_CATYPE_STOCK_SPLIT 'Stock Split(+ QTTY,AMT)
                                    ...
                                    ELSIF l_catype = '023' THEN -- gc_CA_CATYPE_CONVERT_BOND_TO_SHARE 'Convert bond to share (+QTTY Share,-AQTTY Bound)
                                        ...
                                        l_dbl_qttyexp:=trunc (FLOOR( l_trade * l_dbl_right_exrate / l_dbl_left_exrate ),l_roundtype);
                                        l_dbl_amtexp:= trunc( l_parvalue *(1+l_interestrate/100)* l_trade,0);
                                         -- PhuongHT: ghi nhan rieng phan lai
                                          --l_dbl_intamtexp:= round(l_trade * l_parvalue /100 *  to_number(l_interestrate),0) ;
...
*/
    ...
    if l_catype in ('014') then
        select nvl(max(round(to_number(nvl(pv_TRADE,0)/trade * aqtty),0)),0) into l_dbl_aqttyexp
            from caschd
         where autoid=pv_autoid ;
    end if;

    select nvl(max(case when pv_trade = trade then aqtty else nvl(l_dbl_aqttyexp,0) end),0) into l_dbl_aqttyexp
        from caschd
     where autoid=pv_autoid ;

    v_Result:=nvl(l_dbl_aqttyexp,0);
    RETURN v_Result;
