SET DEFINE OFF;
CREATE OR REPLACE FUNCTION fn_3332_get_amt( pv_CAMASTID IN VARCHAR2,pv_AFACCTNO IN VARCHAR2,pv_TRADE IN NUMBER,pv_QTTYREGISTER IN NUMBER,pv_autoid number)
    RETURN NUMBER IS
    v_Result  NUMBER;
...
BEGIN
    ...
    l_dbl_amtexp:=0;
    l_dbl_qttyexp:=0;
    l_dblQTTYREGISTER:=pv_QTTYREGISTER;
    ...
    ELSIF l_catype = '014' THEN --gc_CA_CATYPE_STOCK_RIGHTOFF 'Stock Rightoff(+QTTY,-AAMT)
      l_dbl_qttyexp:=FLOOR((l_trade *l_dbl_right_rightoffrate * l_dbl_right_exrate )/( l_dbl_left_rightoffrate * l_dbl_left_exrate ));
      l_dbl_aamtexp:= round(l_exprice  * TRUNC( FLOOR((l_trade * l_dbl_right_rightoffrate * l_dbl_right_exrate)/( l_dbl_left_rightoffrate * l_dbl_left_exrate )),  l_roundtype ),0);
      l_dbl_RETAILBALEXP:= TRUNC(l_trade * l_dbl_right_exrate / l_dbl_left_exrate);
    ...
    ELSIF l_catype = '023' THEN -- gc_CA_CATYPE_CONVERT_BOND_TO_SHARE 'Convert bond to share (+QTTY Share,-AQTTY Bound)
      ...
      l_dbl_qttyexp:=trunc (FLOOR( l_trade * l_dbl_right_exrate / l_dbl_left_exrate ),l_roundtype);
      l_dbl_amtexp:= trunc( l_parvalue *(1+l_interestrate/100)* TRUNC((l_trade-(l_dblQTTYREGISTER* l_dbl_left_exrate  / l_dbl_right_exrate )),l_ciroundtype),0);
       -- PhuongHT: ghi nhan rieng phan lai
      --  l_dbl_intamtexp:= round(l_trade * l_parvalue /100 *  to_number(l_interestrate),0) ;
  ...
  /*  ELSIF  l_catype IN ( '005' , '006','022') THEN
    ...
    if l_catype in ('011','014','021','020','017','023') then
        select nvl(max(round(to_number(nvl(pv_TRADE,0)/trade * amt),0)),0) into l_dbl_amtexp
            from caschd
         where autoid=pv_autoid ;
    end if;

    select nvl(max(case when pv_trade = trade then amt else nvl(l_dbl_amtexp,0) end),0) into l_dbl_amtexp
        from caschd
     where autoid=pv_autoid ;

    v_Result:=nvl(l_dbl_amtexp,0);
