SET DEFINE OFF;
CREATE OR REPLACE FUNCTION fn_check_clearday_sec2(pv_codeid In VARCHAR2, pv_sectype IN VARCHAR2, pv_clearday in number)
--T10/2015 TTBT T+2:
--Ham check Chu ky thanh toan tren man hinh SYSTEM co <= Chu ky TT tren sysvar k voi TH khac trai phieu?
...
BEGIN
    ...
    IF (v_clearday <> v_sysclearday) THEN
        IF (v_codeid = '--ALL--') THEN
          if (v_sectype not in ('444', '003', '006', '222','012')) THEN
            v_result := 'False';
          ELSE
            v_result := 'True' ;
          END IF;
         ...
         ELSE
            ...
            IF (v_sectype not in ('444', '003', '006', '222','012') and v_sybol_sectype not in ('444', '003', '006', '222','012')) THEN
            v_result := 'False';
          ELSE
            v_result := 'True' ;
          END IF;
