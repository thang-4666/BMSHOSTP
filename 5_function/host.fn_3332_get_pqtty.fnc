SET DEFINE OFF;
CREATE OR REPLACE FUNCTION fn_3332_get_pqtty( pv_CAMASTID IN VARCHAR2,pv_AFACCTNO IN VARCHAR2,pv_TRADE IN NUMBER,pv_QTTY IN NUMBER,pv_autoid number)
    RETURN NUMBER IS
    v_Result  NUMBER;
...
SELECT catype,nvl(exrate,'1/1'),nvl(interestrate,0),nvl(exprice,0),nvl(parvalue,0),
    ...
    if l_catype in ('014') then
        select nvl(max(round( least(pqtty ,nvl(pv_TRADE,0)/trade * (pqtty+qtty)) ,0)),0) into l_dbl_pqtty
            from caschd
        where autoid=pv_autoid ;
    end if;

    select nvl(max(case when pv_trade = trade then pqtty else nvl(l_dbl_pqtty,0) end),0) into l_dbl_pqtty
        from caschd
   where autoid=pv_autoid ;
