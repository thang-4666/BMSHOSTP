SET DEFINE OFF;
CREATE OR REPLACE FUNCTION fn_3332_get_minpqtty( pv_CAMASTID IN VARCHAR2,pv_AFACCTNO IN VARCHAR2,pv_TRADE IN NUMBER,pv_QTTY IN NUMBER,pv_autoid number)
    RETURN NUMBER IS
    v_Result  NUMBER;
...
SELECT catype,nvl(exrate,'1/1'),nvl(interestrate,0),nvl(exprice,0),nvl(parvalue,0),
    ...
    l_dbl_right_rightoffrate := nvl(to_number(substr(l_rightoffrate,instr(l_rightoffrate,'/') + 1,length(l_rightoffrate))),1);
       ...
       if(l_catype='023') THEN
         ...
         l_dbl_pqtty:=l_dbl_qttyexp-l_dbl_qtty;
       ELSIF (l_catype='014') THEN
         ...
         l_dbl_pqtty:=round(l_dbl_qttyexp-l_dbl_qtty);
       END IF;
    select qtty into l_qtty from caschd where autoid=pv_autoid;
    if l_catype in ('014') then
        select greatest( (nvl(greatest(round( least(pqtty ,nvl(pv_TRADE,0)/trade * (pqtty+qtty)) ,0)),0) -l_qtty),0) into l_dbl_pqtty
            from caschd
        where autoid=pv_autoid ;
    end if;

    select (nvl(max(case when pv_trade = trade then pqtty else nvl(l_dbl_pqtty,0) end),0))
         into l_dbl_pqtty
        from caschd
    where autoid=pv_autoid ;

    v_Result:=nvl(l_dbl_pqtty,0);
    RETURN v_Result;
