SET DEFINE OFF;
CREATE OR REPLACE FUNCTION fnc_check_tltxcd_scope_by_type
(
pv_CUSTODYCD IN Varchar2,
V_TELLERBRID IN Varchar2,
v_TLID IN Varchar2,
v_CMDID IN Varchar2,
v_TLTYPE IN Varchar2 default 'ALL' --A: Approve T: Teller C: Cachier R: Risk
)
RETURN  NUMBER IS
  v_Return number;
  v_BrID varchar2(20);
...
if v_TLTYPE ='ALL' then
    ...
    SELECT COUNT(LEVSCOPE) INTO v_Count FROM
    (SELECT A.LSTODR LEVSCOPE
    FROM CMDAUTH C, ALLCODE A,TLAUTH TL
    WHERE C.CMDTYPE='T' AND C.CMDCODE=v_CMDID AND C.AUTHTYPE='U' AND C.AUTHID=v_TLID
    AND A.CDTYPE='SY' AND A.CDNAME='RIGHTSCOPE' AND A.CDVAL=C.RIGHTSCOPE
    and c.authtype = tl.authtype and c.authid=tl.authid and c.cmdcode = tl.tltxcd and tl.tltype=v_TLTYPE
    UNION ALL
    SELECT A.LSTODR LEVSCOPE
    FROM CMDAUTH C, ALLCODE A, TLGRPUSERS T,TLAUTH TL
    WHERE C.CMDTYPE='T' AND C.CMDCODE=v_CMDID AND C.AUTHTYPE='G' AND C.AUTHID=T.GRPID AND T.TLID=v_TLID
    AND A.CDTYPE='SY' AND A.CDNAME='RIGHTSCOPE' AND A.CDVAL=C.RIGHTSCOPE
    and c.authtype = tl.authtype and c.authid=tl.authid and c.cmdcode = tl.tltxcd and tl.tltype=v_TLTYPE);
