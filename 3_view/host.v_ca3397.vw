...
SELECT CA.ISCI,CA.ISCISE,CA.AUTOID, CA.BALANCE, SUBSTR(CA.CAMASTID,1,4) || '.' || SUBSTR(CA.CAMASTID,5,6) || '.' || SUBSTR(CA.CAMASTID,11,6) CAMASTID,
       ...
       (case when CF.STATUS IN ('P','A') Then 'Hoạt động' else 'Đóng' end ) CFSTATUS,
       (CASE WHEN cf.VAT='Y'
             THEN
               --T9/2019 CW_PhaseII
             --round(CAMAST.pitrate*CA.AMT/100)
             (case when CAMAST.Catype = '028' then round(LEAST(CA.AMT,CA.balance*CAMAST.EXPRICE*CAMAST.pitrate/100/(to_number(SUBSTR(SYM.EXERCISERATIO,0,INSTR(SYM.EXERCISERATIO,'/') - 1))/to_number(SUBSTR(SYM.EXERCISERATIO,INSTR(SYM.EXERCISERATIO,'/')+1,LENGTH(SYM.EXERCISERATIO)) ))
                                                                    )
                                                              )
                        else CAMAST.pitrate*CA.AMT/100 end )
            --End --T9/2019 CW_PhaseII
            --CAMAST.pitrate*CA.AMT/100
...
WHERE A0.CDTYPE = 'CA' AND A0.CDNAME = 'CATYPE' AND A0.CDVAL = CAMAST.CATYPE
      ...
      And CAMAST.CATYPE in ('010','009','011','021','017','020','015','016','023','028') and cf.custatcom='Y'
