SET DEFINE OFF;
CREATE OR REPLACE FORCE VIEW VW_MR0008
(CUSTODYCD, FULLNAME, AFACCTNO, ACTYPE, TYPENAME, 
 ...
 BALANCE, MARGINRATE, GRPNAME, CAREBY, MONEYPAY, 
 DAYPAY, DEPOFEEAMT, RTNAMOUNTREF, OVDAMOUNTREF, SELLLOSTASSREF, 
 SELLAMOUNTREF, SELLINGAMOUNT, MRIRATE, MRMRATE, RTNREMAINAMT, 
 ADDAMOUNT)
BEQUEATH DEFINER
AS 
...
(select od.afacctno,
    ...
    round(greatest(
            least(sum(od.remainqtty*od.quoteprice*(1-odt.deffeerate/100-to_number(sy.varvalue)/100)/(1+nvl(rsk.advrate,0)*getnonworkingday(3)/365)/*Gia tri tien ve tinh theo ty le UTTB*/),
                    sum(od.remainqtty*od.quoteprice*(1-odt.deffeerate/100-to_number(sy.varvalue)/100)-nvl(rsk.advminfee,0)/*Gia tri tien ve tinh theo phi UTTB toi thieu*/))
