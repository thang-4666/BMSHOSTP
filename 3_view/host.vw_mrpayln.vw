SET DEFINE OFF;
CREATE OR REPLACE FORCE VIEW VW_MRPAYLN
(ACCTNO, AMT)
BEQUEATH DEFINER
AS 
SELECT af.acctno , GREATEST( LEAST(CI.OVAMT+CI.DUEAMT, (CASE WHEN (CI.BALANCE+BUF.Avladvance) >=SYS.MINLOANAUTOPAYMENT THEN (CI.BALANCE+BUF.Avladvance)
                                                                                 ELSE 0 END))+
                                                                              -- no trong han se tra
