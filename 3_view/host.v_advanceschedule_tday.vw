SET DEFINE OFF;
CREATE OR REPLACE FORCE VIEW V_ADVANCESCHEDULE_TDAY
(CUSTODYCD, ISMORTAGE, MORTAGE, AFACCTNO, EXECAMT, 
 AMT, FAMT, AAMT, PAIDAMT, PAIDFEEAMT, 
 ACTYPE, CUSTID, CLEARDATE, TXDATE, VATRATE, 
 FEERATE, CURRDATE, ADVRATE, ADVMINAMT, ADVBANKRATE, 
 ADVMINFEEBANK, ADVMINBANK, AVATRATE, ADVMAXAMT, ADVMINFEE, 
 ADVMAXFEE, RRTYPE, CUSTBANK, CIACCTNO, TDAY, 
 DAYS, EXFEEAMT, AINTRATE, AFEEBANK, AMINBAL, 
 AMINFEEBANK)
BEQUEATH DEFINER
AS 
(
...
from (select mt.afacctno, mt.cleardate, mt.tday, mt.ISMORTAGE, MT.TXDATE,
             round((sum(case when (mt.feeacr <= 0 AND MT.TXDATE=TO_DATE(SYS.VARVALUE,'DD/MM/RRRR'))
                             then (mt.deffeerate/100)*mt.execamt
