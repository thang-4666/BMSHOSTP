SET DEFINE OFF;
CREATE OR REPLACE FORCE VIEW V_ORDERADVANCESCHEDULE
(ISMORTAGE, AFACCTNO, AMT, QTTY, FULLNAME, 
 ...
 ODAMT, DEPOAMT, MAXDEPOAMT, ADVAMT)
BEQUEATH DEFINER
AS 
(
SELECT ISMORTAGE,AFACCTNO,AMT,QTTY,FULLNAME,FAMT,CUSTODYCD,SYMBOL,AAMT,ORGORDERID,PAIDAMT,
    ...
    ROUND(LEAST(ODAMT/(1-FEERATE*DAYS/100/360),LEAST(ROUND(ADVAMT), (ADVAMT-MINBAL)/(1-FEERATE*DAYS/100/360))),4) DEFADVAMT,
    ROUND(LEAST(ODAMT,LEAST(ROUND(ADVAMT*(1-FEERATE*DAYS/100/360)), ADVAMT-MINBAL)),4) ODAMT,
    ROUND(GREATEST(LEAST(ROUND(ADVAMT*(1-FEERATE*DAYS/100/360)), ADVAMT-MINBAL)-ODAMT,0),4) DEPOAMT,
    ROUND(GREATEST(LEAST(ROUND(ADVAMT*(1-FEERATE*DAYS/100/360)), ADVAMT-MINBAL)-ODAMT,0),4) MAXDEPOAMT,
    GREATEST(ADVAMT,0) ADVAMT
FROM (
    SELECT  1 ISMORTAGE,STSCHD.AFACCTNO,AMT,QTTY,FULLNAME,FAMT,
            CUSTODYCD,STSCHD.SYMBOL,AAMT,ORGORDERID,PAIDAMT,PAIDFEEAMT,STSCHD.ODAMT,CD.CDCONTENT DFTYPE,
    ...
    (SELECT greatest(max(round((sts.qtty-sts.aqtty + nvl(v.rlsqtty,0)) * nvl(v.amt,0)/(nvl(v.remainqtty,1) + nvl(v.rlsqtty,1))
                          -nvl(v.rlsamt,0) + nvl(v.dealfee,0),4)),0) ODAMT,
        ...
        FROM STSCHD STS,ODMAST MST,AFMAST AF,SBSECURITIES SEC, AFTYPE TYP, SYSVAR SYS,v_getDealInfo v, cfmast cf
