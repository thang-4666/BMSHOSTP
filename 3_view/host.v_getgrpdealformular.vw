SET DEFINE OFF;
CREATE OR REPLACE FORCE VIEW V_GETGRPDEALFORMULAR
(AFACCTNO, GROUPID, LNACCTNO, SUMALLVALUE, ODDF, 
 TA0DF, IRATE, MRATE, LRATE, ARATE, 
 ALRATE, DFBLOCKAMT, EXAMT, CURAMT, CURINT, 
 CURFEE, INTPAIDMETHOD, RATE1, RATE2, CFRATE1, 
 CFRATE2, MINTERM, PRINFRQ, RLSDATE, DUEDATE, 
 GOCPHAITRA, INTMIN, FEEMIN, DUE, INTPAID, 
 FEEPAID, TADF, DFAMT, ODOVERDUEDF, ODDUEDF, 
 ISVSD, ODSELLDF, MAXDEPOAMT, VNDSELLDF, DDF, 
 DOVERDUEDF, DDUEDF, RTTDF, ODCALLRTTDF, ODCALLMRATE, 
 ODCALLLRATE, ODCALLDF, VNDWITHDRAWDF, ADDVALUE, ODCALLSELLDF, 
 ODCALLSELLIRATE, ODCALLSELLMRATE, ODCALLSELLRCB, ODCALLSELLRXL, ODCALLSELLARATE, 
 ODCALLSELLALRATE, ADDVALUEDF1050, ADDVALUEDF1050_R)
BEQUEATH DEFINER
AS 
(

SELECT A."AFACCTNO",A."GROUPID",A."LNACCTNO",A."SUMALLVALUE",A."ODDF",A."TA0DF",A."IRATE",A."MRATE",A."LRATE",A."ARATE",A."ALRATE",A."DFBLOCKAMT",A."EXAMT",A."CURAMT",A."CURINT",A."CURFEE",A."INTPAIDMETHOD",A."RATE1",A."RATE2",A."CFRATE1",A."CFRATE2",A."MINTERM",A."PRINFRQ",A."RLSDATE",A."DUEDATE",A."GOCPHAITRA",A."INTMIN",A."FEEMIN",A."DUE",A."INTPAID",A."FEEPAID",A."TADF",A."DFAMT",A."ODOVERDUEDF",A."ODDUEDF",A."ISVSD",A."ODSELLDF",A."MAXDEPOAMT",A."VNDSELLDF",A."DDF",A."DOVERDUEDF",A."DDUEDF",A."RTTDF",A."ODCALLRTTDF",A."ODCALLMRATE",A."ODCALLLRATE", CEIL( ODcallRttDF + GREATEST(DoverdueDF + DdueDF - ODcallRttDF ,0)) ODcallDF, ROUND(LEAST ( GREATEST ( TADF/(CASE WHEN IRATE/100=0 THEN 1 ELSE IRATE/100 END) - DDF ,0 ), dfamt )) VNDwithdrawDF,
    ...
    ODCALLRTTDF + GREATEST ( DoverdueDF - ODCALLRTTDF,0) ODCALLSELLIRATE,
    ...
    ODCALLARATE + GREATEST ( DoverdueDF - ODCALLARATE,0) ODCALLSELLARATE,  ODCALLALRATE + GREATEST ( DoverdueDF - ODCALLALRATE,0) ODCALLSELLALRATE,
    ADDVALUEDF1050 + GREATEST ( DoverdueDF - ADDVALUEDF1050,0) ADDVALUEDF1050,
    ADDVALUEDF1050  ADDVALUEDF1050_R
...
FROM (
    SELECT A.*,  GREATEST ( 0 ,  ODoverdueDF - VNDSellDF - dfamt - A.DFBLOCKAMT ) DoverdueDF, ROUND(GREATEST ( 0 ,  ODdueDF - VNDSellDF - dfamt - A.DFBLOCKAMT )) DdueDF
     , CASE WHEN DDF=0 THEN 10000000 ELSE ROUND(TADF/(CASE WHEN DDF=0 THEN 1 ELSE DDF END),4) * 100  END RTTDF, CEIL(GREATEST ( DDF - TADF/ ((CASE WHEN IRATE=0 THEN 1 ELSE IRATE END)/100) ,0)) ODCALLRTTDF,
        ...
        ROUND(GREATEST ( DDF - TADF/ ((CASE WHEN ARATE=0 THEN 1 ELSE ARATE END)/100) ,0)) ODCALLARATE, ROUND(GREATEST ( DDF - TADF/ ((CASE WHEN ARATE=0 THEN 1 ELSE ALRATE END)/100) ,0)) ODCALLALRATE,
        ROUND(GREATEST ( DDF - TADF/ ((CASE WHEN ALRATE=0 THEN 1 ELSE ARATE + EXPECTRATE END)/100) ,0)) ADDVALUEDF1050

         FROM (


       SELECT A.*,ROUND(NVL(B.MAXDEPOAMT,0),0) MAXDEPOAMT,
            CASE WHEN ROUND(LEAST(A.ODSELLDF,NVL(B.MAXDEPOAMT,0))) <=2 THEN 0 ELSE ROUND(LEAST(A.ODSELLDF,NVL(B.MAXDEPOAMT,0))) end VNDSellDF,  ROUND(GREATEST ( 0 ,  ODDF - LEAST(A.ODSELLDF,NVL(B.MAXDEPOAMT,0)) - dfamt - A.DFBLOCKAMT )) DDF
       FROM (

         SELECT AFACCTNO,GROUPID,LNACCTNO, SUMALLVALUE,  ODDF, TA0DF,IRATE, MRATE,LRATE,ARATE,ALRATE,DFBLOCKAMT,EXAMT,
                   ...
                   INTPAID , FEEPAID,TADF,DFAMT, ODoverdueDF,ODdueDF,ISVSD, EXPECTRATE,
            ...
            FROM (
                ...
                SELECT AFACCTNO,GROUPID,LNACCTNO, SUMALLVALUE, ODDF, TA0DF, CURAMT, CURINT, CURFEE, INTPAIDMETHOD,DFBLOCKAMT,EXAMT,
                       RATE1, RATE2,CFRATE1, CFRATE2, MINTERM, PRINFRQ, RLSDATE, DUEDATE, GOCPHAITRA, INTMIN, FEEMIN, DUE,IRATE,MRATE,LRATE,ARATE,ALRATE,TADF,DFAMT,  ODoverdueDF,ODdueDF,INTPENA_CUR, FEEPENA_CUR,
                ...
                FROM (
                    ...
                    FROM (
                     ...
                     SELECT  B.AFACCTNO, B.GROUPID, A.LNACCTNO, SUMALLVALUE,  CURAMT + CURINT + CURFEE ODDF, CURAMT, CURINT, CURFEE, INTPAIDMETHOD, RATE1, RATE2,  CFRATE1, CFRATE2, MINTERM,
                            DUE, ODoverdueDF,ODdueDF, PRINFRQ, RLSDATE, DUEDATE , DF.DFAMT, DF.IRATE, DF.MRATE, DF.LRATE,DF.ARATE,DF.ALRATE, DF.DFBLOCKAMT, DF.EXAMT,
                            SUMALLVALUE - SUM(TADF) TADF, SUMALLVALUE TA0DF, DFT.ISVSD, DFT.EXPECTRATE,
                     ...
                     FROM (  SELECT LN.ACCTNO LNACCTNO, round(LNS.NML) + round(LNS.OVD) CURAMT,
                        ...
                        ) B, DFGROUP DF, DFTYPE DFT
                     WHERE A.LNACCTNO=B.LNACCTNO AND DF.LNACCTNO = B.LNACCTNO AND DF.ACTYPE=DFT.ACTYPE

                     GROUP BY B.AFACCTNO, B.GROUPID, A.LNACCTNO, SUMALLVALUE, DFT.ISVSD, DFT.EXPECTRATE, CURAMT + CURINT + CURFEE, CURAMT, CURINT, CURFEE, INTPAIDMETHOD, RATE1, RATE2,  CFRATE1, CFRATE2, MINTERM,
                        DUE, ODoverdueDF,ODdueDF, PRINFRQ, RLSDATE, DUEDATE, DF.DFAMT, DF.IRATE, DF.MRATE, DF.LRATE,DF.ARATE,DF.ALRATE, DF.DFBLOCKAMT, DF.EXAMT
            ...
            FROM (
              ...
              SELECT  1 ISMORTAGE,STSCHD.AFACCTNO,AMT,QTTY,CFMAST.FULLNAME,CFMAST.ADDRESS,CFMAST.idcode LICENSE,FAMT,
                ...
                (SELECT STS.ORGORDERID,STS.TXDATE,STS.AFACCTNO, STS.CODEID CODEID,
                       ...
                       (CASE WHEN cf.VAT='Y' THEN TO_NUMBER(SYS.VARVALUE) ELSE 0 END) SECDUTY
                    FROM STSCHD STS,ODMAST MST,AFMAST AF,SBSECURITIES SEC, cfmast cf, AFTYPE TYP, SYSVAR SYS
