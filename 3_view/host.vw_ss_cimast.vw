SET DEFINE OFF;
CREATE OR REPLACE FORCE VIEW VW_SS_CIMAST
(TYPE_AF, DESCRIPTION, ALTERNATEACCT, BALANCE_FLEX, TR_BANK, 
 AMTT3_FLEX, BALANCE_SBS, AMTT3_SBS, CHENHLECH)
BEQUEATH DEFINER
AS 
SELECT substr(CI_FLEX.DESCRIPTION,11) TYPE_AF, CI_FLEX.DESCRIPTION, ALTERNATEACCT, CI_FLEX.BALANCE BALANCE_FLEX , NVL(CI_FLEX.TR_BANK,0) TR_BANK  ,
...
FROM (
SELECT CF.CUSTODYCD||CASE WHEN AFTYPE.mnemonic='T3' THEN 'T' WHEN  AFTYPE.mnemonic='Margin' THEN 'M' ELSE 'C' END DESCRIPTION,ci.acctno,(ALTERNATEACCT) ALTERNATEACCT,
 (CI.BALANCE)-  NVL( tr.namt,0)- sts.execbuyamt   BALANCE, (TR_BANK.NAMT) TR_BANK
FROM CIMAST CI,AFMAST AF ,
(SELECT sum( CASE WHEN TR.TXTYPE='C' THEN TR.NAMT ELSE - TR.NAMT END ) namt ,acctno FROM vw_citran_gen tr 
     WHERE busdate >= '16-DEC-2013'  AND field ='BALANCE' AND 0=1
 GROUP BY acctno     
     )TR,
(SELECT ACCTNO, SUM(NAMT) NAMT FROM vw_citran_gen  WHERE busdate >= '16-DEC-2013'  AND field ='BALANCE' AND TLTXCD ='6669' GROUP BY ACCTNO )TR_BANK,
...
AND substr(CI_FLEX.DESCRIPTION,11) in( 'T','C','M')

UNION

SELECT substr(CI_FLEX.DESCRIPTION,11) TYPE_AF,CI_FLEX.DESCRIPTION, ALTERNATEACCT, CI_FLEX.BALANCE BALANCE_FLEX ,NVL( CI_FLEX.TR_BANK,0)  TR_BANK,0 AMTT3_FLEX,CI_SBS.BALANCE BALANCE_SBS , 0 AMTT3_SBS ,
NVL(CI_FLEX.BALANCE,0 )- NVL(CI_SBS.BALANCE   ,0) CHENHLECH
FROM (

SELECT CF.CUSTODYCD||CASE WHEN AFTYPE.mnemonic='T3' THEN 'T' WHEN  AFTYPE.mnemonic='Margin' THEN 'M' ELSE 'C' END DESCRIPTION,ci.acctno,(af.ALTERNATEACCT) ALTERNATEACCT,
 (CI.BALANCE)-  NVL( tr.namt,0)- sts.execbuyamt   BALANCE, (TR_BANK.NAMT) TR_BANK
FROM CIMAST CI,AFMAST AF ,
(SELECT sum( CASE WHEN TR.TXTYPE='C' THEN TR.NAMT ELSE - TR.NAMT END ) namt ,acctno FROM vw_citran_gen tr 
     WHERE busdate >= '16-DEC-2013'  AND field ='BALANCE' AND 0=1
