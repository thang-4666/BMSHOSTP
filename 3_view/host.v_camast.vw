...
CAMAST.CODEID, CAMAST.EXCODEID, A1.CDVAL TYPEID, A1.CDCONTENT CATYPE, getprevdate(REPORTDATE,2) KHQDATE , REPORTDATE ,DUEDATE,ACTIONDATE ,BEGINDATE, EXPRICE, EXRATE, RIGHTOFFRATE, DEVIDENTRATE,OPTCODEID,
...
(CASE WHEN EXRATE IS NOT NULL THEN EXRATE ELSE (CASE WHEN RIGHTOFFRATE IS NOT NULL
       ...
       (CASE WHEN DEVIDENTSHARES IS NOT NULL THEN DEVIDENTSHARES ELSE '0' END)END)END)END) END)END) RATE,CASE WHEN  PITRATE = 0 THEN TO_NUMBER( SYS.VARVALUE) ELSE TO_NUMBER( PITRATE) END PITRATE, A3.CDVAL PITRATEMETHOD_CD, A3.CDCONTENT PITRATEMETHOD,
...
CASE WHEN  PITRATESE = 0 THEN  TO_NUMBER( SYSSE.VARVALUE) ELSE TO_NUMBER( PITRATESE) END      PITRATESE,
    ...
    nvl(schd2.QTTY,0) CAQTTY, NVL(camast.CANCELDATE,TO_DATE('20/03/2050','DD/MM/RRRR')) CANCELDATE, NVL(camast.RECEIVEDATE,TO_DATE('20/03/2050','DD/MM/RRRR')) RECEIVEDATE, camast.isincode,
    CAMAST.TRADEDATE, NVL(schd2.TOTALQTTY,0) TOTALQTTY, CAMAST.VSDID
 ...
 (SELECT SUM(nvl(trade,0)) trade, SUM(nvl(QTTY,0)) QTTY, SUM(BALANCE) TOTALQTTY, camastid from caschd WHERE deltd='N' AND isexec <> 'N' AND STATUS <> 'O' GROUP BY camastid) schd2,
   (SELECT SUM(TRADE + MARGIN + WTRADE  + MORTAGE + BLOCKED + SECURED + REPO+ NETTING+ DTOCLOSE+ WITHDRAW ) trade, cODEid
        from SEMAST GROUP BY CODEID) SE,
 SBSECURITIES TOSYM,SYSVAR SYS, SYSVAR SYSSE
 WHERE CAMAST.CODEID=SYM.CODEID AND CAMAST.CODEID = SE.CODEID (+)
 AND A1.CDTYPE = 'CA'
