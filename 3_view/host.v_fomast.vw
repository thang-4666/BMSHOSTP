SET DEFINE OFF;
CREATE OR REPLACE FORCE VIEW V_FOMAST
(FOACCTNO, ORDERID, TRADEPLACE, AFACCTNO, CUSTODYCD, 
 ACTYPE, CODEID, SEACCTNO, CIACCTNO, TXNUM, 
 TXDATE, TXTIME, EXPDATE, BRATIO, TIMETYPE, 
 EXECTYPE, NORK, MATCHTYPE, VIA, CLEARDAY, 
 CLEARCD, ORSTATUS, PRICETYPE, EDSTATUS, QUOTEPRICE, 
 STOPPRICE, LIMITPRICE, ORDERQTTY, REMAINQTTY, EXECQTTY, 
 STANDQTTY, CANCELQTTY, ADJUSTQTTY, REJECTQTTY, QUOTEQTTY, 
 REJECTCD, CUSTID, EXPRICE, EXQTTY, DELTD, 
 OODSTATUS, FEEDBACKMSG)
BEQUEATH DEFINER
AS 
(
SELECT SUBSTR(FO.ACCTNO,1,4) || '.' || SUBSTR(FO.ACCTNO,5,6) || '.' || SUBSTR(FO.ACCTNO,11,6) FOACCTNO, nvl(ORGACCTNO,'----------------') ORDERID,
...
--(case when pricetype='SL' then  FO.PRICE*INF.TRADEUNIT else 0 end ) STOPPRICE,
...
FO.CANCELQTTY, 0 ADJUSTQTTY, 0 REJECTQTTY,(case when pricetype='IO' then FO.QUOTEQTTY else 0 end ) QUOTEQTTY ,'' REJECTCD,'' CUSTID,0 EXPRICE,0 EXQTTY,A9.CDCONTENT DELTD,
