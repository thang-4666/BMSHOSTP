SET DEFINE OFF;
CREATE OR REPLACE FORCE VIEW VW_SS_CIMAST_AF_BATCH
...
(SELECT sum( CASE WHEN TR.TXTYPE='C' THEN TR.NAMT ELSE - TR.NAMT END ) namt ,acctno FROM vw_citran_gen tr 
     WHERE busdate >= '29-DEC-2033'  AND field ='BALANCE' 
 GROUP BY acctno     
     )TR,
(SELECT ACCTNO, SUM(NAMT) NAMT FROM vw_citran_gen  WHERE busdate >= '29-DEC-2033'  AND field ='BALANCE' AND TLTXCD ='6669' GROUP BY ACCTNO )TR_BANK,
