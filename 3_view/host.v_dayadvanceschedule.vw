SET DEFINE OFF;
CREATE OR REPLACE FORCE VIEW V_DAYADVANCESCHEDULE
(AFACCTNO, AMT, FULLNAME, CUSTODYCD, COREBANK, 
 ...
 REMAINPOOL, CIDRAWNDOWN, BANKDRAWNDOWN, CMPDRAWNDOWN, KEY)
BEQUEATH DEFINER
AS 
(
SELECT MST.AFACCTNO,MST.AMT,MST.FULLNAME,MST.CUSTODYCD,
                ...
                GREATEST (ROUND(ADVAMT*MST.DAYS*FEECMP/100/365/(1+MST.DAYS*FEECMP/100/360)),MST.MINBAL) CMPFEEAMT,
                GREATEST (ROUND(ADVAMT*MST.DAYS*FEEBANK/100/360/(1+MST.DAYS*FEEBANK/100/360)),MST.MINFEEBANK) BANKFEEAMT,
                GREATEST (ROUND(ADVAMT*MST.DAYS*FEECMP/100/360/(1+MST.DAYS*FEECMP/100/360)),MST.MINBAL)* 10/100 +
                GREATEST (ROUND(ADVAMT*MST.DAYS*FEEBANK/100/360/(1+MST.DAYS*FEEBANK/100/360)),MST.MINFEEBANK) * 10/100 VATAMT,
...
SELECT  STS.AFACCTNO,STS.AMT,CF.FULLNAME,CF.CUSTODYCD,CASE WHEN CI.COREBANK='Y' THEN 0 ELSE 1 END COREBANK,
   ...
   DECODE( (sy1.varvalue),'Y',0,'N',    ROUND(LEAST(
...
FROM
v_advanceSchedule sts,AFMAST AF,CIMAST CI, CFMAST CF, ALLCODE A1,sysvar sy1
WHERE STS.AFACCTNO =AF.ACCTNO
        AND AF.CUSTID=CF.CUSTID
        AND CI.AFACCTNO=AF.ACCTNO
        And sy1.grname = 'SYSTEM' and sy1.varname ='ISSTOPADV'
        --AND AF.COREBANK='N'
