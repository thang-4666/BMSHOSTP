SET DEFINE OFF;
CREATE OR REPLACE FORCE VIEW V_ORDER_ADVANCESCHEDULE
(ORDERID, AUTOADV, AFACCTNO, EXECAMT, AMT, 
 FAMT, AAMT, PAIDAMT, PAIDFEEAMT, ACTYPE, 
 CUSTID, CLEARDATE, TXDATE, VATRATE, FEERATE, 
 CURRDATE, ADVRATE, ADVMINAMT, ADVBANKRATE, ADVMINFEEBANK, 
 ADVMINBANK, AVATRATE, ADVMAXAMT, ADVMINFEE, ADVMAXFEE, 
...
AS 
(
select MTA.ORDERID, af.autoadv,mta.afacctno, mta.execamt, mta.amt, mta.famt, mta.aamt, mta.paidamt, mta.paidfeeamt, aft.actype,
...
from (select MT.ORDERID, mt.afacctno, mt.cleardate, MT.TXDATE, round((sum(case when mt.feeacr <= 0 then (mt.deffeerate/100)*mt.execamt
      ...
      from  (SELECT MST.ORDERID, STS.AFACCTNO,STS.CLEARDATE, MST.ACTYPE, STS.TXDATE,
             ...
             GROUP BY MST.ORDERID, STS.AFACCTNO,STS.CLEARDATE,mst.ACTYPE, STS.TXDATE) MT,
             (SELECT * FROm SYSVAR WHERE VARNAME='CURRDATE' AND GRNAME='SYSTEM') SYS
      group by MT.ORDERID, mt.afacctno, mt.cleardate, MT.TXDATE) mta,
...
where mta.afacctno =  af.acctno and af.custid = cf.custid and af.actype = aft.actype
      ...
      --AND aft.actype = map.aftype and adt.actype = map.actype and map.objname ='AD.ADTYPE' and adt.autoadv = 'Y'
       and cf.custatcom='Y'
        --Loai di nhung tai khoan luu ky ben ngoai
