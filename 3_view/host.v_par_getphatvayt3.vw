SET DEFINE OFF;
CREATE OR REPLACE FORCE VIEW V_PAR_GETPHATVAYT3
(CUSTODYCD, ACCTNO, DAYCREDIT, BALANCE, ODAMT, 
 DUEAMT, OVAMT, SECUREAMT, PHATVAYFSS, PHATVAY_SBS, 
 LECH_PHATVAY_FSS_SBS)
BEQUEATH DEFINER
AS 
select a.custodycd,af.acctno, a.daycredit, ci.balance,
        ci.odamt,nvl(indue.nml,0) dueamt, nvl(ov.ovamt,0) ovamt, NVL (b.secureamt, 0) secureamt,
    greatest(-(ci.balance - least(ci.balance,ci.odamt) - nvl(secureamt,0)),0) PhatvayFSS,a.currentdebit Phatvay_SBS,
    greatest(-(ci.balance - least(ci.balance,ci.odamt) - nvl(secureamt,0)),0) - nvl(a.currentdebit,0) Lech_Phatvay_FSS_SBS
