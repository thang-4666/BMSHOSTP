SET DEFINE OFF;
CREATE OR REPLACE PACKAGE txpks_#2276ex
/**----------------------------------------------------------------------------------------------------
 ** Package: TXPKS_#2276EX
...
CREATE OR REPLACE PACKAGE BODY txpks_#2276ex
IS
   pkgctx   plog.log_ctx;
   ...
   c_decustodycd      CONSTANT CHAR(2) := '09';
...
BEGIN
    ...
    ***************************************************************************************************/
    IF p_txmsg.deltd <> 'Y' THEN
       UPDATE sereceived SET status = 'R' WHERE autoid = p_txmsg.txfields(c_autoid).value;
    ELSE
       UPDATE sereceived SET status = 'A' WHERE autoid = p_txmsg.txfields(c_autoid).value;
    END IF;
