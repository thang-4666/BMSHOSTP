...
BEGIN
   IF p_txmsg.deltd <> 'Y' THEN -- Normal transaction
    ...
    IF to_char(p_txmsg.txfields('96').value) <> '0' THEN
      INSERT INTO CITRAN(TXNUM,TXDATE,ACCTNO,TXCD,NAMT,CAMT,ACCTREF,DELTD,REF,AUTOID,TLTXCD,BKDATE,TRDESC)
            VALUES (p_txmsg.txnum, TO_DATE (p_txmsg.txdate, systemnums.C_DATE_FORMAT),p_txmsg.txfields ('43').value,'0011',ROUND(p_txmsg.txfields('10').value+p_txmsg.txfields('11').value+p_txmsg.txfields('14').value,0),NULL,p_txmsg.txfields ('03').value,p_txmsg.deltd,p_txmsg.txfields ('03').value,seq_CITRAN.NEXTVAL,p_txmsg.tltxcd,p_txmsg.busdate,'Giai ngân ung truoc tien ban lenh giao dich ngay ' || p_txmsg.txfields ('42').VALUE || ' thanh toan ngay ' || p_txmsg.txfields ('08').VALUE || '');
    END IF;
