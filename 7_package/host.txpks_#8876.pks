SET DEFINE OFF;
CREATE OR REPLACE PACKAGE txpks_#8876
/** ----------------------------------------------------------------------------------------------------
 ** Module: TX
 ** Description: Make normal buy order-Direct
...
CREATE OR REPLACE PACKAGE BODY txpks_#8876
IS
   pkgctx   plog.log_ctx;
...
plog.setbeginsection (pkgctx, 'pr_txlog');
   ...
   if v_count=0 then
      INSERT INTO tllog(autoid, txnum, txdate, txtime, brid, tlid,offid, ovrrqs, chid, chkid, tltxcd, ibt, brid2, tlid2, ccyusage,off_line, deltd, brdate, busdate, txdesc, ipaddress,wsname, txstatus, msgsts, ovrsts, batchname, msgamt,msgacct, chktime, offtime, reftxnum)
          ...
          p_txmsg.brid2,
          p_txmsg.tlid2,
          NVL(p_txmsg.txfields('01').value,p_txmsg.txdesc),
          p_txmsg.off_line,
          p_txmsg.deltd,
          TO_DATE(p_txmsg.txdate, systemnums.C_DATE_FORMAT),
...
-- Run Pre Update
   IF txpks_#8876EX.fn_txPreAppUpdate(p_txmsg,p_err_code) <> systemnums.C_SUCCESS THEN
...
-- Run Pre check
   IF txpks_#8876EX.fn_txPreAppCheck(p_txmsg,p_err_code) <> systemnums.C_SUCCESS THEN
       RETURN errnums.C_BIZ_RULE_INVALID;
   END IF;
-- Run Auto check
   ...
   END IF;
-- Run After check
   IF txpks_#8876EX.fn_txAftAppCheck(p_txmsg,p_err_code) <> systemnums.C_SUCCESS THEN
       RETURN errnums.C_BIZ_RULE_INVALID;
   END IF;
