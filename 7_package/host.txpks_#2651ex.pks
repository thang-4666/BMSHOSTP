SET DEFINE OFF;
CREATE OR REPLACE PACKAGE txpks_#2651ex
/**----------------------------------------------------------------------------------------------------
 ** Package: TXPKS_#2651EX
...
CREATE OR REPLACE PACKAGE BODY txpks_#2651ex
IS
   pkgctx   plog.log_ctx;
...
BEGIN
    ...
    * END IF;
    ***************************************************************************************************/
    select dft.addciallow into l_addciallow
    from dfgroup dfg, dftype dft
    where dfg.actype = dft.actype and dfg.groupid = p_txmsg.txfields(c_groupid).value;
    if l_addciallow <> 'Y' then
        p_err_code := '-260051'; -- Pre-defined in DEFERROR table
        plog.setendsection (pkgctx, 'fn_txPreAppCheck');
        RETURN errnums.C_BIZ_RULE_INVALID;
    end if;
