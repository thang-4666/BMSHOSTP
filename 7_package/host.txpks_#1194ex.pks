SET DEFINE OFF;
CREATE OR REPLACE PACKAGE txpks_#1194ex
/**----------------------------------------------------------------------------------------------------
 ** Package: TXPKS_#1194EX
...
CREATE OR REPLACE PACKAGE BODY txpks_#1194ex
IS
   pkgctx   plog.log_ctx;
...
BEGIN
    ...
    ***************************************************************************************************/
    if p_txmsg.deltd<>'Y' then
        SELECT TXDESC,EN_TXDESC into v_strDesc, v_strEN_Desc FROM  TLTX WHERE TLTXCD='1138';
        ...
        l_txmsg.tltxcd:='1138';
            ...
            FOR REC in (
                     select a.autoid, a.custodycd, a.acctno, a.amt, b.fullname, b.address, b.idcode, b.iddate, b.idplace, a.fileid,a.des
                        from TBLCI1138 a, cfmast b where a.fileid = p_txmsg.txfields('03').value and nvl(a.status,'N') <> 'Y'
                ...
                SELECT systemnums.C_BATCH_PREFIXED
                  ...
                  FROM DUAL;
                   ...
                   l_txmsg.txfields ('10').VALUE     := rec.amt;
                    ...
                    BEGIN
                        IF txpks_#1138.fn_batchtxprocess (l_txmsg,
                                                         p_err_code,
                                                         l_err_param
                    ...
                    update TBLCI1138 set status = 'Y' where autoid = rec.autoid;

            end loop;

        insert into tblci1138hist select * from TBLCI1138;
        delete from tblci1138;

        cspks_rmproc.sp_exec_create_crbtrflog_multi('TRFRLSFEE',p_err_code);
        ...
        loop
            if txpks_#1138.fn_txrevert(rec.txnum,to_char(rec.txdate,'dd/mm/rrrr'),p_err_code,l_err_param) <> 0 then
