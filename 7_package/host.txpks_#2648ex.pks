SET DEFINE OFF;
CREATE OR REPLACE PACKAGE txpks_#2648ex
/**----------------------------------------------------------------------------------------------------
 ** Package: TXPKS_#2648EX
...
CREATE OR REPLACE PACKAGE BODY txpks_#2648ex
IS
   pkgctx   plog.log_ctx;
...
BEGIN
    ...
    * END IF;
    ***************************************************************************************************/
--    if (p_txmsg.txfields('34').value = p_txmsg.txfields('08').value ) and
--       (p_txmsg.txfields('35').value < p_txmsg.txfields('09').value + p_txmsg.txfields('20').value ) and
--       (p_txmsg.txfields('36').value < p_txmsg.txfields('10').value + p_txmsg.txfields('21').value ) then

    if ROUND(fn_getamt4grpdeal(p_txmsg.txfields('05').value,0,5)) <>   to_number(p_txmsg.txfields('22').value) and substr(p_txmsg.txnum,1,2) <> systemnums.C_BATCH_PREFIXED then
           p_err_code := '-260018';
           plog.setendsection (pkgctx, 'fn_txPreAppCheck');
           RETURN errnums.C_BIZ_RULE_INVALID;
    end if;

    SELECT INTPAIDMETHOD into v_strINTPAIDMETHOD FROM LNMAST WHERE ACCTNO IN (SELECT LNACCTNO FROM DFGROUP WHERE GROUPID=p_txmsg.txfields('05').value);

    IF to_number(p_txmsg.txfields('26').value) >= to_number(p_txmsg.txfields('08').value)
        AND to_number(p_txmsg.txfields('26').value) < to_number(p_txmsg.txfields('22').value) and nvl(instr('L',v_strINTPAIDMETHOD),0) > 0  THEN
       p_err_code := '-260018';
       plog.setendsection (pkgctx, 'fn_txPreAppCheck');
       RETURN errnums.C_BIZ_RULE_INVALID;
    end if;
...
BEGIN
    ...
    ***************************************************************************************************/
    ...
    l_amt:=p_txmsg.txfields('17').value + p_txmsg.txfields('19').value;
