SET DEFINE OFF;
CREATE OR REPLACE PACKAGE txpks_#6685
/** ----------------------------------------------------------------------------------------------------
 ** Module: TX
 ** Description: Send EOD report manualy
...
CREATE OR REPLACE PACKAGE BODY txpks_#6685
IS
   pkgctx   plog.log_ctx;
...
RETURN  NUMBER IS
   l_allow         boolean;
    ...
    l_crbtrflogcheck_arr txpks_check.crbtrflogcheck_arrtype;
    l_crbdefbankcheck_arr txpks_check.crbdefbankcheck_arrtype;
BEGIN
   ...
   IF p_txmsg.deltd = 'N' THEN
      ...
      INTO l_STATUS
        FROM CRBDEFBANK
        WHERE BANKCODE = p_txmsg.txfields('95').value;

     IF NOT ( INSTR('OI',l_STATUS) > 0) THEN
        p_err_code := '-660001';
plog.setendsection (pkgctx, 'fn_txAppAutoCheck');
     ...
     IF NOT ( INSTR('PA',l_STATUS) > 0) THEN
        p_err_code := '-100432';
plog.setendsection (pkgctx, 'fn_txAppAutoCheck');
        RETURN errnums.C_BIZ_RULE_INVALID;
     END IF;
...
-- Run Pre Update
   IF txpks_#6685EX.fn_txPreAppUpdate(p_txmsg,p_err_code) <> systemnums.C_SUCCESS THEN
       RETURN errnums.C_BIZ_RULE_INVALID;
   END IF;
-- Run Auto Update
   ...
   END IF;
-- Run After Update
   IF txpks_#6685EX.fn_txAftAppUpdate(p_txmsg,p_err_code) <> systemnums.C_SUCCESS THEN
       RETURN errnums.C_BIZ_RULE_INVALID;
   END IF;
...
-- Run Pre check
   IF txpks_#6685EX.fn_txPreAppCheck(p_txmsg,p_err_code) <> systemnums.C_SUCCESS THEN
       RETURN errnums.C_BIZ_RULE_INVALID;
   END IF;
-- Run Auto check
   ...
   END IF;
-- Run After check
   IF txpks_#6685EX.fn_txAftAppCheck(p_txmsg,p_err_code) <> systemnums.C_SUCCESS THEN
       RETURN errnums.C_BIZ_RULE_INVALID;
   END IF;
...
BEGIN
   ...
   LOOP
       ...
       IF txpks_#6685.fn_AutoTxProcess (l_txmsg,
                                        p_err_code,
                                        p_err_param
          ) <> systemnums.c_success
       THEN
           plog.debug (pkgctx,
           'got error 6685: ' || p_err_code
           );
           ROLLBACK;
           plog.setendsection (pkgctx, 'fn_txrevert');
           RETURN errnums.C_SYSTEM_ERROR;
