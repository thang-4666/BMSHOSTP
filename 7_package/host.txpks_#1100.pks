SET DEFINE OFF;
CREATE OR REPLACE PACKAGE txpks_#1100
/** ----------------------------------------------------------------------------------------------------
 ** Module: TX
 ** Description: Cash withdraw base on balance
...
CREATE OR REPLACE PACKAGE BODY txpks_#1100
IS
   pkgctx   plog.log_ctx;
...
RETURN  NUMBER IS
   l_allow         boolean;
    ...
    l_custtype apprules.field%TYPE;
...
BEGIN
   ...
   IF p_txmsg.deltd = 'N' THEN
     ...
     If txpks_check.fn_aftxmapcheck(p_txmsg.txfields('03').value,'CIMAST','03','1100')<>'TRUE' then
     ...
     l_CUSTTYPE := l_AFMASTcheck_arr(0).CUSTTYPE;
...
plog.setendsection (pkgctx, 'fn_txAppAutoCheck');
     ...
     END IF;
     IF NOT ( INSTR('CP',l_CUSTTYPE) > 0) THEN
        p_err_code := '-200306';
plog.setendsection (pkgctx, 'fn_txAppAutoCheck');
        RETURN errnums.C_BIZ_RULE_INVALID;
     END IF;
...
-- Run Pre Update
   IF txpks_#1100EX.fn_txPreAppUpdate(p_txmsg,p_err_code) <> systemnums.C_SUCCESS THEN
       RETURN errnums.C_BIZ_RULE_INVALID;
   END IF;
-- Run Auto Update
   ...
   END IF;
-- Run After Update
   IF txpks_#1100EX.fn_txAftAppUpdate(p_txmsg,p_err_code) <> systemnums.C_SUCCESS THEN
       RETURN errnums.C_BIZ_RULE_INVALID;
   END IF;
...
-- Run Pre check
   IF txpks_#1100EX.fn_txPreAppCheck(p_txmsg,p_err_code) <> systemnums.C_SUCCESS THEN
       RETURN errnums.C_BIZ_RULE_INVALID;
   END IF;
-- Run Auto check
   ...
   END IF;
-- Run After check
   IF txpks_#1100EX.fn_txAftAppCheck(p_txmsg,p_err_code) <> systemnums.C_SUCCESS THEN
       RETURN errnums.C_BIZ_RULE_INVALID;
   END IF;
...
BEGIN
   ...
   LOOP
       ...
       IF txpks_#1100.fn_AutoTxProcess (l_txmsg,
                                        p_err_code,
                                        p_err_param
          ) <> systemnums.c_success
       THEN
           plog.debug (pkgctx,
           'got error 1100: ' || p_err_code
           );
           ROLLBACK;
           plog.setendsection (pkgctx, 'fn_txrevert');
           RETURN errnums.C_SYSTEM_ERROR;
