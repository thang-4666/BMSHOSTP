SET DEFINE OFF;
CREATE OR REPLACE PACKAGE txpks_#2256ex
/**----------------------------------------------------------------------------------------------------
 ** Package: TXPKS_#2256EX
...
CREATE OR REPLACE PACKAGE BODY txpks_#2256ex
IS
   pkgctx   plog.log_ctx;
...
BEGIN
    ...
    ***************************************************************************************************/
    ...
    ELSE
       SELECT count(1) into v_count FROM semortage WHERE AUTOID =  p_txmsg.txfields('04').VALUE and status = 'C';
        if v_count >0 then
