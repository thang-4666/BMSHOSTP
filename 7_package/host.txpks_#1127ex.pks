SET DEFINE OFF;
CREATE OR REPLACE PACKAGE txpks_#1127ex
/**----------------------------------------------------------------------------------------------------
 ** Package: TXPKS_#1127EX
...
CREATE OR REPLACE PACKAGE BODY txpks_#1127ex
IS
   pkgctx   plog.log_ctx;
...
BEGIN
    plog.setbeginsection (pkgctx, 'fn_txAftAppUpdate');
     plog.debug (pkgctx, 'point -2');
    plog.debug (pkgctx, '<<BEGIN OF fn_txAftAppUpdate');
     plog.debug (pkgctx, 'point -1');
     if p_txmsg.deltd='Y' then
        v_blnReversal:=true;
    else
        v_blnReversal:=false;
    end if;
     plog.debug (pkgctx, 'point 0');
     v_acctno:= p_txmsg.txfields(c_afacctno).value;
      plog.debug (pkgctx, 'c_afacctno ' ||  p_txmsg.txfields(c_afacctno).value);
     v_cidepofeeacr:= p_txmsg.txfields(c_decrease).value;
       plog.debug (pkgctx, 'v_cidepofeeacr ' ||  p_txmsg.txfields(c_decrease).value);
