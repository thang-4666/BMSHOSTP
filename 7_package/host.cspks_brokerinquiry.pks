     ...
     from buf_ci_account ci, afmast af, cimast cim,
                (select afacctno,
                 ...
                 from (
                        ...
                        --ROC_CongThuc_HSX04
                        select afacctno,mrratioloan,basicprice,nvl(st.closeprice,basicprice) currprice,AVLMRQTTY qtty,AVLDFQTTY dfqtty,buyingqtty
                        ,buyqtty, (buyqtty+receiving-buyingqtty)*basicprice totalbuyamt,(buyqtty-buyingqtty+trade+mortage+receiving+BLOCKed+RESTRICTQTTY+ABSTANDING+Remainqtty+withdraw- case when sb.domain in ('STCK','CBND') then  nvl(se.securities_receiving_t0,0) + nvl(se.securities_receiving_t1,0) + nvl(se.securities_receiving_t2,0) else 0 end )
                        *basicprice totalamt
                        from buf_se_account se, sbsecurities sb ,stockinfor st
                         where afacctno = p_afacctno and se.codeid= sb.codeid and sb.symbol = st.symbol(+)
...
--                (TRADE + DFTRADING + ABSTANDING + RESTRICTQTTY + BLOCKED + CARECEIVING + ODRECEIVING + MATCHINGAMT  - buyingqtty) totalqtty,
                  (TRADE + DFTRADING + ABSTANDING + RESTRICTQTTY + BLOCKED +WITHDRAW + CARECEIVING + /*ODRECEIVING*/+ MATCHINGAMT  /*- buyingqtty*/+ SECURITIES_RECEIVING_T3) totalqtty,    --bo mua Chung khoan cho ve:ODRECEIVING
