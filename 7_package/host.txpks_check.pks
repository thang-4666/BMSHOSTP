...
IS
   ...
   BEGIN                                                               -- Proc
      SELECT TO_DATE (varvalue, 'DD/MM/YYYY')
       ...
       SELECT DECODE(COUNT(1), 1, 'Y', 'N') INTO l_isDVP3
       FROM sbsecurities WHERE codeid = substr(pv_condvalue, 11, 6) AND domain IN ('STCK','CBND');
    ...
    IF L_COUNT > 0 THEN
      OPEN pv_refcursor FOR
         SELECT semast.actype, semast.acctno, semast.codeid, semast.afacctno,
                ...
                semast.costprice, 
                --greatest(semast.trade - NVL (b.secureamt, 0) + NVL(b.sereceiving,0),0) trade,
                greatest(semast.trade + DECODE(l_isDVP3,'Y',semast.execbuyqtty + semast.odreceiving,0) - (NVL (b.secureamt, 0)/*+ nvl(b.sereceiving,0)*/) ,0) trade, --HSX04
                ...
                --greatest(semast.trade - nvl(b.secureamt,0) + nvl(b.sereceiving,0),0) trading,
                greatest(semast.trade + DECODE(l_isDVP3,'Y',semast.execbuyqtty + semast.odreceiving,0) - nvl(b.secureamt,0) /*+ nvl(b.sereceiving,0)*/,0) trading, -- HSX04
   ...
   BEGIN                                                               -- Proc
      ...
      SELECT TO_CHAR(DECODE(COUNT(1), 1, 'Y', 'N')) INTO l_isDVP3
      FROM sbsecurities WHERE codeid = substr(pv_condvalue, 11, 6) AND domain IN ('STCK','CBND');--GW04
      ...
      THEN
        ...
        --GW04
            /*SELECT 1000000000 avlsewithdraw
              FROM DUAL;*/
             SELECT se.trade - DECODE(l_isDVP3, 'Y', greatest(0, nvl(sell.SECUREAMT, 0) - se.execbuyqtty - se.odreceiving), nvl(sell.SECUREAMT,0))
                      AS avlsewithdraw
            FROM semast se, V_GETSELLORDERINFO sell
            WHERE se.acctno = sell.SEACCTNO (+)
            and se.acctno = pv_condvalue;
        --END GW04
      ELSIF     l_margintype in ('S','T','F')
            AND (LENGTH (l_groupleader) = 0 OR l_groupleader IS NULL)
      THEN
         ...
         OPEN pv_refcursor FOR
            select least(--se.trade- nvl(od.SELLQTTY,0),
                        se.trade - DECODE(l_isDVP3, 'Y', greatest(0, nvl(od.SECUREAMT,0) - se.execbuyqtty - se.odreceiving), nvl(od.SECUREAMT,0)),--GW04
                            (CASE WHEN LEAST(NVL(SB.MARGINCALLPRICE,0),NVL(RSK.MRPRICERATE,0)) * NVL(RSK.MRRATIORATE,0) <=0 THEN 1000000000
                                ELSE
