SET DEFINE OFF;
CREATE OR REPLACE PACKAGE txpks_#6682
/** ----------------------------------------------------------------------------------------------------
 ** Module: TX
 ** Description: Duty Fee Transfer
...
CREATE OR REPLACE PACKAGE BODY txpks_#6682
IS
   pkgctx   plog.log_ctx;
...
plog.setbeginsection (pkgctx, 'pr_txlog');
   ...
   if v_count=0 then
      INSERT INTO tllog(autoid, txnum, txdate, txtime, brid, tlid,offid, ovrrqs, chid, chkid, tltxcd, ibt, brid2, tlid2, ccyusage,off_line, deltd, brdate, busdate, txdesc, ipaddress,wsname, txstatus, msgsts, ovrsts, batchname, msgamt,msgacct, chktime, offtime, reftxnum)
       ...
       plog.debug(pkgctx, 'abt to insert into tllogfld');
       INSERT INTO tllogfld(AUTOID, TXNUM, TXDATE, FLDCD, NVALUE, CVALUE, TXDESC)
          VALUES( seq_tllogfld.NEXTVAL, p_txmsg.txnum, TO_DATE(p_txmsg.txdate, systemnums.C_DATE_FORMAT),'31',0,p_txmsg.txfields('31').value,'Org TxNum');
       plog.debug(pkgctx, 'abt to insert into tllogfld');
       INSERT INTO tllogfld(AUTOID, TXNUM, TXDATE, FLDCD, NVALUE, CVALUE, TXDESC)
...
BEGIN
   ...
   IF p_txmsg.deltd = 'N' THEN
     ...
     If txpks_check.fn_aftxmapcheck(p_txmsg.txfields('03').value,'CIMAST','03','6682')<>'TRUE' then
...
BEGIN
       ...
       if not v_blnREVERSAL then
           ...
           --Begin Gen yeu cau sang ngan hang 6682-TRFODTAX
           v_strOBJTYPE:='T';
           v_strREFCODE:=p_txmsg.txfields('31').value;
           v_strAFFECTDATE:=p_txmsg.txfields('08').value;
           v_strTRFCODE:='TRFODTAX';
           ...
           if length(v_strBANK)>0 and length(v_strBANKACCT)>0 and v_strVALUE > 0 then
               ...
               --Gen detail request to bankORGTXNUM
               v_strFLDNAME:='ORGTXNUM';
               v_strFLDTYPE:='C';
               v_strAMTEXP:='$31';
               v_strSQL:='';
               v_strVALUE := substr(v_strAMTEXP, 2);
...
-- Run Pre Update
   IF txpks_#6682EX.fn_txPreAppUpdate(p_txmsg,p_err_code) <> systemnums.C_SUCCESS THEN
       RETURN errnums.C_BIZ_RULE_INVALID;
   END IF;
-- Run Auto Update
...
-- Run send bank request
   ...
   END IF;
-- Run After Update
   IF txpks_#6682EX.fn_txAftAppUpdate(p_txmsg,p_err_code) <> systemnums.C_SUCCESS THEN
       RETURN errnums.C_BIZ_RULE_INVALID;
   END IF;
...
-- Run Pre check
   IF txpks_#6682EX.fn_txPreAppCheck(p_txmsg,p_err_code) <> systemnums.C_SUCCESS THEN
       RETURN errnums.C_BIZ_RULE_INVALID;
   END IF;
-- Run Auto check
   ...
   END IF;
-- Run After check
   IF txpks_#6682EX.fn_txAftAppCheck(p_txmsg,p_err_code) <> systemnums.C_SUCCESS THEN
       RETURN errnums.C_BIZ_RULE_INVALID;
   END IF;
...
BEGIN
   ...
   LOOP
       ...
       IF txpks_#6682.fn_AutoTxProcess (l_txmsg,
                                        p_err_code,
                                        p_err_param
