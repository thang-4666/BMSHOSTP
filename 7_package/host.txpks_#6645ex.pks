SET DEFINE OFF;
CREATE OR REPLACE PACKAGE txpks_#6645ex
/**----------------------------------------------------------------------------------------------------
 ** Package: TXPKS_#6645EX
...
CREATE OR REPLACE PACKAGE BODY txpks_#6645ex
IS
   pkgctx   plog.log_ctx;
   ...
   plog.setbeginsection (pkgctx, 'fn_genBankRequest');
       ...
       if not v_blnREVERSAL then
           ...
           if length(v_strBANK)>0 and length(v_strBANKACCT)>0 and v_strVALUE >0 then
               ...
               --Bang ke nay khong hach toan tien
               /*if v_refdorc = 'D' then
