SET DEFINE OFF;
CREATE OR REPLACE PACKAGE txpks_#5541ex
/**----------------------------------------------------------------------------------------------------
 ** Package: TXPKS_#5541EX
...
CREATE OR REPLACE PACKAGE BODY txpks_#5541ex
IS
   pkgctx   plog.log_ctx;
   ...
   c_rtnamt           CONSTANT CHAR(2) := '10';
   c_marginrate       CONSTANT CHAR(2) := '11';
   c_marginratio      CONSTANT CHAR(2) := '12';
   c_mriratio         CONSTANT CHAR(2) := '14';
   c_lntype           CONSTANT CHAR(2) := '20';
...
BEGIN
    ...
    ***************************************************************************************************/
    ...
    begin
        select rrtype, custbank into l_rrtype, l_custbank from lntype where actype = p_txmsg.txfields(c_lntype).value;
    exception when others then
        l_rrtype:= 'X';
        l_custbank:= 'XXXXXXXXXXXX';
    end;
    select custid into l_custid from afmast where acctno = p_txmsg.txfields(c_afacctno).value;
    if l_rrtype = 'B' then
        begin
        ...
        IF NOT l_avlamt >= TO_NUMBER(p_txmsg.txfields(c_rtnamt).value) THEN
    ...
    select 100 - mriratio into l_mriratio from afmast where acctno = p_txmsg.txfields(c_afacctno).value;
    for rec_ln in
    (
      select * from (
      ...
      where actype = p_txmsg.txfields(c_lntype).value
    ...
    loop -- rec_ln
        begin
            ...
            (select trfacctno,
                ...
                where trfacctno = p_txmsg.txfields(c_afacctno).value
        ...
        IF NOT l_afavlamt >= TO_NUMBER(p_txmsg.txfields(c_rtnamt).value) THEN
            p_err_code := '-100423'; -- Pre-defined in DEFERROR table
            plog.setendsection (pkgctx, 'fn_txPreAppCheck');
            RETURN errnums.C_BIZ_RULE_INVALID;
        END IF;
...
BEGIN
    ...
    ***************************************************************************************************/
    ...
    FOR REC IN
        (
        SELECT p_txmsg.txfields(c_afacctno).value ACCTNO, LNT.CCYCD, LNT.LNTYPE, LNT.LNCLDR, LNT.PRINFRQ, LNT.PRINPERIOD,
        ...
        FROM LNTYPE LNT
        WHERE LNT.actype=p_txmsg.txfields(c_lntype).value
        and not exists (select 1 from lnmast where trfacctno = p_txmsg.txfields(c_afacctno).value and actype = p_txmsg.txfields(c_lntype).value)
        )
    LOOP
...
BEGIN
    ...
    ***************************************************************************************************/
    ...
    /*
    -- Giai ngan Margin: 5567
    */

    l_AvlAmt:= to_number(p_txmsg.txfields(c_rtnamt).value);
    ...
    (
        select ln.trfacctno, ln.acctno, ls.autoid lnschdid,
            ...
            where ln.trfacctno = p_txmsg.txfields(c_afacctno).value and ls.reftype = 'P' and ln.actype = lnt.actype and lnt.chksysctrl = 'Y'
            and (ln.prinnml+ln.prinovd+ln.intnmlacr+ln.intdue+ln.intovdacr+ln.intnmlovd+ln.feeintnmlacr+ln.feeintdue+ln.feeintovdacr+ln.feeintnmlovd) > 0
    ...
    loop
        ...
        l_T0INTNMLOVD := 0;
        If l_AvlAmt > 0 Then
            l_T0INTNMLOVD := round(least(l_AvlAmt, rec_rls.T0INTNMLOVD),0);
            l_AvlAmt := l_AvlAmt - l_T0INTNMLOVD;
        End If;
        --02.T0INTOVDACR
        l_T0INTOVDACR := 0;
        If l_AvlAmt > 0 Then
            l_T0INTOVDACR := round(least(l_AvlAmt, rec_rls.T0INTOVDACR),0);
            l_AvlAmt := l_AvlAmt - l_T0INTOVDACR;
        End If;
        --03.T0PRINOVD
        l_T0PRINOVD := 0;
        If l_AvlAmt > 0 Then
            l_T0PRINOVD := round(least(l_AvlAmt, rec_rls.T0PRINOVD),0);
            l_AvlAmt := l_AvlAmt - l_T0PRINOVD;
        end if;
        --04.T0INTDUE
        l_T0INTDUE := 0;
        If l_AvlAmt > 0 Then
             l_T0INTDUE := round(least(l_AvlAmt, rec_rls.T0INTDUE),0);
        ...
        l_T0INTNMLACR := 0;
        If l_AvlAmt > 0 Then
            l_T0INTNMLACR := round(least(l_AvlAmt, rec_rls.T0INTNMLACR),0);
            l_AvlAmt := l_AvlAmt - l_T0INTNMLACR;
        End If;
        --07.T0PRINNML
        l_T0PRINNML := 0;
        If l_AvlAmt > 0 Then
            l_T0PRINNML := round(least(l_AvlAmt, rec_rls.T0PRINNML),0);
        ...
        l_FEEINTNMLOVD := 0;
        If l_AvlAmt > 0 Then
            l_FEEINTNMLOVD := round(least(l_AvlAmt, rec_rls.FEEINTNMLOVD),0);
            l_AvlAmt := l_AvlAmt - l_FEEINTNMLOVD;
        End If;
        --09.FEEINTDUE
        l_FEEINTDUE := 0;
        If l_AvlAmt > 0 Then
             l_FEEINTDUE := round(least(l_AvlAmt, rec_rls.FEEINTDUE),0);
             l_AvlAmt := l_AvlAmt - l_FEEINTDUE;
        End If;
        --10.FEEINTNMLACR
        l_FEEINTNMLACR := 0;
        If l_AvlAmt > 0 Then
            l_FEEINTNMLACR := round(least(l_AvlAmt, rec_rls.FEEINTNMLACR),0);
        ...
        l_INTNMLOVD := 0;
        If l_AvlAmt > 0 Then
            l_INTNMLOVD := round(least(l_AvlAmt, rec_rls.INTNMLOVD),0);
            l_AvlAmt := l_AvlAmt - l_INTNMLOVD;
        End If;
        --12.INTOVDACR
        l_INTOVDACR := 0;
        If l_AvlAmt > 0 Then
             l_INTOVDACR := round(least(l_AvlAmt, rec_rls.INTOVDACR),0);
             l_AvlAmt := l_AvlAmt - l_INTOVDACR;
        End If;
        --13.INTDUE
        l_INTDUE := 0;
        If l_AvlAmt > 0 Then
             l_INTDUE := round(least(l_AvlAmt, rec_rls.INTDUE),0);
             l_AvlAmt := l_AvlAmt - l_INTDUE;
        End If;
        --14.INTNMLACR
        l_INTNMLACR := 0;
        If l_AvlAmt > 0 Then
            l_INTNMLACR := round(least(l_AvlAmt, rec_rls.INTNMLACR),0);
        ...
        l_FEEOVD := 0;
        If l_AvlAmt > 0 Then
            l_FEEOVD := round(least(l_AvlAmt, rec_rls.FEEOVD),0);
            l_AvlAmt := l_AvlAmt - l_FEEOVD;
        End If;
        --16.FEEDUE
        l_FEEDUE := 0;
        If l_AvlAmt > 0 Then
            l_FEEDUE := round(least(l_AvlAmt, rec_rls.FEEDUE),0);
            l_AvlAmt := l_AvlAmt - l_FEEDUE;
        End If;
        --17.FEENML
        l_FEENML := 0;
        If l_AvlAmt > 0 Then
            l_FEENML := round(least(l_AvlAmt, rec_rls.FEENML),0);
        ...
        l_PRINOVD := 0;
        If l_AvlAmt > 0 Then
            l_PRINOVD := round(least(l_AvlAmt, rec_rls.PRINOVD),0);
            l_AvlAmt := l_AvlAmt - l_PRINOVD;
        End If;
        --19.PRINDUE
        l_PRINDUE := 0;
        If l_AvlAmt > 0 Then
           l_PRINDUE := round(least(l_AvlAmt, rec_rls.PRINDUE),0);
        ...
        if rec_rls.PRINNML > 0 then
            If l_AvlAmt > 0 Then
                 l_PRINNML := trunc(least(rec_rls.PRINNML, l_AvlAmt * 1 / (1+rec_rls.ADVPAYFEE/100)));
        ...
        if l_PRINNML > 0 then
            If l_AvlAmt > 0 Then
                 l_ADVPAYFEE := rec_rls.FINANCETYPE * trunc(least(l_AvlAmt, l_PRINNML * rec_rls.ADVPAYFEE / 100 ),0);
        ...
        l_FEEINTOVDACR := 0;
        If l_AvlAmt > 0 Then
             l_FEEINTOVDACR := round(least(l_AvlAmt, rec_rls.FEEINTOVDACR),0);
        ...
        SELECT systemnums.C_BATCH_PREFIXED
                      ...
                      INTO l_txmsg.txnum
                      FROM DUAL;
        l_txmsg.brid        := substr(p_txmsg.txfields(c_afacctno).value,1,4);
        ...
        l_txmsg.txfields ('05').VALUE     := p_txmsg.txfields(c_afacctno).value;
    ...
    */
    ...
    from lnmast ln, cfmast cf where ln.custbank = cf.custid(+) and ln.actype = p_txmsg.txfields(c_lntype).value
    and ln.trfacctno = p_txmsg.txfields(c_afacctno).value and ln.STATUS NOT IN ('P','R','C') AND ln.FTYPE='AF';

    l_exec_MRamount:= to_number(p_txmsg.txfields(c_rtnamt).value);
    ...
    select 'Giai ngan '||decode(l_rrtype,'C',cspks_system.fn_get_sysvar('SYSTEM','COMPANYSHORTNAME'),'B',nvl(l_cfmnemonic,''),'')||'/CL/'||to_char(l_CURRDATE,'DD.MM.RRRR')||'/' || to_char(l_exec_MRamount) || ' VND'
        into l_OrgDesc
        from dual;
    --Set cac field giao dich
    --03   C   ACCTNO
