SET DEFINE OFF;
CREATE OR REPLACE PACKAGE txpks_#2666ex
/**----------------------------------------------------------------------------------------------------
 ** Package: TXPKS_#2666EX
...
CREATE OR REPLACE PACKAGE BODY txpks_#2666ex
IS
   pkgctx   plog.log_ctx;
   ...
   c_intpena          CONSTANT CHAR(2) := '31';
   c_sumpaid          CONSTANT CHAR(2) := '26';
   c_sumint           CONSTANT CHAR(2) := '32';
   c_intpaid          CONSTANT CHAR(2) := '33';
   c_intpena          CONSTANT CHAR(2) := '34';
   c_sumint           CONSTANT CHAR(2) := '35';
   c_intpaid          CONSTANT CHAR(2) := '36';
   c_feeintovdacr     CONSTANT CHAR(2) := '37';
   c_intfeepaid       CONSTANT CHAR(2) := '38';
   c_sump             CONSTANT CHAR(2) := '39';
...
BEGIN
    ...
    ***************************************************************************************************/
    ...
    l_amt:=p_txmsg.txfields('17').value + p_txmsg.txfields('19').value + p_txmsg.txfields('33').value + p_txmsg.txfields('36').value;
    v_groupid:= p_txmsg.txfields('05').value ;

    --- Cap nhap so tien tra goc, phi, lai vao cac bang du lieu
    l_AMTPAID:= p_txmsg.txfields('17').value + p_txmsg.txfields('19').value;
    l_intpaid:= p_txmsg.txfields('33').value;
    l_intpena:= p_txmsg.txfields('31').value;
    l_FEEPAID:= p_txmsg.txfields('36').value;
    l_FEEPENA:= p_txmsg.txfields('34').value;
    l_FEEINTOVDACR:=p_txmsg.txfields('38').value;
    ...
    if l_limitchk = 'Y' then
         UPDATE CIMAST SET DFODAMT = DFODAMT - (ROUND(p_txmsg.txfields('39').value-p_txmsg.txfields('17').value,0)) WHERE ACCTNO=p_txmsg.txfields('03').value;

         INSERT INTO CITRAN(TXNUM,TXDATE,ACCTNO,TXCD,NAMT,CAMT,ACCTREF,DELTD,REF,AUTOID,TLTXCD,BKDATE,TRDESC)
            VALUES (p_txmsg.txnum, TO_DATE (p_txmsg.txdate, systemnums.C_DATE_FORMAT),p_txmsg.txfields ('03').value,'0071',ROUND(p_txmsg.txfields('39').value-p_txmsg.txfields('17').value,0),NULL,p_txmsg.txfields ('03').value,p_txmsg.deltd,p_txmsg.txfields ('03').value,seq_CITRAN.NEXTVAL,p_txmsg.tltxcd,p_txmsg.busdate,'' || '' || '');
