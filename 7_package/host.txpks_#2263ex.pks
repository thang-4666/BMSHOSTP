...
from semast se , afmast af , cfmast cf, sbsecurities sb ,sbsecurities sbwft, SECURITIES_INFO SEINFO,
   (SELECT sum(schd.qtty) qtty ,(schd.afacctno ||symwft.codeid )seacctno
    ...
    AND (INSTR(nvl(schd.pstatus,'A'),'W')= 0 and schd.status not in ('W','C')) GROUP BY schd.afacctno,symwft.codeid) ca
...
v_avgcostprice number default 0;
BEGIN
...
-- locpt 20180604   --> hien tai do dang quan ly gia von rieng cua wft va trade nen luc chuyen phai ghi nhan lai gia von
--                     giam wft va tang trade voi gia von chinh la gia von hien tai cua wft
 ...
 -- giam wft
              secmast_generate(p_txmsg.txnum, p_txmsg.txdate, p_txmsg.busdate, substr(p_txmsg.txfields('03').value,1,10),
                 substr(p_txmsg.txfields('03').value,11), 'D', 'O', NULL, NULL, (ROUND(p_txmsg.txfields('10').value,0)), 0, 'N');
              begin

                    select avgcostprice into v_avgcostprice from buf_se_account_log
                    where   acctno = p_txmsg.txfields('03').value;

               EXCEPTION
                  WHEN NO_DATA_FOUND
                  THEN v_avgcostprice:=0;
              end;
                  -- tang trade
              secmast_generate(p_txmsg.txnum, p_txmsg.txdate, p_txmsg.busdate, substr(p_txmsg.txfields('05').value,1,10),
                 substr(p_txmsg.txfields('05').value,11), 'D', 'I', NULL, NULL, (ROUND(p_txmsg.txfields('10').value,0)), v_avgcostprice, 'N');


     /* --bo theo gap cua bmsc 20180320 --> da tinh o buoc ghi nhan wft
      for rec1 in
        (
        ...
        loop
            ...
            if(V_QTTY > 0 and V_QTTY <= rec1.qtty-rec1.mapqtty) then
                ...
                where autoid = rec1.autoid;
                -----    secmast_generate(p_txmsg.txnum, p_txmsg.txdate, p_txmsg.busdate, PV_AFACCTNO=>?, PV_SYMBOL=>?, PV_SECTYPE=>?, PV_PTYPE=>?, PV_CAMASTID=>?, PV_ORDERID=>?, PV_QTTY=>?, PV_COSTPRICE=>?, PV_MAPAVL=>?);
           --theo gap cua bmsc 20180320 --> da tinh o buoc ghi nhan wft
             secmast_generate(p_txmsg.txnum, p_txmsg.txdate, p_txmsg.busdate, p_txmsg.txfields('02').VALUE,
   ...
   WHERE AFACCTNO =  SUBSTR( v_SEACCTNODR,1,10) AND CODEID = SUBSTR( v_SEACCTNODR,11) AND  ISWFT='Y';
        ...
        INSERT INTO SEOTCTRANLOG (AUTOID,TXNUM,TXDATE,TLTXCD,SEACCTNO,TYPEPON,SHAREHOLDERSID,STATUS,AMOUNT,DELTD,TRADE,BLOCKED)
            SELECT seq_SEOTCTRANLOG.NEXTVAL AUTOID, p_txmsg.txnum TXNUM, TO_DATE(p_txmsg.txdate, systemnums.C_DATE_FORMAT) TXDATE, p_txmsg.tltxcd TLTXCD,
                ...
                v_BLOCKED+p_txmsg.txfields('10').VALUE AMOUNT,'N' DELTD,
                p_txmsg.txfields('10').VALUE TRADE,
                v_BLOCKED BLOCKED
            FROM SEMAST SE
            WHERE ACCTNO=p_txmsg.txfields('05').value AND LENGTH(TRIM(NVL(SHAREHOLDERSID,''))) > 0;
