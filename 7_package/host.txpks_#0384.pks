SET DEFINE OFF;
CREATE OR REPLACE PACKAGE txpks_#0384
/** ----------------------------------------------------------------------------------------------------
 ** Module: TX
 ** Description: Remove customer sub-account from remiser
...
CREATE OR REPLACE PACKAGE BODY txpks_#0384
IS
   pkgctx   plog.log_ctx;
...
BEGIN
   plog.setbeginsection (pkgctx, 'pr_PrintInfo');
    ...
    l_acctno := p_txmsg.txfields('21').value;
    ...
    END IF;
    BEGIN
         SELECT A.FULLNAME CUSTNAME, A.ADDRESS, A.IDCODE LICENSE, A.CUSTODYCD,A.BANKACCTNO,l_acctno
         INTO p_txmsg.txPrintInfo('21').custname,p_txmsg.txPrintInfo('21').address,p_txmsg.txPrintInfo('21').license,p_txmsg.txPrintInfo('21').custody,p_txmsg.txPrintInfo('21').bankac,p_txmsg.txPrintInfo('21').value
         FROM CFMAST A
         WHERE EXISTS (
             SELECT 1 FROM REMAST
...
RETURN  NUMBER IS
   l_allow         boolean;
    ...
    --l_remastcheck_arr txpks_check.remastcheck_arrtype;
BEGIN
...
BEGIN
   IF p_txmsg.deltd <> 'Y' THEN -- Normal transaction
      ...
      INSERT INTO RETRAN(TXNUM,TXDATE,ACCTNO,TXCD,NAMT,CAMT,ACCTREF,DELTD,REF,AUTOID,TLTXCD,BKDATE,TRDESC)
            VALUES (p_txmsg.txnum, TO_DATE (p_txmsg.txdate, systemnums.C_DATE_FORMAT),p_txmsg.txfields ('21').value,'0020',ROUND(p_txmsg.txfields('10').value,0),NULL,'',p_txmsg.deltd,'',seq_RETRAN.NEXTVAL,p_txmsg.tltxcd,p_txmsg.busdate,'' || '' || '');
...
-- Run Pre Update
   IF txpks_#0384EX.fn_txPreAppUpdate(p_txmsg,p_err_code) <> systemnums.C_SUCCESS THEN
       RETURN errnums.C_BIZ_RULE_INVALID;
   END IF;
-- Run Auto Update
   ...
   END IF;
-- Run After Update
   IF txpks_#0384EX.fn_txAftAppUpdate(p_txmsg,p_err_code) <> systemnums.C_SUCCESS THEN
       RETURN errnums.C_BIZ_RULE_INVALID;
   END IF;
...
-- Run Pre check
   IF txpks_#0384EX.fn_txPreAppCheck(p_txmsg,p_err_code) <> systemnums.C_SUCCESS THEN
       RETURN errnums.C_BIZ_RULE_INVALID;
   END IF;
-- Run Auto check
   ...
   END IF;
-- Run After check
   IF txpks_#0384EX.fn_txAftAppCheck(p_txmsg,p_err_code) <> systemnums.C_SUCCESS THEN
       RETURN errnums.C_BIZ_RULE_INVALID;
   END IF;
...
BEGIN
   ...
   LOOP
       ...
       IF txpks_#0384.fn_AutoTxProcess (l_txmsg,
                                        p_err_code,
                                        p_err_param
          ) <> systemnums.c_success
       THEN
           plog.debug (pkgctx,
           'got error 0384: ' || p_err_code
           );
           ROLLBACK;
           plog.setendsection (pkgctx, 'fn_txrevert');
           RETURN errnums.C_SYSTEM_ERROR;
