SET DEFINE OFF;
CREATE OR REPLACE PACKAGE txpks_#1180
/** ----------------------------------------------------------------------------------------------------
 ** Module: TX
...
CREATE OR REPLACE PACKAGE BODY txpks_#1180
IS
   pkgctx   plog.log_ctx;
...
BEGIN
   ...
   IF p_txmsg.deltd = 'N' THEN
     ...
     If txpks_check.fn_aftxmapcheck(p_txmsg.txfields('03').value,'CIMAST','03','1180')<>'TRUE' then
...
BEGIN
       ...
       if not v_blnREVERSAL then
           ...
           --Begin Gen yeu cau sang ngan hang 1180-TRFSEFEE
           v_strOBJTYPE:='T';
...
-- Run Pre Update
   IF txpks_#1180EX.fn_txPreAppUpdate(p_txmsg,p_err_code) <> systemnums.C_SUCCESS THEN
       RETURN errnums.C_BIZ_RULE_INVALID;
   END IF;
-- Run Auto Update
...
-- Run send bank request
   ...
   END IF;
-- Run After Update
   IF txpks_#1180EX.fn_txAftAppUpdate(p_txmsg,p_err_code) <> systemnums.C_SUCCESS THEN
       RETURN errnums.C_BIZ_RULE_INVALID;
   END IF;
...
-- Run Pre check
   IF txpks_#1180EX.fn_txPreAppCheck(p_txmsg,p_err_code) <> systemnums.C_SUCCESS THEN
       RETURN errnums.C_BIZ_RULE_INVALID;
   END IF;
-- Run Auto check
   ...
   END IF;
-- Run After check
   IF txpks_#1180EX.fn_txAftAppCheck(p_txmsg,p_err_code) <> systemnums.C_SUCCESS THEN
       RETURN errnums.C_BIZ_RULE_INVALID;
   END IF;
...
BEGIN
   ...
   LOOP
       ...
       IF txpks_#1180.fn_AutoTxProcess (l_txmsg,
                                        p_err_code,
                                        p_err_param
