SET DEFINE OFF;
CREATE OR REPLACE PACKAGE txpks_#2205ex
/**----------------------------------------------------------------------------------------------------
 ** Package: TXPKS_#2205EX
...
CREATE OR REPLACE PACKAGE BODY txpks_#2205ex
IS
   pkgctx   plog.log_ctx;
   ...
   c_vouchertype      CONSTANT CHAR(2) := '66';
   c_avlcash          CONSTANT CHAR(2) := '89';
   c_qtty             CONSTANT CHAR(2) := '10';
   c_refid            CONSTANT CHAR(2) := '79';
   c_desc             CONSTANT CHAR(2) := '30';
   c_bankname         CONSTANT CHAR(2) := '68';
   c_bankacctno       CONSTANT CHAR(2) := '69';
FUNCTION fn_txPreAppCheck(p_txmsg in tx.msg_rectype,p_err_code out varchar2)
RETURN NUMBER
IS
...
--        IF ( p_txmsg.txfields('13').VALUE > 0 ) THEN
 IF  p_txmsg.txdate <> p_txmsg.BUSDATE  then
            ...
            VALUES (SEQ_SEDEPOBAL.NEXTVAL, p_txmsg.txfields('03').value,v_TBALDT,v_count_days, p_txmsg.txfields('10').value, 'N',to_char(V_txdate)||V_txnum,p_txmsg.txfields('13').VALUE);
*/
             cspks_ciproc.PR_LOGSEDEPOBAL(SUBSTR( p_txmsg.txfields('03').value,11,6),SUBSTR( p_txmsg.txfields('03').value,0,10),
                                           v_count_days,TO_NUMBER(P_TXMSG.TXFIELDS('13').value),
                                     TO_NUMBER(P_TXMSG.TXFIELDS('10').VALUE),v_TBALDT,to_char(V_txdate)||V_txnum);
         END IF;
        --- them mot dong tinh phi luu ky den han
