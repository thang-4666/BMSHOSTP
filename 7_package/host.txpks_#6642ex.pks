SET DEFINE OFF;
CREATE OR REPLACE PACKAGE txpks_#6642ex
/**----------------------------------------------------------------------------------------------------
 ** Package: TXPKS_#6642EX
...
CREATE OR REPLACE PACKAGE BODY txpks_#6642ex
IS
   pkgctx   plog.log_ctx;
...
BEGIN
   plog.setbeginsection (pkgctx, 'fn_genBankRequest');
       ...
       if not v_blnREVERSAL then
           ...
           --Begin Gen yeu cau sang ngan hang 6642-$06
           v_strOBJTYPE:='T';
           ...
           plog.error(pkgctx, 'v_strBANK 6642' || v_strBANK);
           v_strBANKACCT:=p_txmsg.txfields('93').value;
           plog.error(pkgctx, 'v_strBANKACCT 6642' || v_strBANKACCT);
