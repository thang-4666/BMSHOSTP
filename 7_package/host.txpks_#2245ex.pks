...
BEGIN
   ...
   END IF;
    ...
    IF p_txmsg.txfields('02').value IS NOT NULL THEN
        ...
        BEGIN
          SELECT se.status, se.symbol, se.recustodycd, se.trade, se.blocked
          INTO l_status, l_symbol, l_custodycd, l_tradeqtty, l_blockedqtty
          FROM sereceived se WHERE reqid = p_txmsg.txfields('02').value;
       EXCEPTION WHEN OTHERS THEN
          NULL;
       END;

       IF NOT l_status = 'A' THEN
          p_err_code := '-905556';
          plog.setEndSection(pkgctx,'fn_txPreAppCheck');
          RETURN errnums.C_BIZ_RULE_INVALID;
       END IF;

       IF NOT l_custodycd = p_txmsg.txfields('88').value THEN
          p_err_code := '-900005';
          plog.setEndSection(pkgctx,'fn_txPreAppCheck');
          RETURN errnums.C_BIZ_RULE_INVALID;
       END IF;
       ...
       IF NOT NVL(l_tradeqtty,0) = p_txmsg.txfields('10').value THEN
          p_err_code := '-900007';
          plog.setEndSection(pkgctx,'fn_txPreAppCheck');
          RETURN errnums.C_BIZ_RULE_INVALID;
       END IF;

       IF NOT NVL(l_blockedqtty,0) = p_txmsg.txfields('06').value THEN
          p_err_code := '-900007';
          plog.setEndSection(pkgctx,'fn_txPreAppCheck');
          RETURN errnums.C_BIZ_RULE_INVALID;
       END IF;
    ...
    ELSE --- p_txmsg.deltd = 'Y'
     --Kiem tra sl du cho phep xoa hay khong
       ...
       --             IF ( p_txmsg.txfields('15').VALUE > 0 ) THEN
        ...
        -- KRX04: Update trang thai dien
        IF p_txmsg.txfields('02').value IS NOT NULL THEN
           UPDATE sereceived SET status = 'A' WHERE reqid = p_txmsg.txfields('02').value;
        END IF;
        --End KRX04
    end if;
