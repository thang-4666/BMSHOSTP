    ...
    ***************************************************************************************************/
    ...
    --HSX04|iss:103 check market domain
        IF NOT fn_checkdomain(p_txmsg.txfields('03').value, p_txmsg.txfields('01').value,false) THEN
            plog.error('day la log 03 '||p_txmsg.txfields('03').value||' 01:'||p_txmsg.txfields('01').value);
    ...
    if l_blnReversal then
        ...
        plog.error('day la log l_strACTYPE:'||l_strACTYPE);
        if l_count<=0 then
        ...
        if l_count<=0 then
            ...
            If l_strPRICETYPE = 'LO' Or l_strPRICETYPE = 'SL' Then
                ...
                plog.error('day la log l_strCODEID: '||l_strCODEID||' l_dblQUOTEPRICE: '||l_dblQUOTEPRICE||' l_dblQUOTEPRICE:'||l_dblQUOTEPRICE);
                if l_count<=0 then
