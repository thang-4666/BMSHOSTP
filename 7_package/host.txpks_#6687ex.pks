SET DEFINE OFF;
CREATE OR REPLACE PACKAGE txpks_#6687ex
/**----------------------------------------------------------------------------------------------------
 ** Package: TXPKS_#6687EX
...
CREATE OR REPLACE PACKAGE BODY txpks_#6687ex
IS
   pkgctx   plog.log_ctx;
...
BEGIN
    ...
    ***************************************************************************************************/
    ...
    UPDATE CRBTRFLOGDTL SET STATUS='E' WHERE VERSION=l_version AND BANKCODE=l_bankque AND TRFCODE=l_trfcode AND TXDATE=TO_DATE(l_txdate,'DD/MM/RRRR');
    UPDATE CRBTRFLOG SET STATUS='D' WHERE VERSION=l_version AND REFBANK=l_bankque AND TRFCODE=l_trfcode AND TXDATE=TO_DATE(l_txdate,'DD/MM/RRRR');

    UPDATE CRBTXREQ SET STATUS = 'P' WHERE REQID IN (
        SELECT REFREQID FROM CRBTRFLOGDTL
        WHERE VERSION=l_version AND BANKCODE=l_bankque
