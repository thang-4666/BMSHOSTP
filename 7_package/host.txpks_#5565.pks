SET DEFINE OFF;
CREATE OR REPLACE PACKAGE txpks_#5565
/** ----------------------------------------------------------------------------------------------------
 ** Module: TX
 ** Description: Move interest to overdue(auto)
...
CREATE OR REPLACE PACKAGE BODY txpks_#5565
IS
   pkgctx   plog.log_ctx;
...
plog.setbeginsection (pkgctx, 'pr_txlog');
   ...
   INSERT INTO tllog(autoid, txnum, txdate, txtime, brid, tlid,offid, ovrrqs, chid, chkid, tltxcd, ibt, brid2, tlid2, ccyusage,off_line, deltd, brdate, busdate, txdesc, ipaddress,wsname, txstatus, msgsts, ovrsts, batchname, msgamt,msgacct, chktime, offtime, reftxnum)
       ...
       p_txmsg.ovrsts,
       p_txmsg.batchname,
       p_txmsg.txfields('09').value+p_txmsg.txfields('10').value+p_txmsg.txfields('11').value+p_txmsg.txfields('12').value ,
   ...
   plog.debug(pkgctx, 'abt to insert into tllogfld');
   INSERT INTO tllogfld(AUTOID, TXNUM, TXDATE, FLDCD, NVALUE, CVALUE, TXDESC)
      VALUES( seq_tllogfld.NEXTVAL, p_txmsg.txnum, TO_DATE(p_txmsg.txdate, systemnums.C_DATE_FORMAT),'04',0,p_txmsg.txfields('04').value,'CI account number');
   plog.debug(pkgctx, 'abt to insert into tllogfld');
   INSERT INTO tllogfld(AUTOID, TXNUM, TXDATE, FLDCD, NVALUE, CVALUE, TXDESC)
      VALUES( seq_tllogfld.NEXTVAL, p_txmsg.txnum, TO_DATE(p_txmsg.txdate, systemnums.C_DATE_FORMAT),'05',TO_NUMBER(p_txmsg.txfields('05').value),NULL,'Schedule ID');
   plog.debug(pkgctx, 'abt to insert into tllogfld');
   INSERT INTO tllogfld(AUTOID, TXNUM, TXDATE, FLDCD, NVALUE, CVALUE, TXDESC)
      VALUES( seq_tllogfld.NEXTVAL, p_txmsg.txnum, TO_DATE(p_txmsg.txdate, systemnums.C_DATE_FORMAT),'09',TO_NUMBER(p_txmsg.txfields('09').value),NULL,'Amount');
   plog.debug(pkgctx, 'abt to insert into tllogfld');
   INSERT INTO tllogfld(AUTOID, TXNUM, TXDATE, FLDCD, NVALUE, CVALUE, TXDESC)
   ...
   plog.debug(pkgctx, 'abt to insert into tllogfld');
   INSERT INTO tllogfld(AUTOID, TXNUM, TXDATE, FLDCD, NVALUE, CVALUE, TXDESC)
      VALUES( seq_tllogfld.NEXTVAL, p_txmsg.txnum, TO_DATE(p_txmsg.txdate, systemnums.C_DATE_FORMAT),'11',TO_NUMBER(p_txmsg.txfields('11').value),NULL,'Amount');
   plog.debug(pkgctx, 'abt to insert into tllogfld');
   INSERT INTO tllogfld(AUTOID, TXNUM, TXDATE, FLDCD, NVALUE, CVALUE, TXDESC)
      VALUES( seq_tllogfld.NEXTVAL, p_txmsg.txnum, TO_DATE(p_txmsg.txdate, systemnums.C_DATE_FORMAT),'12',TO_NUMBER(p_txmsg.txfields('12').value),NULL,'Amount');
   plog.debug(pkgctx, 'abt to insert into tllogfld');
   INSERT INTO tllogfld(AUTOID, TXNUM, TXDATE, FLDCD, NVALUE, CVALUE, TXDESC)
      VALUES( seq_tllogfld.NEXTVAL, p_txmsg.txnum, TO_DATE(p_txmsg.txdate, systemnums.C_DATE_FORMAT),'06',0,p_txmsg.txfields('06').value,'Status');
   plog.debug(pkgctx, 'abt to insert into tllogfld');
   INSERT INTO tllogfld(AUTOID, TXNUM, TXDATE, FLDCD, NVALUE, CVALUE, TXDESC)
      VALUES( seq_tllogfld.NEXTVAL, p_txmsg.txnum, TO_DATE(p_txmsg.txdate, systemnums.C_DATE_FORMAT),'07',0,p_txmsg.txfields('07').value,'Old status');
   plog.debug(pkgctx, 'abt to insert into tllogfld');
   INSERT INTO tllogfld(AUTOID, TXNUM, TXDATE, FLDCD, NVALUE, CVALUE, TXDESC)
...
RETURN  NUMBER IS
   l_allow         boolean;
    ...
    l_intdue apprules.field%TYPE;
    l_status apprules.field%TYPE;
    l_feedue apprules.field%TYPE;
    l_ointdue apprules.field%TYPE;
...
plog.setbeginsection (pkgctx, 'fn_txAppAutoCheck');
   IF p_txmsg.deltd = 'N' THEN
     ...
     If txpks_check.fn_aftxmapcheck(p_txmsg.txfields('04').value,'CIMAST','04','5565')<>'TRUE' then
         p_err_code := errnums.C_SA_TLTX_NOT_ALLOW_BY_ACCTNO;
         RETURN errnums.C_BIZ_RULE_INVALID;
     End if;

     If txpks_check.fn_aftxmapcheck(p_txmsg.txfields('03').value,'LNMAST','03','5565')<>'TRUE' then
         p_err_code := errnums.C_SA_TLTX_NOT_ALLOW_BY_ACCTNO;
         RETURN errnums.C_BIZ_RULE_INVALID;
     End if;
     ...
     END IF;
       ...
       SELECT  STATUS, INTDUE, OINTDUE, FEEDUE
      INTO l_STATUS,l_INTDUE,l_OINTDUE,l_FEEDUE
        FROM LNMAST
     ...
     IF NOT ( INSTR('NOSBL',l_STATUS) > 0) THEN
        ...
        RETURN errnums.C_BIZ_RULE_INVALID;
     END IF;
     /*IF NOT (to_number(l_INTDUE) >= to_number(p_txmsg.txfields('10').value)) THEN
        p_err_code := '-540005';
        RETURN errnums.C_BIZ_RULE_INVALID;
     END IF;*/
     IF NOT (to_number(l_OINTDUE) >= to_number(p_txmsg.txfields('12').value)) THEN
        p_err_code := '-540012';
        RETURN errnums.C_BIZ_RULE_INVALID;
     END IF;
     IF NOT (to_number(l_FEEDUE) >= to_number(p_txmsg.txfields('11').value)) THEN
        p_err_code := '-540013';
        RETURN errnums.C_BIZ_RULE_INVALID;
     END IF;
...
BEGIN
   IF p_txmsg.deltd <> 'Y' THEN -- Normal transaction
      ...
      INSERT INTO CITRAN(TXNUM,TXDATE,ACCTNO,TXCD,NAMT,CAMT,ACCTREF,DELTD,REF,AUTOID,TLTXCD,BKDATE,TRDESC)
            VALUES (p_txmsg.txnum, TO_DATE (p_txmsg.txdate, systemnums.C_DATE_FORMAT),p_txmsg.txfields ('04').value,'0064',ROUND(p_txmsg.txfields('20').value*(p_txmsg.txfields('10').value+p_txmsg.txfields('11').value+p_txmsg.txfields('12').value+p_txmsg.txfields('09').value),0),NULL,p_txmsg.txfields ('03').value,p_txmsg.deltd,p_txmsg.txfields ('03').value,seq_CITRAN.NEXTVAL,p_txmsg.tltxcd,p_txmsg.busdate,'' || '' || '');

      INSERT INTO CITRAN(TXNUM,TXDATE,ACCTNO,TXCD,NAMT,CAMT,ACCTREF,DELTD,REF,AUTOID,TLTXCD,BKDATE,TRDESC)
            VALUES (p_txmsg.txnum, TO_DATE (p_txmsg.txdate, systemnums.C_DATE_FORMAT),p_txmsg.txfields ('04').value,'0063',ROUND(p_txmsg.txfields('20').value*(p_txmsg.txfields('10').value+p_txmsg.txfields('11').value+p_txmsg.txfields('12').value+p_txmsg.txfields('09').value),0),NULL,p_txmsg.txfields ('03').value,p_txmsg.deltd,p_txmsg.txfields ('03').value,seq_CITRAN.NEXTVAL,p_txmsg.tltxcd,p_txmsg.busdate,'' || '' || '');

      INSERT INTO LNTRAN(TXNUM,TXDATE,ACCTNO,TXCD,NAMT,CAMT,ACCTREF,DELTD,REF,AUTOID,TLTXCD,BKDATE,TRDESC)
            VALUES (p_txmsg.txnum, TO_DATE (p_txmsg.txdate, systemnums.C_DATE_FORMAT),p_txmsg.txfields ('03').value,'0068',ROUND(p_txmsg.txfields('11').value,0),NULL,p_txmsg.txfields ('04').value,p_txmsg.deltd,p_txmsg.txfields ('04').value,seq_LNTRAN.NEXTVAL,p_txmsg.tltxcd,p_txmsg.busdate,'' || '' || '');

      INSERT INTO LNTRAN(TXNUM,TXDATE,ACCTNO,TXCD,NAMT,CAMT,ACCTREF,DELTD,REF,AUTOID,TLTXCD,BKDATE,TRDESC)
            VALUES (p_txmsg.txnum, TO_DATE (p_txmsg.txdate, systemnums.C_DATE_FORMAT),p_txmsg.txfields ('03').value,'0081',ROUND(p_txmsg.txfields('09').value,4),NULL,p_txmsg.txfields ('04').value,p_txmsg.deltd,p_txmsg.txfields ('04').value,seq_LNTRAN.NEXTVAL,p_txmsg.tltxcd,p_txmsg.busdate,'' || '' || '');

      INSERT INTO LNTRAN(TXNUM,TXDATE,ACCTNO,TXCD,NAMT,CAMT,ACCTREF,DELTD,REF,AUTOID,TLTXCD,BKDATE,TRDESC)
            VALUES (p_txmsg.txnum, TO_DATE (p_txmsg.txdate, systemnums.C_DATE_FORMAT),p_txmsg.txfields ('03').value,'0084',ROUND(p_txmsg.txfields('09').value,4),NULL,p_txmsg.txfields ('04').value,p_txmsg.deltd,p_txmsg.txfields ('04').value,seq_LNTRAN.NEXTVAL,p_txmsg.tltxcd,p_txmsg.busdate,'' || '' || '');

      INSERT INTO LNTRAN(TXNUM,TXDATE,ACCTNO,TXCD,NAMT,CAMT,ACCTREF,DELTD,REF,AUTOID,TLTXCD,BKDATE,TRDESC)
            VALUES (p_txmsg.txnum, TO_DATE (p_txmsg.txdate, systemnums.C_DATE_FORMAT),p_txmsg.txfields ('03').value,'0067',ROUND(p_txmsg.txfields('11').value,0),NULL,p_txmsg.txfields ('04').value,p_txmsg.deltd,p_txmsg.txfields ('04').value,seq_LNTRAN.NEXTVAL,p_txmsg.tltxcd,p_txmsg.busdate,'' || '' || '');

      INSERT INTO LNTRAN(TXNUM,TXDATE,ACCTNO,TXCD,NAMT,CAMT,ACCTREF,DELTD,REF,AUTOID,TLTXCD,BKDATE,TRDESC)
            VALUES (p_txmsg.txnum, TO_DATE (p_txmsg.txdate, systemnums.C_DATE_FORMAT),p_txmsg.txfields ('03').value,'0025',ROUND(p_txmsg.txfields('10').value,0),NULL,p_txmsg.txfields ('04').value,p_txmsg.deltd,p_txmsg.txfields ('04').value,seq_LNTRAN.NEXTVAL,p_txmsg.tltxcd,p_txmsg.busdate,'' || '' || '');

      INSERT INTO LNTRAN(TXNUM,TXDATE,ACCTNO,TXCD,NAMT,CAMT,ACCTREF,DELTD,REF,AUTOID,TLTXCD,BKDATE,TRDESC)
            VALUES (p_txmsg.txnum, TO_DATE (p_txmsg.txdate, systemnums.C_DATE_FORMAT),p_txmsg.txfields ('03').value,'0028',ROUND(p_txmsg.txfields('10').value,0),NULL,p_txmsg.txfields ('04').value,p_txmsg.deltd,p_txmsg.txfields ('04').value,seq_LNTRAN.NEXTVAL,p_txmsg.tltxcd,p_txmsg.busdate,'' || '' || '');

      INSERT INTO LNTRAN(TXNUM,TXDATE,ACCTNO,TXCD,NAMT,CAMT,ACCTREF,DELTD,REF,AUTOID,TLTXCD,BKDATE,TRDESC)
            VALUES (p_txmsg.txnum, TO_DATE (p_txmsg.txdate, systemnums.C_DATE_FORMAT),p_txmsg.txfields ('03').value,'0056',ROUND(p_txmsg.txfields('12').value,0),NULL,p_txmsg.txfields ('04').value,p_txmsg.deltd,p_txmsg.txfields ('04').value,seq_LNTRAN.NEXTVAL,p_txmsg.tltxcd,p_txmsg.busdate,'' || '' || '');

      INSERT INTO LNTRAN(TXNUM,TXDATE,ACCTNO,TXCD,NAMT,CAMT,ACCTREF,DELTD,REF,AUTOID,TLTXCD,BKDATE,TRDESC)
            VALUES (p_txmsg.txnum, TO_DATE (p_txmsg.txdate, systemnums.C_DATE_FORMAT),p_txmsg.txfields ('03').value,'0059',ROUND(p_txmsg.txfields('12').value,0),NULL,p_txmsg.txfields ('04').value,p_txmsg.deltd,p_txmsg.txfields ('04').value,seq_LNTRAN.NEXTVAL,p_txmsg.tltxcd,p_txmsg.busdate,'' || '' || '');



      UPDATE CIMAST
         SET
           DUEAMT = DUEAMT - (ROUND(p_txmsg.txfields('20').value*(p_txmsg.txfields('10').value+p_txmsg.txfields('11').value+p_txmsg.txfields('12').value+p_txmsg.txfields('09').value),0)),
           OVAMT = OVAMT + (ROUND(p_txmsg.txfields('20').value*(p_txmsg.txfields('10').value+p_txmsg.txfields('11').value+p_txmsg.txfields('12').value+p_txmsg.txfields('09').value),0)), LAST_CHANGE = SYSTIMESTAMP
        WHERE ACCTNO=p_txmsg.txfields('04').value;


      UPDATE LNMAST
         SET
           INTDUE = INTDUE - (ROUND(p_txmsg.txfields('10').value,0)),
           INTNMLOVD = INTNMLOVD + (ROUND(p_txmsg.txfields('10').value,0)),
           FEEINTNMLOVD = FEEINTNMLOVD + (ROUND(p_txmsg.txfields('09').value,4)),
           FEEDUE = FEEDUE - (ROUND(p_txmsg.txfields('11').value,0)),
           OINTNMLOVD = OINTNMLOVD + (ROUND(p_txmsg.txfields('12').value,0)),
           OINTDUE = OINTDUE - (ROUND(p_txmsg.txfields('12').value,0)),
           FEEINTDUE = FEEINTDUE - (ROUND(p_txmsg.txfields('09').value,4)),
           FEEOVD = FEEOVD + (ROUND(p_txmsg.txfields('11').value,0)), LAST_CHANGE = SYSTIMESTAMP
        WHERE ACCTNO=p_txmsg.txfields('03').value;
...
-- Run Pre Update
   IF txpks_#5565EX.fn_txPreAppUpdate(p_txmsg,p_err_code) <> systemnums.C_SUCCESS THEN
       RETURN errnums.C_BIZ_RULE_INVALID;
   END IF;
-- Run Auto Update
   ...
   END IF;
-- Run After Update
   IF txpks_#5565EX.fn_txAftAppUpdate(p_txmsg,p_err_code) <> systemnums.C_SUCCESS THEN
       RETURN errnums.C_BIZ_RULE_INVALID;
   END IF;
...
-- Run Pre check
   IF txpks_#5565EX.fn_txPreAppCheck(p_txmsg,p_err_code) <> systemnums.C_SUCCESS THEN
       RETURN errnums.C_BIZ_RULE_INVALID;
   END IF;
-- Run Auto check
   ...
   END IF;
-- Run After check
   IF txpks_#5565EX.fn_txAftAppCheck(p_txmsg,p_err_code) <> systemnums.C_SUCCESS THEN
       RETURN errnums.C_BIZ_RULE_INVALID;
   END IF;
...
BEGIN
   ...
   LOOP
       ...
       IF txpks_#5565.fn_AutoTxProcess (l_txmsg,
                                        p_err_code,
                                        p_err_param
