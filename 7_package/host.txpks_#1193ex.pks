SET DEFINE OFF;
CREATE OR REPLACE PACKAGE txpks_#1193ex
/**----------------------------------------------------------------------------------------------------
 ** Package: TXPKS_#1193EX
...
CREATE OR REPLACE PACKAGE BODY txpks_#1193ex
IS
   pkgctx   plog.log_ctx;
...
BEGIN
    ...
    ***************************************************************************************************/
    ...
    IF NOT ( LEAST( to_number(l_BALDEFOVD),TO_NUMBER(l_balance))  >= to_number(p_txmsg.txfields('10').value)) THEN
        p_err_code := '-400110';
...
BEGIN
    ...
    ***************************************************************************************************/
    ...
    UPDATE crintacr_temp SET status ='C' WHERE  afacctno= p_txmsg.txfields(c_acctno).value ;
    ELSE
    UPDATE crintacr_temp SET status ='N' WHERE  afacctno= p_txmsg.txfields(c_acctno).value ;
    END IF ;
