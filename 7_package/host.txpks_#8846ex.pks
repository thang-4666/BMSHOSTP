...
IS
    ...
    l_matchdate DATE;
    l_count number;
    l_exectype odmast.exectype%type;
...
BEGIN
    ...
    ***************************************************************************************************/
    plog.error('day la log 01: '||p_txmsg.txfields('01').value);
...
/*    -- REVERT AFMAST
    ...
    --HSX04
    IF l_matchdate = l_currdate THEN
      execorder_update_aft (L_orderid, p_txmsg.txfields('12').value,p_txmsg.txfields('14').value/p_txmsg.txfields('12').value,'0','','',p_err_code,'1');
     else
       if ( l_exectype = 'NB') then
         update semast se set odreceiving =0 where se.acctno = l_seacctno;
           ...
           USING (SELECT sum(st.QTTY - od.NETEXECQTTY - od.CFNETEXECQTTY - ST.AQTTY) QTTY , ACCTNO
                   ...
                   WHERE st.DUETYPE='RS' and st.orgorderid =  od.orderid
                       and st.deltd = 'N' and st.status = 'N' and od.txdate < l_currdate
                       and st.QTTY - od.NETEXECQTTY - od.CFNETEXECQTTY - st.aqtty > 0
                       and od.seacctno  = l_seacctno
                           group by ACCTNO) st
    ...
    plog.error('day la log 02: '||L_orderid);
    -- REVERT STSCHD
    UPDATE STSCHD SET
        DELTD = 'Y'
    WHERE ORGORDERID = L_orderid;
