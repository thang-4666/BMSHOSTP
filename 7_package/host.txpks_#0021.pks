SET DEFINE OFF;
CREATE OR REPLACE PACKAGE txpks_#0021
/** ----------------------------------------------------------------------------------------------------
 ** Module: TX
 ** Description: Change CFTYPE
...
CREATE OR REPLACE PACKAGE BODY txpks_#0021
IS
   pkgctx   plog.log_ctx;
...
BEGIN
   IF p_txmsg.deltd <> 'Y' THEN -- Normal transaction
      ...
      UPDATE CFMAST
         SET 
           ACTYPE = p_txmsg.txfields('46').value, LAST_CHANGE = SYSTIMESTAMP
...
-- Run Pre Update
   IF txpks_#0021EX.fn_txPreAppUpdate(p_txmsg,p_err_code) <> systemnums.C_SUCCESS THEN
...
-- Run Auto Update
   ...
   END IF;
-- Run After Update
   IF txpks_#0021EX.fn_txAftAppUpdate(p_txmsg,p_err_code) <> systemnums.C_SUCCESS THEN
       RETURN errnums.C_BIZ_RULE_INVALID;
   END IF;
...
-- Run Pre check
   IF txpks_#0021EX.fn_txPreAppCheck(p_txmsg,p_err_code) <> systemnums.C_SUCCESS THEN
       RETURN errnums.C_BIZ_RULE_INVALID;
   END IF;
-- Run Auto check
   ...
   END IF;
-- Run After check
   IF txpks_#0021EX.fn_txAftAppCheck(p_txmsg,p_err_code) <> systemnums.C_SUCCESS THEN
       RETURN errnums.C_BIZ_RULE_INVALID;
   END IF;
