SET DEFINE OFF;
CREATE OR REPLACE PACKAGE txpks_#8894
/** ----------------------------------------------------------------------------------------------------
 ** Module: TX
 ** Description: Match cash trade lot retail
...
CREATE OR REPLACE PACKAGE BODY txpks_#8894
IS
   pkgctx   plog.log_ctx;
...
plog.setbeginsection (pkgctx, 'pr_txlog');
   ...
   if v_count=0 then
      INSERT INTO tllog(autoid, txnum, txdate, txtime, brid, tlid,offid, ovrrqs, chid, chkid, tltxcd, ibt, brid2, tlid2, ccyusage,off_line, deltd, brdate, busdate, txdesc, ipaddress,wsname, txstatus, msgsts, ovrsts, batchname, msgamt,msgacct, chktime, offtime, reftxnum)
          ...
          p_txmsg.ovrsts,
          p_txmsg.batchname,
          p_txmsg.txfields('10').value*p_txmsg.txfields('11').value ,
       ...
       plog.debug(pkgctx, 'abt to insert into tllogfld');
       INSERT INTO tllogfld(AUTOID, TXNUM, TXDATE, FLDCD, NVALUE, CVALUE, TXDESC)
          VALUES( seq_tllogfld.NEXTVAL, p_txmsg.txnum, TO_DATE(p_txmsg.txdate, systemnums.C_DATE_FORMAT),'07',0,p_txmsg.txfields('07').value,'Contract number buy');
       plog.debug(pkgctx, 'abt to insert into tllogfld');
       INSERT INTO tllogfld(AUTOID, TXNUM, TXDATE, FLDCD, NVALUE, CVALUE, TXDESC)
       ...
       plog.debug(pkgctx, 'abt to insert into tllogfld');
       INSERT INTO tllogfld(AUTOID, TXNUM, TXDATE, FLDCD, NVALUE, CVALUE, TXDESC)
          VALUES( seq_tllogfld.NEXTVAL, p_txmsg.txnum, TO_DATE(p_txmsg.txdate, systemnums.C_DATE_FORMAT),'04',0,p_txmsg.txfields('04').value,'Trade date');
       plog.debug(pkgctx, 'abt to insert into tllogfld');
       INSERT INTO tllogfld(AUTOID, TXNUM, TXDATE, FLDCD, NVALUE, CVALUE, TXDESC)
          VALUES( seq_tllogfld.NEXTVAL, p_txmsg.txnum, TO_DATE(p_txmsg.txdate, systemnums.C_DATE_FORMAT),'06',0,p_txmsg.txfields('06').value,'Ref ID');
       plog.debug(pkgctx, 'abt to insert into tllogfld');
       INSERT INTO tllogfld(AUTOID, TXNUM, TXDATE, FLDCD, NVALUE, CVALUE, TXDESC)
          VALUES( seq_tllogfld.NEXTVAL, p_txmsg.txnum, TO_DATE(p_txmsg.txdate, systemnums.C_DATE_FORMAT),'05',0,p_txmsg.txfields('05').value,'Transaction No');
       plog.debug(pkgctx, 'abt to insert into tllogfld');
       INSERT INTO tllogfld(AUTOID, TXNUM, TXDATE, FLDCD, NVALUE, CVALUE, TXDESC)
       ...
       plog.debug(pkgctx, 'abt to insert into tllogfld');
       INSERT INTO tllogfld(AUTOID, TXNUM, TXDATE, FLDCD, NVALUE, CVALUE, TXDESC)
          VALUES( seq_tllogfld.NEXTVAL, p_txmsg.txnum, TO_DATE(p_txmsg.txdate, systemnums.C_DATE_FORMAT),'16',0,p_txmsg.txfields('16').value,'Description');
...
BEGIN
   plog.setbeginsection (pkgctx, 'pr_PrintInfo');
    ...
    l_acctno := p_txmsg.txfields('07').value;
    ...
    BEGIN
         ...
         INTO p_txmsg.txPrintInfo('07').custname,p_txmsg.txPrintInfo('07').address,p_txmsg.txPrintInfo('07').license,p_txmsg.txPrintInfo('07').custody
         FROM CFMAST A
         WHERE EXISTS (
...
BEGIN
   ...
   IF p_txmsg.deltd = 'N' THEN
     ...
     If txpks_check.fn_aftxmapcheck(p_txmsg.txfields('07').value,'CIMAST','07','8894')<>'TRUE' then
         p_err_code := errnums.C_SA_TLTX_NOT_ALLOW_BY_ACCTNO;
         plog.setendsection (pkgctx, 'fn_txAppAutoCheck');
         RETURN errnums.C_BIZ_RULE_INVALID;
     End if;

     If txpks_check.fn_aftxmapcheck(p_txmsg.txfields('02').value,'CIMAST','02','8894')<>'TRUE' then
         p_err_code := errnums.C_SA_TLTX_NOT_ALLOW_BY_ACCTNO;
         plog.setendsection (pkgctx, 'fn_txAppAutoCheck');
         RETURN errnums.C_BIZ_RULE_INVALID;
     End if;
     ...
     IF NOT (to_number(l_BALDEFOVD) >= to_number(p_txmsg.txfields('14').value+p_txmsg.txfields('15').value+p_txmsg.txfields('22').value-p_txmsg.txfields('10').value*p_txmsg.txfields('11').value)) THEN
        p_err_code := '-400110';
plog.setendsection (pkgctx, 'fn_txAppAutoCheck');
        RETURN errnums.C_BIZ_RULE_INVALID;
     END IF;
     ...
     l_CIMASTcheck_arr := txpks_check.fn_CIMASTcheck(p_txmsg.txfields('07').value,'CIMAST','ACCTNO');

     l_BALDEFOVD := l_CIMASTcheck_arr(0).BALDEFOVD;

     IF NOT (to_number(l_BALDEFOVD) >= to_number(p_txmsg.txfields('10').value*p_txmsg.txfields('11').value)) THEN
        p_err_code := '-400110';
plog.setendsection (pkgctx, 'fn_txAppAutoCheck');
        RETURN errnums.C_BIZ_RULE_INVALID;
     END IF;
...
BEGIN
   IF p_txmsg.deltd <> 'Y' THEN -- Normal transaction
      ...
      l_txdesc:= cspks_system.fn_DBgen_trandesc_with_format(p_txmsg,'8894','CI','0011','0002');
      INSERT INTO CITRAN(TXNUM,TXDATE,ACCTNO,TXCD,NAMT,CAMT,ACCTREF,DELTD,REF,AUTOID,TLTXCD,BKDATE,TRDESC)
            VALUES (p_txmsg.txnum, TO_DATE (p_txmsg.txdate, systemnums.C_DATE_FORMAT),p_txmsg.txfields ('02').value,'0011',ROUND(p_txmsg.txfields('15').value*p_txmsg.txfields('60').value,0),NULL,p_txmsg.txfields ('06').value,p_txmsg.deltd,p_txmsg.txfields ('06').value,seq_CITRAN.NEXTVAL,p_txmsg.tltxcd,p_txmsg.busdate,'' || l_txdesc || '');

      l_txdesc:= cspks_system.fn_DBgen_trandesc_with_format(p_txmsg,'8894','CI','0011','0001');
      INSERT INTO CITRAN(TXNUM,TXDATE,ACCTNO,TXCD,NAMT,CAMT,ACCTREF,DELTD,REF,AUTOID,TLTXCD,BKDATE,TRDESC)
            VALUES (p_txmsg.txnum, TO_DATE (p_txmsg.txdate, systemnums.C_DATE_FORMAT),p_txmsg.txfields ('02').value,'0011',ROUND(p_txmsg.txfields('14').value*p_txmsg.txfields('60').value,0),NULL,p_txmsg.txfields ('06').value,p_txmsg.deltd,p_txmsg.txfields ('06').value,seq_CITRAN.NEXTVAL,p_txmsg.tltxcd,p_txmsg.busdate,'' || l_txdesc || '');

      l_txdesc:= cspks_system.fn_DBgen_trandesc_with_format(p_txmsg,'8894','CI','0011','0003');
      INSERT INTO CITRAN(TXNUM,TXDATE,ACCTNO,TXCD,NAMT,CAMT,ACCTREF,DELTD,REF,AUTOID,TLTXCD,BKDATE,TRDESC)
            VALUES (p_txmsg.txnum, TO_DATE (p_txmsg.txdate, systemnums.C_DATE_FORMAT),p_txmsg.txfields ('02').value,'0011',ROUND(p_txmsg.txfields('22').value*p_txmsg.txfields('60').value,0),NULL,p_txmsg.txfields ('06').value,p_txmsg.deltd,p_txmsg.txfields ('06').value,seq_CITRAN.NEXTVAL,p_txmsg.tltxcd,p_txmsg.busdate,'' || l_txdesc || '');

      l_txdesc:= cspks_system.fn_DBgen_trandesc_with_format(p_txmsg,'8894','CI','0011','0004');
      INSERT INTO CITRAN(TXNUM,TXDATE,ACCTNO,TXCD,NAMT,CAMT,ACCTREF,DELTD,REF,AUTOID,TLTXCD,BKDATE,TRDESC)
            VALUES (p_txmsg.txnum, TO_DATE (p_txmsg.txdate, systemnums.C_DATE_FORMAT),p_txmsg.txfields ('02').value,'0011',ROUND(p_txmsg.txfields('19').value*p_txmsg.txfields('60').value,0),NULL,p_txmsg.txfields ('06').value,p_txmsg.deltd,p_txmsg.txfields ('06').value,seq_CITRAN.NEXTVAL,p_txmsg.tltxcd,p_txmsg.busdate,'' || l_txdesc || '');

      INSERT INTO CITRAN(TXNUM,TXDATE,ACCTNO,TXCD,NAMT,CAMT,ACCTREF,DELTD,REF,AUTOID,TLTXCD,BKDATE,TRDESC)
            VALUES (p_txmsg.txnum, TO_DATE (p_txmsg.txdate, systemnums.C_DATE_FORMAT),p_txmsg.txfields ('02').value,'0029',ROUND((p_txmsg.txfields('10').value*p_txmsg.txfields('11').value)*p_txmsg.txfields('60').value,0),NULL,p_txmsg.txfields ('06').value,p_txmsg.deltd,p_txmsg.txfields ('06').value,seq_CITRAN.NEXTVAL,p_txmsg.tltxcd,p_txmsg.busdate,'' || '' || '');

      INSERT INTO CITRAN(TXNUM,TXDATE,ACCTNO,TXCD,NAMT,CAMT,ACCTREF,DELTD,REF,AUTOID,TLTXCD,BKDATE,TRDESC)
            VALUES (p_txmsg.txnum, TO_DATE (p_txmsg.txdate, systemnums.C_DATE_FORMAT),p_txmsg.txfields ('02').value,'0045',ROUND((p_txmsg.txfields('10').value*p_txmsg.txfields('11').value)*p_txmsg.txfields('60').value,0),NULL,p_txmsg.txfields ('06').value,p_txmsg.deltd,p_txmsg.txfields ('06').value,seq_CITRAN.NEXTVAL,p_txmsg.tltxcd,p_txmsg.busdate,'' || '' || '');

      INSERT INTO CITRAN(TXNUM,TXDATE,ACCTNO,TXCD,NAMT,CAMT,ACCTREF,DELTD,REF,AUTOID,TLTXCD,BKDATE,TRDESC)
            VALUES (p_txmsg.txnum, TO_DATE (p_txmsg.txdate, systemnums.C_DATE_FORMAT),p_txmsg.txfields ('07').value,'0028',ROUND(p_txmsg.txfields('10').value*p_txmsg.txfields('11').value,0),NULL,p_txmsg.txfields ('06').value,p_txmsg.deltd,p_txmsg.txfields ('06').value,seq_CITRAN.NEXTVAL,p_txmsg.tltxcd,p_txmsg.busdate,'' || '' || '');



      UPDATE CIMAST
         SET
           BALANCE = BALANCE - (ROUND(p_txmsg.txfields('14').value*p_txmsg.txfields('60').value,0)) - (ROUND(p_txmsg.txfields('15').value*p_txmsg.txfields('60').value,0)) - (ROUND(p_txmsg.txfields('22').value*p_txmsg.txfields('60').value,0)) + (ROUND((p_txmsg.txfields('10').value*p_txmsg.txfields('11').value)*p_txmsg.txfields('60').value,0)) - (ROUND(p_txmsg.txfields('19').value*p_txmsg.txfields('60').value,0)),
           LASTDATE = to_date(TO_DATE(p_txmsg.txdate, systemnums.C_DATE_FORMAT), 'DD/MM/RRRR'),
           RECEIVING = RECEIVING - (ROUND((p_txmsg.txfields('10').value*p_txmsg.txfields('11').value)*p_txmsg.txfields('60').value,0)), LAST_CHANGE = SYSTIMESTAMP
        WHERE ACCTNO=p_txmsg.txfields('02').value;


      UPDATE CIMAST
         SET
           BALANCE = BALANCE - (ROUND(p_txmsg.txfields('10').value*p_txmsg.txfields('11').value,0)),
           LASTDATE = to_date(TO_DATE(p_txmsg.txdate, systemnums.C_DATE_FORMAT), 'DD/MM/RRRR'), LAST_CHANGE = SYSTIMESTAMP
        WHERE ACCTNO=p_txmsg.txfields('07').value;
...
-- Run Pre Update
   IF txpks_#8894EX.fn_txPreAppUpdate(p_txmsg,p_err_code) <> systemnums.C_SUCCESS THEN
       RETURN errnums.C_BIZ_RULE_INVALID;
   END IF;
-- Run Auto Update
   ...
   END IF;
-- Run After Update
   IF txpks_#8894EX.fn_txAftAppUpdate(p_txmsg,p_err_code) <> systemnums.C_SUCCESS THEN
       RETURN errnums.C_BIZ_RULE_INVALID;
   END IF;
...
-- Run Pre check
   IF txpks_#8894EX.fn_txPreAppCheck(p_txmsg,p_err_code) <> systemnums.C_SUCCESS THEN
       RETURN errnums.C_BIZ_RULE_INVALID;
   END IF;
-- Run Auto check
   ...
   END IF;
-- Run After check
   IF txpks_#8894EX.fn_txAftAppCheck(p_txmsg,p_err_code) <> systemnums.C_SUCCESS THEN
       RETURN errnums.C_BIZ_RULE_INVALID;
   END IF;
...
BEGIN
   ...
   LOOP
       ...
       IF txpks_#8894.fn_AutoTxProcess (l_txmsg,
                                        p_err_code,
                                        p_err_param
          ) <> systemnums.c_success
       THEN
           plog.debug (pkgctx,
           'got error 8894: ' || p_err_code
           );
           ROLLBACK;
           plog.setendsection (pkgctx, 'fn_txrevert');
           RETURN errnums.C_SYSTEM_ERROR;
