...
PROCEDURE pr_AllocationAdvancedPayment(p_afacctno varchar2, p_err_code out varchar2);
END;
/
...
BEGIN
    ...
    ***************************************************************************************************/
    ...
    --HSX04 them cot 12 
    v_dblAMT:=round(p_txmsg.txfields('10').value,0) + round(p_txmsg.txfields('11').value,0) + round(p_txmsg.txfields('12').value,0);
    begin
        SELECT TO_CHAR(CLEARDT,'DD/MM/YYYY') TXDATE
        into v_strSTSDATE
        FROM ADSCHD WHERE AUTOID=p_txmsg.txfields('09').value;
    ...
    if not v_blnREVERSAL then
        UPDATE ADSCHD
            SET PAIDAMT=round(PAIDAMT+v_dblAMT,0), STATUS='C', PAIDDATE=TO_DATE(p_txmsg.txdate, systemnums.C_DATE_FORMAT),
                OVERFEEAMT=OVERFEEAMT+round(p_txmsg.txfields('12').value,0) --HSX04
        WHERE AUTOID=p_txmsg.txfields('09').value;
        for rec in
        (
            SELECT AUTOID,STS.AFACCTNO,STS.AAMT-STS.PAIDAMT AMT,STS.FAMT-STS.PAIDFEEAMT FAMT
...
BEGIN
    ...
    ***************************************************************************************************/
    ...
    -- HSX04: Cap nhat GTUT den thoi diem hien tai
    BEGIN
        SELECT SUM(amt + feeamt) INTO v_sblTOTALCURAMT
        FROM adschd
        WHERE acctno = p_txmsg.txfields('03').value AND deltd <> 'Y'
        AND status <> 'C'
    ...
    END;
        ...
        if v_dblADAMT< v_limit_adv then
           ...
           INSERT INTO adschd (autoid, ismortage, acctno, txdate, txnum, refadno, cleardt,
                        amt, feeamt, vatamt, bankfee, paidamt, adtype, rrtype, ciacctno, custbank,oddate, totalcuramt)
            SELECT  seq_adschd.nextval autoid,
                    0 ismortage,
                    p_txmsg.txfields('03').value acctno,
                    To_date(p_txmsg.txdate,systemnums.c_date_format) txdate,
     ...
     ELSE
        ...
        -- HSX04: Ghi giam tong ung o cac lan ung sau
        SELECT totalcuramt INTO v_sblTOTALCURAMT FROM adschd
        WHERE txnum = p_txmsg.txnum  and txdate = p_txmsg.txdate;

        UPDATE adschd SET totalcuramt = totalcuramt - v_dblADAMT
        WHERE totalcuramt > v_sblTOTALCURAMT
        AND txdate = p_txmsg.txdate AND deltd <> 'Y' AND status <> 'C'
        AND cleardt = to_date(p_txmsg.txfields('08').value, systemnums.c_date_format)
        AND oddate = TO_DATE(p_txmsg.txfields('42').value, systemnums.c_date_format)
...
IS
   ...
   v_isvat           NUMBER;
   v_dbladamt        NUMBER(20,4);
   v_dbltemp         NUMBER(20,4);
BEGIN
   plog.setbeginsection (pkgctx, 'pr_AllocationAdvancedPayment');
   plog.debug (pkgctx, '<<BEGIN OF pr_AllocationAdvancedPayment');
   /***************************************************************************************************
   ** PUT YOUR SPECIFIC AFTER PROCESS HERE. DO NOT COMMIT/ROLLBACK HERE, THE SYSTEM WILL DO IT
   ***************************************************************************************************/
   SELECT to_date(varvalue,'DD/MM/YYYY') INTO v_dtcurrdate
   FROM sysvar
   WHERE grname = 'SYSTEM' and varname = 'CURRDATE';
   -- Phan bo lai gia tri UT vao cac bang
   UPDATE odmapext SET aamt = 0 WHERE orderid IN (
      SELECT orgorderid FROM stschd WHERE txdate = v_dtcurrdate AND (p_afacctno = 'ALL' OR afacctno = p_afacctno)
      AND deltd <> 'Y' AND status = 'N' AND duetype = 'RM'
   );
   DELETE adschddtl WHERE orderid IN (
      SELECT orgorderid FROM stschd WHERE txdate = v_dtcurrdate AND (p_afacctno = 'ALL' OR afacctno = p_afacctno)
      AND deltd <> 'Y' AND status = 'N' AND duetype = 'RM'
   );
   UPDATE stschd SET aamt = 0 WHERE txdate = v_dtcurrdate AND (p_afacctno = 'ALL' OR afacctno = p_afacctno)
   AND deltd <> 'Y' AND status = 'N' AND duetype = 'RM';

   FOR I IN
   (  SELECT * FROM adschd WHERE txdate = v_dtcurrdate AND oddate = v_dtcurrdate AND deltd <> 'Y'
      AND (p_afacctno = 'ALL' OR acctno = p_afacctno)
      ORDER BY totalcuramt
   )
   LOOP
      SELECT(CASE WHEN cf.vat = 'Y' THEN 1 ELSE 0 END) INTO v_isvat
      FROM afmast af, aftype aft, cfmast cf
      WHERE cf.custid=af.custid and af.actype=aft.actype AND af.acctno = I.acctno;
      -- Phan bo UT vao stschd
      v_dbladamt := I.amt + i.feeamt;
      FOR rec IN
      (  SELECT autoid, sts.amt execamt, od.afacctno, sts.cleardate, od.orderid,
                (sts.amt - od.netexecamt) - sts.aamt - sts.famt + sts.paidamt + sts.paidfeeamt
                - case when od.feeacr<=0 then round(odt.deffeerate*(sts.amt - od.netexecamt)/100) else od.feeacr - ceil(od.netexecamt*od.feeacr/od.execamt) end
                - v_isvat* (case when od.taxsellamt > 0 then od.taxsellamt else round(to_number(sys.varvalue)*(sts.amt - od.netexecamt)/100) end + sts.aright) amt,
                sts.famt famt
         FROM stschd sts, odmast od, sbsecurities sec, odtype odt, sysvar sys, odmapext odm
         WHERE sts.codeid=sec.codeid AND sts.orgorderid = od.orderid AND sts.deltd <> 'Y' and sts.status = 'N' AND sts.duetype = 'RM'
           AND od.actype = odt.actype
           AND od.errod ='N'
           AND od.orderid = odm.orderid (+)
           AND nvl(odm.isvsd,'N') = decode(I.isvsd,0,'N','Y')
           AND sys.varname = 'ADVSELLDUTY' AND sys.grname = 'SYSTEM'
           AND sts.afacctno = I.acctno
           AND sts.cleardate = I.cleardt
           AND sts.txdate = I.oddate
           AND sts.amt - od.netexecamt > 0
         ORDER BY execamt DESC
      ) LOOP
         IF round(rec.amt,0) <= 0 THEN
            continue;
         END IF;

         v_dbltemp := least(round(rec.amt,0), v_dbladamt);
         -- log thong tin lenh ung
         INSERT INTO adschddtl(autoid, acctno, txdate, txnum, cleardate, orderid, aamt, feeamt, vatamt, deltd, status)
         VALUES(seq_adschddtl.nextval, rec.afacctno, I.txdate, I.txnum, rec.cleardate, rec.orderid, v_dbltemp, 0, 0, 'N', 'A');

         UPDATE stschd SET aamt = aamt + v_dbltemp WHERE autoid = rec.autoid;

         --- hailt them de cap nhap so tien da ung truoc vao odmapext doi voi ung truoc vsd (cap nhap vao dong dau tien lay dc)
         IF I.isvsd <> 0 THEN
            FOR rec1 IN (SELECT * FROM odmapext WHERE orderid = rec.orderid AND isvsd <> 'N' AND deltd <> 'Y' AND rownum = 1)
            LOOP
               UPDATE odmapext SET aamt = aamt + v_dbltemp
               WHERE orderid = rec1.orderid and refid = rec1.refid;
            END LOOP;
         END IF;

         v_dbladamt := v_dbladamt - v_dbltemp;
         EXIT WHEN v_dbladamt <= 0;
      END LOOP;
      ...
      IF v_dbladamt <= 0 THEN
         continue;
      END IF;
      -- Phan bo UT thua vao stschd
      FOR rec IN
      (  SELECT autoid, sts.amt execamt, od.afacctno, sts.cleardate, od.orderid,
                sts.amt - sts.aamt - sts.famt + sts.paidamt + sts.paidfeeamt
                - case when od.feeacr<=0 then round(odt.deffeerate*sts.amt/100) else od.feeacr end
                - v_isvat* (case when od.taxsellamt > 0 then od.taxsellamt else round(to_number(sys.varvalue)*sts.amt/100) end + sts.aright) amt,
                sts.famt famt
         FROM stschd sts, odmast od, sbsecurities sec, odtype odt, sysvar sys, odmapext odm
         WHERE sts.codeid=sec.codeid AND sts.orgorderid = od.orderid AND sts.deltd <> 'Y' and sts.status = 'N' AND sts.duetype = 'RM'
           AND od.actype = odt.actype
   ...
   plog.debug (pkgctx, '<<END OF pr_AllocationAdvancedPayment');
   plog.setendsection (pkgctx, 'pr_AllocationAdvancedPayment');
   p_err_code := systemnums.C_SUCCESS;
EXCEPTION WHEN OTHERS THEN
   p_err_code := errnums.C_SYSTEM_ERROR;
   plog.error (pkgctx, SQLERRM || dbms_utility.format_error_backtrace);
   RAISE errnums.E_SYSTEM_ERROR;
END pr_AllocationAdvancedPayment;
