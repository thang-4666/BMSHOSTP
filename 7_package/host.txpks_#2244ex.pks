...
BEGIN
   ...
   End if;*/
        ...
        /*IF l_count = 0 THEN
            ...
            return errnums.C_SYSTEM_ERROR;
        END IF;*/
    l_autoid_sesend := p_txmsg.txfields('98').value;
   if(l_autoid_sesend > 0  )then
    select count(*) into l_Count from  sesend_req where  status ='P' and  autoid = l_autoid_sesend;
    IF l_Count =0  THEN
       p_err_code:='-100018';
       plog.setendsection (pkgctx, 'fn_txPreAppCheck');
      RETURN errnums.C_BIZ_RULE_INVALID;
...
BEGIN
    ...
    ***************************************************************************************************/
    ...
    Else
        ...
        -- HSX04: cap nhat cho truong hop voi luong xu ly dien nhan yeu cau chuyen
          l_autoid_sesend := p_txmsg.txfields('98').value;
          if(l_autoid_sesend > 0  )then
            update sesend_req set status ='C' where autoid = l_autoid_sesend;
          end if;

          INSERT INTO SESENDOUT
          (AUTOID, TXNUM, TXDATE, ACCTNO, TRADE,
          BLOCKED,CAQTTY,STRADE,SBLOCKED,SCAQTTY,CTRADE,CBLOCKED,CCAQTTY,DELTD,STATUS,RECUSTODYCD,REAFACCTNO,RECUSTNAME,codeid,PRICE,OUTWARD,TRTYPE,QTTYTYPE,FEE,TAX,ISTRFCA,GTCG, vsdmessagetype,reidcode,reidate,reidplace,TRFTXNUM,autoidreq )
          VALUES (v_sendoutid, p_txmsg.txnum,p_txmsg.txdate,p_txmsg.txfields('03').value,
          p_txmsg.txfields('10').value, p_txmsg.txfields('06').value,p_txmsg.txfields('13').value,
          0,0,0,0,0,0,'N','N',p_txmsg.txfields('88').value, p_txmsg.txfields('04').value, p_txmsg.txfields('49').value,p_txmsg.txfields('01').value,p_txmsg.txfields('09').value,p_txmsg.txfields('05').value,
          p_txmsg.txfields('31').value,p_txmsg.txfields('14').value,p_txmsg.txfields('45').value,p_txmsg.txfields('46').value,'N',p_txmsg.txfields('60').value, p_txmsg.txfields('97').value,p_txmsg.txfields('50').value,p_txmsg.txfields('51').value,p_txmsg.txfields('52').value,p_txmsg.txfields('96').value, l_autoid_sesend);
