    ...
    ***************************************************************************************************/
    ...
    if p_txmsg.deltd <> 'Y' THEN
      ...
      --29/01/2018 DieuNDA: Check chan nhap so am
        if to_number(p_txmsg.txfields('10').value) <= 0 or to_number(p_txmsg.txfields('11').value) < 0 or to_number(p_txmsg.txfields('12').value) < 0 then
            p_err_code := '-100810';
            plog.error (pkgctx, p_err_code || ': Acctno='||p_txmsg.txfields('03').value
                                    ||', Amount='||p_txmsg.txfields('10').value
                                    ||', Fee='||p_txmsg.txfields('11').value
                                    ||', VAT='||p_txmsg.txfields('12').value);
            plog.setendsection (pkgctx, 'fn_txPreAppCheck');
            RETURN errnums.C_BIZ_RULE_INVALID;
        end if;
