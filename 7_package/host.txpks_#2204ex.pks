SET DEFINE OFF;
CREATE OR REPLACE PACKAGE TXPKS_#2204EX
/**----------------------------------------------------------------------------------------------------
 ** Package: TXPKS_#2204EX
...
CREATE OR REPLACE PACKAGE BODY TXPKS_#2204EX
IS
   pkgctx   plog.log_ctx;
       ...
       for rec in (
        ...
        loop
            if v_caqtty > rec.qtty - rec.mapqtty then
                ...
                update sepitlog set qtty = 0 where autoid = rec.autoid;


            else

                update sepitlog set qtty = qtty - v_caqtty where autoid = rec.autoid;
