...
BEGIN
    ...
    ***************************************************************************************************/
    ...
    IF p_txmsg.DELTD <> 'Y' THEN
        INSERT INTO SEOTCTRANLOG (AUTOID,TXNUM,TXDATE,TLTXCD,OLDSEACCTNO,TYPEPON,OLDSHAREHOLDERSID,STATUS,AMOUNT,DELTD,TRADE,BLOCKED)
        VALUES(seq_SEOTCTRANLOG.NEXTVAL,p_txmsg.txnum,TO_DATE(p_txmsg.txdate, systemnums.C_DATE_FORMAT), p_txmsg.tltxcd,p_txmsg.txfields('05').value,p_txmsg.txfields('08').value,
            p_txmsg.txfields('22').value,'A',p_txmsg.txfields('12').value,'N',p_txmsg.txfields('10').value,p_txmsg.txfields('06').value);

        /*update semast
        set status = 'C'
        where acctno=p_txmsg.txfields('05').value
            and p_txmsg.txfields('08').value='OL' and status = 'A';*/
    ...
    ELSE
       ...
       /*update semast
        set status = 'A'
        where acctno=p_txmsg.txfields('05').value
            and p_txmsg.txfields('08').value='OL' and status = 'C';*/
