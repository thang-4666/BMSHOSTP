SET DEFINE OFF;
CREATE OR REPLACE PACKAGE txpks_#2625ex
/**----------------------------------------------------------------------------------------------------
 ** Package: TXPKS_#2625EX
...
CREATE OR REPLACE PACKAGE BODY txpks_#2625ex
IS
   pkgctx   plog.log_ctx;
...
BEGIN
    ...
    * END IF;
    ***************************************************************************************************/
    SELECT ABS(DF.RELEVSDQTTY) INTO v_dfqtty
    from dfmast df
...
/*    select amt into v_dfqtty from dfgroup where groupid = p_txmsg.txfields('05').value;

    if not v_dfqtty > 0 then
       p_err_code := '-260040';
       plog.setendsection (pkgctx, 'fn_txAftAppUpdate');
       RETURN errnums.C_BIZ_RULE_INVALID;
    end if;*/

    SELECT TRADELOT into v_tradelot FROM SECURITIES_INFO where codeid = p_txmsg.txfields('04').value;

    if mod(p_txmsg.txfields('15').value,v_tradelot) > 0 then
       p_err_code := '-260153';
       plog.setendsection (pkgctx, 'fn_txAftAppUpdate');
       RETURN errnums.C_BIZ_RULE_INVALID;
    end if;
...
/*
    ...
    if v_strISVSD <> 'N' then
        ...
        update dfgroup set isvsd='N' where groupid = p_txmsg.txfields('05').value;
    else
       p_err_code := '-260036';
       plog.setendsection (pkgctx, 'fn_txAftAppUpdate');
       RETURN errnums.C_BIZ_RULE_INVALID;
