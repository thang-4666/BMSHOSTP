...
BEGIN
    ...
    ***************************************************************************************************/
    ...
    l_optcodeid:= substr(p_txmsg.txfields(c_optseacctno).value,11);


      IF p_txmsg.txfields(c_qtty).value >0 AND l_optcodeid IS NOT NULL  AND NOT  cspks_cfproc.fn_check_account_mrirate(p_txmsg.txfields(c_afacctno).value,l_optcodeid,p_txmsg.txfields(c_qtty).value,0 )   THEN
            p_err_code:='-180064';
            plog.setendsection (pkgctx, 'fn_txPreAppCheck');
            RETURN errnums.C_BIZ_RULE_INVALID;
       END IF;
