SET DEFINE OFF;
CREATE OR REPLACE PACKAGE txpks_#3371ex
/**----------------------------------------------------------------------------------------------------
 ** Package: TXPKS_#3371EX
...
CREATE OR REPLACE PACKAGE BODY txpks_#3371ex
IS
   pkgctx   plog.log_ctx;
...
BEGIN
    ...
    ***************************************************************************************************/
    ...
    IF p_txmsg.deltd <> 'Y' THEN -- NORMAL TRANSACTION
        ...
        -- Xoa du lieu cu.
        UPDATE CASCHD4P SET deltd = 'Y' WHERE camastid = l_camastid;
        -- Tao du lieu moi.
        IF length(l_camastid) > 0 THEN
            ...
            LOOP
                ...
                SELECT ROUND( (tdateotc-fdateotc+1)*interestrate) INTO L_INTERESTRATE_OTC FROM CAMAST WHERE CAMASTID =camast_rec.CAMASTID;
              ...
              ELSIF l_catype = '020' THEN --gc_CA_CATYPE_CONVERT_STOCK
               ...
               ELSIF l_catype = '023' THEN -- chuyen doi TP-CP dk nhan tien
                ...
                ELSIF l_catype = '027' THEN --gc_CA_CATYPE_BOND_PAY_INTEREST 'Bond pay interest, Lai suat theo thang, chu ky theo nam (+AMT)
                    l_amtexp:='(SUM(MST.TRADE + MST.MARGIN + MST.WTRADE  + MST.MORTAGE + MST.BLOCKWITHDRAW + MST.EMKQTTY + MST.BLOCKDTOCLOSE + MST.BLOCKED + MST.SECURED + MST.REPO+ MST.RECEIVING+ MST.DTOCLOSE+MST.WITHDRAW)-SUM(TR.AMOUNT))*MAX(SYM.PARVALUE)/(100)*' || L_INTERESTRATE_OTC||'/' || L_DAYOFYEAR ;
                    l_roundtype := 0;
                     L_ROUND:= ' (CASE WHEN (FLOOR( '|| l_amtexp  || ') ) <> '|| l_amtexp || ' THEN 1 ELSE 0 END)';
                    -- amt=intamt
                    l_intamtexp:='(SUM(MST.TRADE + MST.MARGIN + MST.WTRADE  + MST.MORTAGE + MST.BLOCKWITHDRAW + MST.EMKQTTY + MST.BLOCKDTOCLOSE + MST.BLOCKED + MST.SECURED + MST.REPO+ MST.RECEIVING+ MST.DTOCLOSE+MST.WITHDRAW)-SUM(TR.AMOUNT))*MAX(SYM.PARVALUE)/(100)*' || L_INTERESTRATE_OTC || '/' || L_DAYOFYEAR ;
...
/*                IF l_catype in ('010', '011', '015', '016', '017', '020', '021') THEN
                ...
                DELETE  FROM   CASCHDTEMP;
                    ...
                    l_sql := 'INSERT INTO CASCHDTEMP (AUTOID, CAMASTID, AFACCTNO, CODEID, EXCODEID, BALANCE, QTTY, AMT, AQTTY, AAMT, STATUS,REQTTY,REAQTTY,RETAILBAL,TRADE,INTAMT,RQTTY,ROUND)  '
                             ...
                             || ' FROM SBSECURITIES SYM, CAMAST CA, (SELECT * FROM  SEMAST WHERE CUSTID =''0001540677'')MST,  '
             ...
             /*   plog.error (pkgctx,'3375EX: ' || substr(l_sql,0,1500) );
                ...
                SELECT AUTOID,CAMASTID,BALANCE,QTTY,AMT,AQTTY,AAMT,CA.STATUS,AFACCTNO,CODEID,EXCODEID,DELTD,CA.PSTATUS,REFCAMASTID,RETAILSHARE,DEPOSIT,REQTTY,REAQTTY,RETAILBAL,PBALANCE,PQTTY,PAAMT,CA.COREBANK,ISCISE,DFQTTY,ISCI,ISSE,ISRO,TQTTY,TRADE,INBALANCE,OUTBALANCE,PITRATEMETHOD,ISEXEC,NMQTTY,DFAMT,INTAMT,RQTTY,ROUND
                FROM CASCHDTEMP CA, AFMAST AF
                WHERE CA.AFACCTNO =AF.ACCTNO
                AND CUSTID ='0001540677'
                                ;

                --Insert nhung khach hang co mot tieu khoan
                insert into CASCHD4P (AUTOID, CAMASTID, AFACCTNO, CODEID, EXCODEID, BALANCE, QTTY, AMT, AQTTY, AAMT, STATUS,REQTTY,REAQTTY,RETAILBAL,TRADE,PBALANCE,PQTTY,PAAMT,INTAMT,RQTTY)
                    ...
                    -- Tinh gia tri chung khoan cho quyen ve.
                              ...
                              SELECT COUNT(*) INTO l_count_temp FROM caschdtempGRP,afmast af
                                  ...
                                  if(l_count_temp>0  )    THEN
                                        INSERT INTO CASCHD4P (AUTOID, CAMASTID, AFACCTNO, CODEID, EXCODEID, BALANCE, QTTY, AMT, AQTTY, AAMT, STATUS,REQTTY,REAQTTY,RETAILBAL,TRADE,PBALANCE,PQTTY,PAAMT,INTAMT,RQTTY,ORGPBALANCE)
                                                ...
                                                (l_dbl_INTAMTexp-nvl(schdsum.INTAMT,0)) INTAMT,
                                                 ...
                                                 (l_dbl_RQTTYexp-nvl(schdsum.RQTTY,0)) RQTTY ,
                                                  ...
                                                  FROM (select * from (SELECT temp.* FROM caschdtempGRP temp,afmast
                                                        ...
                                                        FROM CASCHD4P, afmast
                                                        WHERE camastid=l_camastid AND afmast.acctno=CASCHD4P.afacctno
